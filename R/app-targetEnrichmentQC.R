      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          designFile = ezFrame(
            Type = "character",
            DefaultValue = "",
            Description = "file describing the regions selected by the enrichment kit"
          ),
          covUniformityPlot = ezFrame(
            Type = "logical",
            DefaultValue = "TRUE",
            Description = "generate plots for coverage uniformity?"
          ),
          covTargetLengthPlot = ezFrame(
            Type = "logical",
            DefaultValue = "TRUE",
            Description = "generate plots for coverage vs. target length"
          ),
          duplicatesPlot = ezFrame(
            Type = "logical",
            DefaultValue = "TRUE",
            Description = "generate plots for duplicates"
          ),
          removeDuplicates = ezFrame(
            Type = "logical",
            DefaultValue = "TRUE",
            Description = "remove Duplicates for CovCalculations"
          )
        )
      }
    )
...
ezMethodTeqc = function(input = NA, output = NA, param = NA) {
  ...
  if (basename(param$designFile) == param$designFile) {
    param$designFile = list.files(
      file.path(TEQC_DESIGN_DIR, param$designFile),
      pattern = 'Covered\\.bed$',
      full.names = T
    )[1]
  }
  ...
  TEQC::multiTEQCreport(
    singleReportDirs = destDirs,
    samplenames = samples,
...
runTEQC = function(file, param) {
  ...
  genomeSize = sum(as.numeric(system(
    paste("samtools", "view -H", file, "|grep @SQ|cut -f 3|sed 's/LN://'"),
    intern = T
  )))

  reads <- ezReadBamFileAsGRanges(
    file,
    chromosomes = NULL,
    pairedEndReads = param$paired,
    max.fragment.width = 1000,
    min.mapq = 10,
    remove.duplicate.reads = param$removeDuplicates
  )
  ...
  seqlevels(reads) <- c(
    levels(seqnames(reads)),
    as.character(setdiff(seqnames(targets), seqnames(reads)))
  )
  reads <- keepSeqlevels(
    reads,
    levels(seqnames(targets)),
    pruning.mode = "coarse"
  )
  ...
  TEQC::TEQCreport(
    ...
    CovUniformityPlot = param$covUniformityPlot,
    CovTargetLengthPlot = param$covTargetLengthPlot,
    duplicatesPlot = param$duplicatesPlot, #CovGCPlot = T,
    k = c(1, 5, 10, 20, 30, 50),
