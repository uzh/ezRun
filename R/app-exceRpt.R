...
pasteExcerptCmd = function(readFile, outputDir, param) {
  cmd = paste(
    ...
    paste0(
      'RANDOM_BARCODE_LOCATION=',
      "'",
      param[['RANDOM_BARCODE_LOCATION']],
      "'"
    ),
    ...
    paste0(
      ...
      param[['STAR_outFilterMatchNminOverLread']]
    ),
    paste0(
      'STAR_outFilterMismatchNmax=',
      param[['STAR_outFilterMismatchNmax']]
    ),

    ## output options
    paste0(
      'REMOVE_LARGE_INTERMEDIATE_FILES=',
      param[['REMOVE_LARGE_INTERMEDIATE_FILES']]
    )
  )
  return(cmd)
}
