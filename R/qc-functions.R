...
ezComputeBias = function(
  ...
  outMeta$'bootstrappedCount [File]' = paste0(
    rownames(outMeta),
    "-bootstrap.h5"
  )
  ...
  if (is.null(param)) {
    ...
    param[['cut_tail_mean_quality']] = '15'
  ...
  if (is.na(paramList$refBuild)) {
    ...
    if (is.na(paramList$refBuild)) {
      stop(paste(
        "refBuild not defined for species:",
        refBuildMap[inputMeta$Species[1]]
      ))
    }
  }
  ...
  if (is.null(dsName)) {
    ...
    plates = paste(
      unique(inputMeta$`PlateName [Characteristic]`),
      collapse = "_"
    )
  ...
  for (i in 1:nrow(inputMeta)) {
    ...
    sampledTranscriptCov <- ezMclapply(
      ...
      mc.preschedule = TRUE,
      mc.cores = min(as.integer(param$cores), 4)
    )
      ...
      if (sum(isInLc) > 50) {
        ...
        if (length(avgCovQuant) == 2) {
          ...
          for (cc in levels(covClasses)) {
            genebody_coverage[[lc]][[cc]] = rowMeans(relativeCov[,
              covClasses == cc,
              drop = FALSE
            ])
          }
  ...
  rawData = subset(
    rawData,
    select = colSums(assays(rawData)$counts >= minReadsPerGene) >= 100
  )
  counts = assays(rawData)$counts
  nGenes = colSums(counts >= param$sigThresh)
  meta = ezFrame(colData(rawData))
  assays(rawData)$signal = ezNorm(
    assays(rawData)$counts,
  ...
  widthTypes = data.frame(
    "width < 800nt" = as.numeric(rowData(rawData)$featWidth) < 800,
    "width > 4000nt" = as.numeric(rowData(rawData)$featWidth) > 4000,
    check.names = FALSE
  )
  widthEffect = apply(logRatio, 2, function(x) {
    mean(x[widthTypes$`width > 4000nt`], na.rm = TRUE) -
      mean(x[widthTypes$`width < 800nt`], na.rm = TRUE)
  })
  ...
  countDs$"Read Count"[
    countDs$"Read Count" > param$maxReadsPerSample
  ] = param$maxReadsPerSample
  ...
  smoothScatter(
    ...
    xlim = c(-1, 1),
    ylim = c(-1, 1),
    main = paste(dsName, tag),
    nrpoints = 0,
    nbin = 40,
    bandwidth = 0.05,
    cex.main = 0.8,
    colramp = colorRampPalette(c("white", blues9[1:6]))
  )
  ...
  ctsCorr = ezCorrectBias(
    counts,
    seqAnno$gc,
    seqAnno$featWidth,
    logWidthBreaks = logWidthBreaks,
    gcBreaks = gcBreaks,
    widthOffset = 100
  )
  ...
  image(
    1:nrow(mat),
    1:ncol(mat),
    shrinkToRange(mat, lim),
    col = getBlueRedScale(),
    breaks = seq(from = lim[1], to = lim[2], length.out = 257),
    axes = FALSE,
    xlim = c(0.5, nrow(mat) + 0.5),
    ylim = c(0.5, ncol(mat) + 0.5),
    ylab = "GC",
    xlab = "width"
  )
  ...
  plot(
    ...
    type = "n",
    ylim = c(-1, 1),
    xlim = c(1, length(gcEffect) * 1.1)
  )
  text(
    1:length(gcEffect),
    gcEffect,
    labels = names(gcEffect),
    cex = 0.8,
    pos = 4
  )

  smoothScatter(
    biasScores[, "medium genes"],
    biasScores[, "long genes"],
    xlim = c(0.1, 0.5),
    ylim = c(0.1, 0.5),
    xlab = "medium genes",
    ylab = "long genes",
  ...
  readCount = countDs[names(nGenes), "Read Count"] *
    exp(runif(length(nGenes), min = -0.1, max = 0.1))
  plot(
    readCount,
    nGenes,
    log = "xy",
  ...
  text(readCount, nGenes, labels = ptLabels, cex = 0.8)
  dev.off()
}
