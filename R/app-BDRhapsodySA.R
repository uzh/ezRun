...
ezMethodBdRhapsodySA <- function(input = NA, output = NA, param = NA) {
  ...
  cellsMexUnfilteredOutput <- basename(output$getColumn(
    "UnfilteredCountMatrix"
  ))
  ezSystem(sprintf(
    "unzip %s.zip -d %s",
    cellsMexUnfilteredOutput,
    cellsMexUnfilteredOutput
  ))
...
makeBdYmlFile <- function(input, param, bdRef) {
  ...
  bdParams <- list(
    "cwl:tools" = "Rhapsody",
    "Reads" = c(
      lapply(input$getFullPathsList("Read1")[[1]], function(x) {
        list("class" = "File", "location" = x)
      }),
      lapply(input$getFullPathsList("Read2")[[1]], function(x) {
        list("class" = "File", "location" = x)
      })
    ),
...
makeBdRhapRefYmlFile <- function(fasta_path, gtf_path, archive_prefix, param) {
  ...
  bdParams <- list(
    "cwl:tools" = "Rhapsody",
    "Genome_fasta" = list(
      list("class" = "File", "location" = fasta_path)
    ),
    "Gtf" = list(
      list("class" = "File", "location" = gtf_path)
    ),
...
getBdWtaReference <- function(param) {
  ...
  if (ezIsSpecified(param$controlSeqs)) {
    ...
    refDir <- sub(
      "\\.gtf$",
      paste0("_BD-WTA_SC_", bdWtaBase, "_Index"),
...
postProcessTagResults <- function(param, output, sampleName) {
  ...
  for (i in seq_along(tagNums)) {
    myZipFile <- Sys.glob(sprintf(
      "%s_SampleTag%02d*.zip",
      sampleNameMut,
      tagNums[i]
    ))
    sampleTagZips[i] <- switch(
      as.character(length(myZipFile)),
      "0" = '',
      "1" = myZipFile
    )
  }
