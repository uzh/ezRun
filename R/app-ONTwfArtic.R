...
ezMethodONTwfArtic <- function(input = NA, output = NA, param = NA) {
  ...
  ezSystem(cmdcopy)
  cmd = paste(
    "nextflow run /srv/GT/software/epi2me-labs/wf-artic/",
    "--fastq",
    dataset,
    "--sample_sheet",
    basename(input$getFullPaths("SampleSheet")),
    "--normalise",
    normalise_param,
    "--scheme_version",
    scheme_version_param,
    "-profile singularity"
  )
  ezSystem(cmd)
  return("Success")
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          normalise = ezFrame(
            Type = "integer",
            DefaultValue = "200",
            Description = "depth ceiling for depth of coverage normalization"
          ),
          schemeVersion = ezFrame(
            Type = "character",
            DefaultValue = "ARTIC/V3",
