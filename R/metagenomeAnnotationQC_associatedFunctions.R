...
prodigalFileReport <- function(x, meth) {
  ...
  prodigalSummaryDF <- data.frame(
    mcols(prodigalGffImport),
    stringsAsFactors = F
  )
  ...
  subsetDataToPartial00DF <- prodigalSummaryDF[
    prodigalSummaryDF$partial == "00" &
      (prodigalSummaryDF$start_type == "ATG" |
        prodigalSummaryDF$start_type == "GTG"),
  ]
  subsetDataToPartial00DF$method <- meth
  return(list(
    fullSumm = prodigalSummaryDF,
    subsetDataToPartial00 = subsetDataToPartial00DF
  ))
}
...
extractTopN <- function(DF, column, N) {
  ...
  colnames(col) <- column
  topN <- data.frame(
    cbind(col, abundance = tail(as.data.frame(tab_s)$Freq, N)),
    stringsAsFactors = F
  )
...
interproscanFileReport <- function(x, meth, N) {
  ...
  IPSGffSummaryDF <- data.frame(
    score = as.numeric(mcols(IPSGffImport)$score),
    description = description,
    GOterm = ontology,
    type = mcols(IPSGffImport)$type,
    stringsAsFactors = F
  )
  IPSGffSummaryDF <- IPSGffSummaryDF[
    IPSGffSummaryDF$type == "protein_match",
...
summaryScorePlot <- function(x) {
  p <- ggplot(x, aes(x = score, fill = method)) +
...
summaryConfPlot <- function(x) {
  p <- ggplot(x, aes(x = conf, fill = method)) +
    geom_histogram(binwidth = 5, position = "dodge") +
    facet_grid(
      rows = vars(start_type),
      cols = vars(partial),
      labeller = label_both
    ) +
...
summaryGcContPlot <- function(x) {
  p <- ggplot(x, aes(x = gc_cont, fill = method)) +
    ...
    labs(title = "Summary of the GC-content")
  return(p)
}

### summary rbs_spacer hist
summaryRBSSpacePlot <- function(x) {
  p <- ggplot(x, aes(x = rbs_spacer, fill = method)) +
...
summaryMatchScorePlot <- function(x) {
  p <- ggplot(x, aes(x = -log(score), fill = sample)) +
...
summaryGOPlot <- function(x, numberOfTopNCategories) {
  ...
  GOdesc <- sapply(
    DFforSummProtGO$GOterm,
    function(x) Term(GOTERM)[names(Term(GOTERM)) %in% x],
    USE.NAMES = F
  )
  ...
  basicPlot <- ggplot(
    DFforSummProtGO,
    aes(x = reorder(GOterm, -abundance), y = abundance, fill = method)
  )
  withThemes <- basicPlot +
  ...
  p <- withThemes +
    ...
    aes(stringr::str_wrap(GOterm, 30)) +
    ylab(NULL) +
    coord_flip()
  return(p)
}
...
summaryFamilyPlot <- function(x, numberOfTopNCategories) {
  ...
  expandedPalette <- getPalette(numberOfTopNCategories)
  basicPlot <- ggplot(
    DFforSummProtFamilies,
    aes(x = reorder(description, -abundance), y = abundance, fill = method)
  )
  withThemes <- basicPlot +
    geom_bar(stat = "Identity", position = "dodge") +
    theme(
  ...
  p <- withThemes +
    ...
    guides(fill = guide_legend(ncol = 1, byrow = F, title.position = "top")) +
    aes(stringr::str_wrap(description, 30)) +
    ylab(NULL) +
    coord_flip()
...
summaryMetagenomeBins <- function(krakenFile, binFiles) {
  ...
  taxonSummary <- lapply(krakenLabels$orgName, function(x) {
    y <- unlist(strsplit(x, ";"))
    y <- c(y[1:2], tail(y, 7))
...
summaryHeatmap <- function(
  ...
  plot_heatmap_Pheatmap <- function() {
    ...
    mergedBinAbundTableOrdered <- mergedBinAbundTable[
      order(mergedBinAbundTable$sdV, decreasing = T),
    ]
    mergedBinAbundTableNoSdV <- subset(
      mergedBinAbundTableOrdered,
      select = -c(sdV)
    )
    fontsize = 8
    if (isGroupThere) {
      mergedBinAbundTableTopNToPlot <- head(
        mergedBinAbundTableNoSdV,
        numberOfTopNCategories
      )
      cellwidth = max(1, 25 - (nrow(dataset)))
    } else {
      mergedBinAbundTableTopN <- head(
        mergedBinAbundTableNoSdV,
        numberOfTopNCategories
      )
