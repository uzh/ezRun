...
ezMethodRunJunctionSeqPipeline <- function(
  input,
  output,
  param,
...
runQoRTs <- function(input, output, param) {
  ...
  javaCall = paste0(
    ...
    param$ram,
    'G',
    ' -jar /usr/local/ngseq/src/QoRTs/QoRTs.jar'
  )
  #1. Get BAM-Files:
  ...
  for (i in c(1:length(bamFiles))) {
    ...
    if (param$paired) {
      ...
      cmd = paste(
        javaCall,
        'QC --stranded --singleEnded --maxReadLength',
        param$maxReadLength,
        '--runFunctions writeKnownSplices,writeNovelSplices,writeSpliceExon',
        bamFiles[i],
        gtfFile,
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          maxReadLength = ezFrame(
            Type = "numeric",
            DefaultValue = 151,
            Description = "define max read length"
          ),
          minCount = ezFrame(
            Type = "numeric",
            DefaultValue = 6,
            Description = "min Junction Count"
          ),
          maxGenes = ezFrame(
            Type = "numeric",
            DefaultValue = 1000,
            Description = "max number of genes to plot"
          )
        )
      }
    )
