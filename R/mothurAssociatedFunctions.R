...
createSummaryTable <- function(summary) {
  part2 <- vector()
  part1 <- apply(subset(summary, select = start:polymer), 2, function(x) {
  ...
  rownames(rawDataSummaryTable) <- c(
    "Mininmun",
    "2.5%-tile",
    "25%-tile",
    "Median",
...
datasetToMothur <- function(sushiInputDataset, param) {
  for (i in (1:nrow(sushiInputDataset))) {
    filePathInDatset <- paste0(
      param$dataRoot,
      "/",
      sushiInputDataset$`Read1 [File]`[i]
    )
...
otuSaturationTable <- function(sharedFile) {
  ...
  tempList <- apply(cumSumTransform, 2, function(y) {
    ldply(lapply(seq(from = 10, to = 100, by = 10), function(x) {
...
countAndAssignSeqsFromFasta <- function(fastaFile, filterStep, groupFile) {
  ...
  if (fastaFile == "Mothur.fasta") {
    ...
    finaldDF <- cbind(
      data.frame(table(groupDesc[relIndex, ])),
      fStep = filterStep
    )
  }
