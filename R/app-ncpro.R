...
ezMethodNcpro = function(input = NA, output = NA, param = NA) {
  ...
  postProcessResults(
    dataset = input$meta,
    psInputFn = input$file,
    psReportDir = output$getColumn("Report")
  )
  return("Success")
}
...
ncpro = function(input, dataset, param = NULL) {
  ...
  ncproConfigFile = list.files(
    paste0(NCPRO_ANNOTATION_DIR, "/config-templates/"),
    pattern = paste0('-', buildName, '-'),
    full.names = T
  )[1]
  if (is.na(ncproConfigFile)) {
    stop(paste0(
      "No ncpro config-template for Genome-Build ",
      param[['refBuild']],
      " available."
    ))
  }
  ...
  if (
    !is.null(dataset$"Read Count") &&
      is.numeric(dataset$"Read Count") &&
      all(dataset$"Read Count" > 0)
  ) {
  ...
  png(
    param$readCountsBarplot,
    width = 400 + nrow(readCounts) * 10,
    height = 700
  )
  par(mar = c(12, 4.1, 4.1, 2.1))
  barplot(
    t(as.matrix(readCounts[, c("remaining", "removed")])),
    las = 2,
    border = NA,
    main = "Read Counts after trimming",
    legend.text = TRUE,
    col = c("gray30", "gray")
  )
  dev.off()
...
splitCounts <- function(psCountType, pvSamples, plCountParam) {
  ...
  for (sam in pvSamples) {
    ...
    if (!(sam %in% vColNames)) {
      stop(
        "ERROR in splitCounts, sample: ",
        sam,
        " not found in result file",
        sInputFile,
        "\n"
      )
    }
...
modifyInput <- function(psCountCategory, pdataset, psInputFn, psReportDir) {
  ...
  vResultPaths <- as.vector(sapply(vSampleNames, function(sam) {
    file.path(psReportDir, psCountCategory, paste(sam, "txt", sep = "."))
  }))
  ...
  colnames(dfDataSet)[which(
    colnames(dfDataSet) == "Read1 [File]"
  )] <- "Count [File]"
  ...
  write.table(
    dfDataSet,
    file = file.path('.', paste0(psCountCategory, '_dataset.tsv')),
    quote = FALSE,
    sep = "\t",
    row.names = FALSE
  )
...
lGetGlobalCountParam <- function() {
  return(list(
    countParam = list(
      allRNA = list(
        inputFile = "ncpro/doc/all_samples_all_subfamcov.data",
        resultDir = "allRNA",
        nSampleNameColIdx = 1
      ),
      mature_miRNA = list(
        inputFile = "ncpro/doc/mature_miRNA_miRNA_e_+2_+2_all_samples_subfamcov.data",
        resultDir = "mature_miRNA",
        nSampleNameColIdx = 1
      ),
      precursor_miRNA = list(
        inputFile = "ncpro/doc/precursor_miRNA_miRNA_all_samples_subfamcov.data",
