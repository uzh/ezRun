...
phyloSeqPreprocess <- function(phyloseqObj, rawCount, sampleFraction) {
  ...
  filteredTaxa <- filter_taxa(
    phyloseqObj,
    function(x) sum(x > rawCount) > (sampleFraction * length(x)),
    TRUE
  )
...
phyloSeqToDeseq2_tableAndPlots <- function(
  phyloseqObj,
  rank,
  group,
  sampleGroup,
  ...
  phyloseqObjNoMock <- prune_samples(
    sample_data(phyloseqObj)[[group]] != "Mock",
    phyloseqObj
  )
  phyloseqToDeseq2Obj = phyloseq_to_deseq2(
    phyloseqObjNoMock,
  ...
  phyloseqToDeseq2Obj = estimateSizeFactors(
    phyloseqToDeseq2Obj,
    geoMeans = geoMeans
  )
  DEseqPhyRes <- DESeq(
  ...
  otuObj <- data.frame(
    t(otu_table(phyloseqObjNoMock)@.Data),
    check.names = F,
    stringsAsFactors = F
  )
  taxObj <- data.frame(
    tax_table(phyloseqObjNoMock)@.Data,
    check.names = F,
    stringsAsFactors = F
  )
  taxObj[is.na(taxObj)] = "NA"
  addTaxaOut <- cbind(data.frame(res), otuObj, taxObj)
  addTaxaOut <- addTaxaOut[
    !is.na(addTaxaOut$Kingdom) & !is.na(addTaxaOut$padj),
  ]
  ...
  addTaxa$Significance <- factor(
    addTaxa$Significance,
  ...
  plotLogFoldVsTaxon <- ggplot(
    addTaxa,
    aes(x = addTaxa[[rank]], y = log2FoldChange, color = addTaxa[[rank]])
  ) +
    geom_violin() +
    geom_point(size = 3) +
    theme(
    ...
    geom_hline(aes(yintercept = -1), color = "blue")
  plotLogFoldVsTaxon <- plotLogFoldVsTaxon +
    labs(title = title) +
  ...
  if (isAllNa) {
    isAllNaMsg <- paste(
      "No differentially abundant  OTUs are annotated to the rank",
      rank,
      ". No pie chart to plot."
    )
    ...
    pct = paste0(pct, "%")
    titleText = paste(
      "Distribution of differentially abundant taxa at rank",
      rank,
      "."
    )
    ...
    finalVersionPie <- pieVersion +
      labs(title = titleText, y = "") +
      theme(plot.title = element_text(size = 10, hjust = 0.5)) +
      labs(fill = rank)
  }
  return(list(
    logPlot = plotLogFoldVsTaxon,
    vPlot = volcanoPlot,
    pieChart = finalVersionPie,
    tableToReport = tableToReport,
    fullTable = addTaxa,
    isAllNaMsg = isAllNaMsg,
    isAllNa = isAllNa
  ))
}
...
phyloSeqDivPlotAndPercUnclassified <- function(taxaFileName, rank) {
  ...
  percUnclassified <- length(grep("unclassified", taxaDF[, rank])) /
    length(taxaDF[, rank]) *
    100
  ...
  bp <- ggplot(tableTaxa, aes(x = "", y = Freq, fill = get(rank))) +
    geom_bar(width = 10, stat = "identity") +
    scale_fill_manual(values = colRain)
  pieVersion <- bp + coord_polar("y", start = 0)
  finalVersion <- pieVersion +
    labs(title = titleText, subtitle = subtitleText, y = "") +
    theme(
...
phyloSeqCommunityComp <- function(physeq) {
  sampleToKeep <- rownames(sample_data(physeq))[grep(
    "mock",
  ...
  joinedData <- data.frame(
    cbind(
      Freq = otus[, sampleToKeep],
      Genus = data.frame(tax_table(physeq), stringsAsFactors = FALSE)$Genus
    ),
    stringsAsFactors = FALSE
  )
  joinedData$Freq <- as.numeric(joinedData$Freq)
  fractions <- lapply(unique(joinedData$Genus), function(x) {
    cbind(
      x,
      sum(joinedData[joinedData$Genus == x, ]$Freq) / sum(joinedData$Freq) * 100
    )
  })
  fractions <- data.frame(
    matrix(unlist(fractions), byrow = TRUE, ncol = 2),
    stringsAsFactors = FALSE
  )
...
pcaForPhyloseqPlot <- function(phySeqObject, type, group) {
  ...
  ggg <- data.frame(
    comp1 = mds$cmdscale.out[, 1],
    comp2 = mds$cmdscale.out[, 2],
    group = groups
  )
  ...
  ## plot
  g <- ggplot(ggg, aes(comp1, comp2, group = ggg[[group]])) +
    geom_point(aes(colour = ggg[[group]]), size = 3)
...
##'
### Heatmap function
heatmapForPhylotseqPlotPheatmap <- function(
  phyloseqOtuObj,
  areThereMultVar,
  isGroupThere,
  rank
  ...
  plot_heatmap_Pheatmap <- function() {
    if (isGroupThere) {
      ...
      mat_col <- data.frame(
        lapply(mat_col_temp, as.factor),
        row.names = rownames(mat_col_temp)
      )
      if (!areThereMultVar) {
...
rarefactionPlot <- function(adundDF, type) {
  ...
  fortifiedObjLine$method <- factor(
    fortifiedObjLine$method,
...
plotBarMod <- function(
  ...
  if (x == "S") {
    ...
    for (sample in unique(mdf$Sample)) {
      ...
      mdf[mdf$Sample == sample, ]$relFract <- mdf[
        mdf$Sample == sample,
      ]$Abundance /
        tot
    }
    ...
    for (g in levels(mdf[[group]])) {
      ...
      mdf[mdf[[group]] == g, ]$relFract <- mdf[mdf[[group]] == g, ]$Abundance /
        tot
    }
  }
...
subsetRankTopN <- function(physeqFullObject, rank, N) {
  phylAsum = tapply(
    taxa_sums(physeqFullObject),
    tax_table(physeqFullObject)[, rank],
    sum,
    na.rm = TRUE
  )
  topN = names(sort(phylAsum, TRUE))[1:N]
  physeqFullObjectTrimmed = prune_taxa(
    (tax_table(physeqFullObject)[, rank] %in% topN),
    physeqFullObject
  )
...
chimeraSummaryPlot <- function(chimeraDF) {
  ...
  finalVersionChimeraPlot <- finalVersionChimeraPlot +
    geom_text(
      aes(y = Freq + 500, label = paste0(pct, '%')),
...
communityPercSummTable <- function(phyloseqOtuObj, rank) {
  ...
  percTable <- apply(aggrDF, 2, function(y) {
    sapply(y, function(x) round(x / sum(y) * 100, 2))
  })
