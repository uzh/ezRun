...
ezMethodCov19QC <- function(
  ...
  mappingResult <- mapToCovidGenome(
    param,
    input = inputProc,
    workDir = "mappingResult"
  )

  ###4. Report results (% Primer Dimer, % Mapped Reads, AvgCoverage, StdCoverage)
  dataset <- data.frame(
    Name = rownames(input$meta),
    input$meta,
  ...
  callCmd = paste("bcftools", "call", "-Ou", param$callOptions, "-") ## read from stdin
  filterCmd = paste(
    "bcftools",
    "filter",
    "--output-type z",
  ...
  file <- file.path(system.file("templates", package = "ezRun"), "Cov19QC.Rmd")
...
getAdapterStats <- function(param, input) {
  ...
  for (nm in names(inputFiles)) {
    ...
    myCmd <- paste(
      ...
      '|head -n 80000|grep',
      paste0('"^.*', pattern, '"'),
      '-o |while read LINE; do echo $LINE | wc -m; done'
    )
    adapterPositions <- as.numeric(system(myCmd, intern = TRUE)) -
      nchar(pattern)
    result[['adapterDimerFraction']][i] <- sum(
      adapterPositions <= 10,
      na.rm = TRUE
    ) /
      readCount
...
mapToCovidGenome <- function(param, input, workDir) {
  ...
  for (nm in names(R1_files)) {
    ...
    if (nReads >= param$minReads) {
      ...
      result[['mappingRate']][i] <- 100 *
        (sum(mappingStats[2:length(mappingStats)]) / nReads)
      if (length(mappingStats) > 1) {
        reads <- ezReadBamFileAsGRanges(
          bamFile,
          chromosomes = NULL,
          pairedEndReads = param$paired,
          max.fragment.width = 5000,
          min.mapq = 10,
        ...
        result[['minCov']][i] <- 100 *
          (sum(as.vector(cov[[1]]) >= param$minCov) /
            length(as.vector(cov[[1]])))
        #file.remove(bamFile)
      }
    }
  }
    ...
    methods = list(
      initialize = function() {
        ...
        appDefaults <<- rbind(
          Adapter1 = ezFrame(
            Type = "character",
            DefaultValue = "CTGTCTCTTATACACATCT",
