...
getRpkm <- function(rawData) {
  ...
  rpkm <- sweep(
    assays(rawData)$counts * 1e9,
    MARGIN = 1,
    STATS = rowData(rawData)$featWidth,
    FUN = "/"
  )
...
getTpm <- function(rawData) {
  ...
  #   }
  if (
    ezIsSpecified(metadata(rawData)$param$scProtocol) &&
      toupper(metadata(rawData)$param$scProtocol) == "10X"
  ) {
    require(scuttle)
    ...
    tpm <- sweep(tpm * 1e6, MARGIN = 2, STATS = Matrix::colSums(tpm), FUN = "/")
  }
  return(tpm)
}
...
aggregateCountsByGene <- function(rawData) {
  ...
  if (is.null(genes)) {
    return(list(
      error = paste(
        "gene summaries requested but not gene column available. did you specify the build?<br>column names tried:<br>",
  ...
  for (nm in setdiff(names(assays(rawData)), "presentFlag")) {
    newRawCounts[[nm]] = as.matrix(averageRows(
      assays(rawData)[[nm]],
      genes,
      func = sum
    ))[rownames(seqAnnoNew), ]
