...
ezMethodRSEM = function(input = NA, output = NA, param = NA) {
  ...
  samtoolsSortMem = paste0(
    round((as.numeric(param$ram) / ezThreads() / 2) * 1000),
    "M"
  )
  ...
  if (ezIsSpecified(param$transcriptFasta)) {
    ...
    result <- result[
      match(transcriptResult$transcript_id, result$transcript_id),
      intersect(colnames(result), c("transcript_id", "gene_id", "width", "gc"))
    ]
  }
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          "bowtie-e" = ezFrame(
            Type = "integer",
            DefaultValue = "200",
            Description = "maximum sum of mismatch base qualities"
          ),
          keepBam = ezFrame(
            Type = "logical",
            DefaultValue = FALSE,
            Description = "if the alignment files should be kept"
          ),
          transcriptFasta = ezFrame(
            Type = "character",
            DefaultValue = '',
            Description = "full path to a transcript fasta file (e.g. trinity output); must be in a writeable directory;"
          )
        )
      }
    )
...
getRSEMReference = function(param) {
  if (ezIsSpecified(param$transcriptFasta)) {
    ...
    refBase = ifelse(
      ...
      sub(
        ".gtf$",
        paste0("_", rsemBase, "_RSEMIndex/transcripts"),
  ...
  if (ezIsSpecified(param$transcriptFasta)) {
    ...
    trxNames = sub(
      " .*",
      "",
      names(readDNAStringSet(param$transcriptFasta, nrec = 100))
    )
