...
ezMethodMageckCountQC = function(
  ...
  param <- ezParam(list(
    refBuild = 'Homo_sapiens/GENCODE/GRCh38.p13/Annotation/Release_42-2023-01-30'
  ))
  ...
  makeRmdReport(
    rawData = rawData,
    rmdFile = "CrisprCountQC.Rmd",
    reportTitle = "ExploreCounts",
    selfContained = TRUE
  )
  return("Success")
}
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          ...
          nSampleClusters = ezFrame(
            Type = "numeric",
            DefaultValue = 6,
            Description = "Number of SampleClusters, max value 6"
          )
        )
      }
    )
...
loadMageckCountDataset <- function(input, param) {
  ...
  if (dataFeatureLevel == "isoform" && param$featureLevel == "gene") {
    ...
    x <- select(x, -id) %>% group_by(gene_id) %>% summarise_all(funs(sum))
