...
ezMethodMothur = function(
  ...
  for (sampleName in sampleNames) {
    fastqFileToRead <- readDNAStringSet(
      paste0(sampleName, ".fastq"),
      format = "fastq"
    )
    readNames <- ldply(strsplit(names(fastqFileToRead), " "), function(x) {
      x[1]
    })$V1
    ...
    fastaFileToWrite <- writeXStringSet(
      fastqFileToRead,
      fastaOutName,
      append = TRUE
    )
  ...
  updateBatchPart1Cmd1 <- paste0(
    "sed -e s/\"###seq.error\"/",
    mockString,
    "/g",
    " -e s/\"CUTOFF_TAXON\"/",
    param$cutOffTaxonomy,
    "/g",
    " -e s/\"CUTOFF_CLUST\"/",
    param$cutOffCluster,
    "/g",
  ...
  cmdMothur2 = paste(
    "[ -f merged.good.filter.unique.precluster.denovo.vsearch.accnos ] && mothur",
    UNIFIED_MOTHUR_WORKFLOW_PART2A,
    "|| mothur",
    UNIFIED_MOTHUR_WORKFLOW_PART2B
  )
  ...
  filterSteps <- c(
    "noFilter",
    "lengthAndHomopPreAlign",
    "deduplication",
    "aligned",
    "lengthAndHomopPostAlign",
    "dedupPostEndTrimming"
  )
  listOfFilteredSummaries <- mapply(
    countAndAssignSeqsFromFasta,
    fastaFiles,
    filterSteps,
    groupFile = groupFile,
    SIMPLIFY = FALSE
  ...
  taxaMothurFile <- paste(
    "merged.good.filter.unique.precluster.pick.opti_mcc",
    param$cutOffCluster,
    "cons.taxonomy",
    sep = "."
  )
    ...
    methods = list(
      initialize = function() {
        ...
        appDefaults <<- rbind(
          ...
          cutOffTaxonomy = ezFrame(
            Type = "integer",
            DefaultValue = "80",
            Description = "cut-off taxonomy"
          ),
          cutOffCluster = ezFrame(
            Type = "numeric",
            DefaultValue = "0.03",
            Description = "cut-off cluster"
          ),
          diffs = ezFrame(
            Type = "integer",
            DefaultValue = "0.03",
