...
errorRateSummaryPlot <- function(errorCountFileName, avReadLen) {
  ...
  overallErrorRate <- round(
    sum(errorTable$wrongBases) / sum(errorTable$totBases) * 100,
    2
  )
  pct <- cumsum(round(
    errorTable$Sequences / sum(errorTable$Sequences) * 100,
    2
  ))
  ...
  bp <- ggplot(errorTable, aes(x = "", y = Sequences, fill = Errors)) +
    geom_bar(width = 1, stat = "identity") +
    scale_fill_manual(
      breaks = c(0, 5, 10, 50),
      labels = lbls[c(1, 6, 11, 51)],
      values = col
    )
...
convStepTable <- function(convStepFile) {
  ...
  colnames(stepTable) <- c(
    colnames(stepTable)[2:length(colnames(stepTable))],
    "dum"
  )
...
otuSaturationTable <- function(sharedFile) {
  ...
  sharedAbundDF <- data.frame(data.matrix(data.frame(
    sharedAbund[rowToKeep, ],
    stringsAsFactors = FALSE
  )))
  ...
  tempList <- apply(cumSumTransform, 2, function(y) {
    ldply(lapply(seq(from = 20, to = 200, by = 20), function(x) {
      y[x] / y[nrow(cumSumTransform)] * 100
    }))
  })
  finalSaturationTable <- data.frame(
    matrix(unlist(tempList), nrow = 10, byrow = F),
    stringsAsFactors = FALSE
  )
  finalSaturationTable <- data.frame(cbind(
    seq(from = 20, to = 200, by = 20),
    finalSaturationTable
  ))
  colnames(finalSaturationTable) <- c("numOTUs", "PercOTUsCovered")
  finalSaturationTableDF <- data.frame(
    PercOTUsCovered = finalSaturationTable$PercOTUsCovered,
    stringsAsFactors = F
  )
