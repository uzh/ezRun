...
ezImportCellRangerBam <- function(
  file,
  selection,
  regionTag = c("E", "N", "I"),
  CBvalues = NULL
  ...
  param <- ScanBamParam(
    ...
    what = scanBamWhat(),
    tag = c("MD", myTags),
    flag = flag #, ## read the extratags too
  ...
  reads <- if (
    as.character(seqnames(selection)[1]) %in% names(scanBamHeader(bf)$targets)
  ) {
  ...
  useAlignment <- bitwAnd(reads$tag$xf, xfIsRepresentative) ==
    xfIsRepresentative &
...
plotCellRangerCoverage = function(
  gRanges,
  bamFiles,
  txdb,
  regionTag = c("E", "N", "I"),
  ...
  if (is.null(CBList)) {
    ...
    body(myImportFun) <- substitute(
      ezImportCellRangerBam(file, selection, myRegionTag),
      list(myRegionTag = force(regionTag))
    )
    for (nm in names(bamFiles)) {
      alTrackList[[nm]] <- AlignmentsTrack(
        bamFiles[nm],
        name = nm,
        isPaired = FALSE,
        type = plotType,
        importFunction = myImportFun
      )
    }
  } else {
    ...
    for (nm in names(CBList)) {
      ...
      myImportFun <- function(file, selection) {}
      body(myImportFun) <- substitute(
        ezImportCellRangerBam(file, selection, myRegionTag, myCB),
      ...
      alTrackList[[nm]] <- AlignmentsTrack(
        ...
        isPaired = FALSE,
        type = plotType,
        importFunction = force(myImportFun)
      )
    }
  }
  pdfFiles = character()
  ...
  for (chrom in names(grList)) {
    ...
    for (i in 1:length(gr)) {
      ...
      plotTracks(
        ...
        ylim = c(0, 40),
        lwd = 1,
        main = paste(names(gr)[i], paste(regionTag, collapse = "-"))
      )
      dev.off()
...
plotLocusCoverageProfile = function(
  gRanges,
  bamFiles,
  gtfFile = NULL,
  ylim = c(0, 40),
...
plotLocusAverageCoverageProfile = function(
  gRanges,
  bamFiles,
  grouping = NULL,
  gtfFile = NULL,
  ...
  for (strand in c("+", "-")) {
    ...
    for (i in 1:length(gRanges)) {
      ...
      coverageList[[strand]][[gene]] = ezMatrix(
        0.0,
        rows = names(bamFiles),
        cols = regionStart:regionEnd
      )
      for (nm in names(bamFiles)) {
        ga = ezReadGappedAlignments(
          bamFiles[nm],
          seqname = chrom,
          start = regionStart - 100,
          end = regionEnd + 100,
          strand = strand
        )
    ...
    for (i in 1:length(gr)) {
      ...
      for (strand in c("+", "-")) {
        ...
        dTrack = DataTrack(
          data = x,
          chromosome = chrom,
          start = as.integer(colnames(x)),
          end = as.integer(colnames(x)),
          genome = "foo",
          name = paste("coverage", strand),
          col.line = groupColors,
          groups = rownames(x),
          type = "l"
        )
...
getTranscriptCoverage = function(chrom, gff, reads, strandMode = "both") {
  ...
  exonCov <- getRangesCoverage(
    unlist(exonsByTx, use.names = FALSE),
    reads,
