...
goClusterTableRmd = function(param, clusterResult, seqAnno) {
  ...
  enrichrTable = ezMatrix(
    "",
    rows = 1:clusterResult$nClusters,
    cols = "Enrichr"
  )
  for (i in 1:clusterResult$nClusters) {
    genesToUse = rownames(seqAnno) %in%
      names(clusterResult$clusterNumbers)[clusterResult$clusterNumbers == i]
    genesList = paste(seqAnno$gene_name[genesToUse], collapse = "\\n")
    jsCall = paste0('enrich({list: "', genesList, '", popup: true});')
    enrichrTable[i, 1] = paste0(
    ...
    for (onto in ontologies) {
      ...
      goFrame = .getGoTermsAsTd(
        x,
        param$pValThreshFisher,
        param$minCountFisher,
        onto = onto
      )
    ...
    ktableCluster <- lapply(ktableCluster, function(x) {
      rbind(
        as.matrix(x),
        ezMatrix("", rows = seq_len(maxNrow - nrow(x)), cols = seq_len(ncol(x)))
      )
    })
  ...
  return(list(
    ktables = ktables,
    linkTable = linkTable,
    enrichrTable = enrichrTable
  ))
}


revigoUpDownTables <- function(param, goResult) {
  revigoLinks = ezMatrix(
    "",
    rows = c('enrichUp', 'enrichDown', 'enrichBoth'),
    cols = c('BP', 'MF', 'CC')
  )
  for (col in colnames(revigoLinks)) {
    for (row in rownames(revigoLinks)) {
      goSubResult = goResult[[col]][[row]]
      if (all(is.na(goSubResult))) {
        next
      }
      goSubResult = goSubResult[
        which(goSubResult$Pvalue < param$pValThreshFisher),
      ]
      if (nrow(goSubResult) > param$maxNumberGroupsDisplayed) {
        goSubResult = goSubResult[1:param$maxNumberGroupsDisplayed, ]
      }
      revigoLinks[row, col] = paste0(
        'http://revigo.irb.hr/?inputGoList=',
        paste(
          rownames(goSubResult),
          goSubResult[, 'Pvalue'],
          collapse = '%0D%0A'
        )
      )
      revigoLinks[row, col] <- kableExtra::text_spec(
        "ReViGO",
        format = "html",
  ...
  for (onto in names(goResult)) {
    ...
    for (sub in names(x)) {
      ...
      ezInteractiveTableRmd(
        ...
        title = paste(
          sub("enrich", "", sub),
          "enriched GO categories of ontology",
          onto
        )
      ) |>
        DT::saveWidget(linkTable[onto, sub])
      linkTable[onto, sub] = ezLink(linkTable[onto, sub], target = "_blank")
    }
  }
  for (sub in names(ktables)) {
    ### Add the ""
    maxNrow <- max(sapply(ktables[[sub]], nrow))
    ktables[[sub]] <- lapply(ktables[[sub]], function(x) {
      rbind(
        as.matrix(x),
        ezMatrix("", rows = seq_len(maxNrow - nrow(x)), cols = seq_len(ncol(x)))
      )
...
.getGoTermsAsTd = function(
  ...
  for (i in 1:length(goRoots)) {
    ...
    for (term in childTerms) {
      ...
      termLink <- paste0('http://amigo.geneontology.org/amigo/term/', term)
      termHypLink <- kableExtra::text_spec(
        term,
        format = "html",
        link = termLink
      )
      ...
      counts = append(
        counts,
        paste(x[term, "Count"], x[term, "Size"], sep = "/")
      )
    }
  }
