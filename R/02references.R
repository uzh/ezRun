...
EzRef <- function(param) {
  ...
  new(
    "EzRef",
    refBuild = refBuild,
    refBuildName = refBuildName,
    refBuildDir = refBuildDir,
    refVariantsDir = refVariantsDir,
    refAnnotationVersion = refAnnotationVersion,
    refIndex = refIndex,
    refAnnotationFile = refAnnotationFile,
    refFeatureFile = refFeatureFile,
    refFastaFile = refFastaFile,
    refChromSizesFile = refChromSizesFile
  )
}
...
buildRefDir <- function(
  ...
  if (!is.null(x@refAnnotationVersion)) {
    ...
    file.symlink(
      file.path(x@refAnnotationVersion, "Genes"),
  ...
  write_tsv(
    fai %>% dplyr::select(1:2),
    file = x@refChromSizesFile,
    col_names = FALSE
  )
  ...
  cmd <- paste(
    "java -Xms1g -Xmx10g -Djava.io.tmpdir=. -jar /misc/ngseq12/packages/Tools/Picard/3.2.0/picard.jar",
  ...
  if (!is.null(genesFile)) {
    ...
    ### genes.gtf
    export(
      gtf[gtf$gene_biotype %in% listBiotypes("genes")],
      con = file.path(gtfPath, "genes.gtf")
    )
    ### transcripts.only.gtf
    export(
      gtf[gtf$type %in% "transcript"],
...
listBiotypes <- function(
  ...
  ## genes.gtf
  genes <- unique(c(
    setdiff(unionBiotypes, pseudogene),
...
buildIgvGenome <- function(x) {
  ...
  genomeFileURL <- file.path(
    REF_HOST,
    str_replace(x@refBuild, "/Annotation.*$", ""),
  ...
  tryCatch(
    {
      ...
      transcriptGtf <- groupGff(
        gtf,
        grouping = gtf$transcript_id,
        type = "transcript"
      )
      transcriptGtf$attributes <- ezBuildAttributeField(transcriptGtf[, c(
        "transcript_id",
        "gene_id",
        "gene_name"
      )])
      transcriptGtf <- transcriptGtf[
        order(transcriptGtf$seqid, transcriptGtf$start, transcriptGtf$end),
      ]
...
makeBiomartFile <- function(gtf) {
  ...
  for (txid in txids) {
    ext <- paste0(
      "/xrefs/id/",
      txid,
      "?external_db=GO;all_levels=0;object_type=transcript"
    )
  ...
  tb1 <- as_tibble(tb1) %>%
    ...
    rename("Transcript stable ID" = ind, "GO term accession" = values)
  ...
  tb <- mutate(
    tb,
    `GO term accession` = ifelse(is.na(`GO domain`), NA, `GO term accession`)
  )
  return(tb)
}
