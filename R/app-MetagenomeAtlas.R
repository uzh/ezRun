...
ezMethodMetagenomeAtlas = function(
  input = NA,
  output = NA,
  param = NA,
  ...
  cmdInitiate = paste(
    "atlas init",
    "--db-dir /srv/GT/databases/metagenome_atlas_db/",
    "sub_folder/"
  )
  ...
  tx2 <- gsub(
    pattern = "final_binner: DASTool",
    replace = "final_binner: maxbin",
    x = tx
  )
  tx2 <- gsub(pattern = "- gtdb_tree", replace = "#- gtdb_tree", x = tx2)
  tx2 <- gsub(
    pattern = "- gtdb_taxonomy",
    replace = "#- gtdb_taxonomy",
    x = tx2
  )
  tx2 <- gsub(pattern = "- gtdb_tree", replace = "#- gtdb_tree", x = tx2)
  tx2 <- gsub(
  ...
  tx2 <- gsub(
    pattern = "#  - checkm_taxonomy",
    replace = "- checkm_taxonomy",
    x = tx2
  )
  ...
  styleFiles <- file.path(
    ...
    c("fgcz.css", markdownFile, "fgcz_header.html", "banner.png")
  )
  rmarkdown::render(
    input = markdownFile,
    envir = new.env(),
