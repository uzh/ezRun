...
ezMethodDnaQC <- function(
  ...
  if (nrow(input$meta) < 2) {
    stop(
      "Please provide at least two samples for DnaQC otherwise use the DnaBamStatsApp!"
    )
  }
...
computeDnaBamStats <- function(input, htmlFile, param, resultList = NULL) {
  ...
  for (sm in samples) {
    ...
    if (param$paired) {
      ...
      png(
        paste0('libComplexity_', sm, '.png'),
        width = 600,
        height = 500,
        res = 90
      )
      ezLibComplexity(
        readsDupFreq(files[sm]),
        main = paste(sm, 'Library Complexity')
      )
      dev.off()
    }
    ...
    cmd = paste(
      'unset DISPLAY; qualimap bamqc -bam',
      files[sm],
      '-c -nt',
      param[['cores']],
      paste0('--java-mem-size=', param[['ram']], 'G'),
      '-outdir',
      sm
    )
    ezSystem(cmd)
    ...
    resultList[[sm]]$dupRate = as.numeric(sub(
      '\\%',
      '',
      sub(
        '^.*duplication rate = ',
        '',
        all_data[grep('duplication rate', all_data)]
      )
    ))
    numReads = as.numeric(gsub(
      ',',
      '',
      sub(
        '\\%',
        ...
        sub(
          '^.*number of reads = ',
          '',
          all_data[grep('number of reads', all_data)]
        )
      )
    )) /
      2
      ...
      dupReads = as.numeric(gsub(
        ...
        sub(
          ...
          sub(
            ...
            '',
            all_data[grep('number of duplicated reads', all_data)]
          )
        )
      ))
      if (param$paired) {
    ...
    resultList[[sm]]$errorRate = as.numeric(gsub(
      '^.*general error rate = ',
      '',
      all_data[grep('general error rate', all_data)]
    ))
    resultList[[sm]]$insertRate = as.numeric(sub(
      '\\%',
      '',
      sub(
        'mapped reads with insertion percentage = ',
        '',
        all_data[grep('mapped reads with insertion percentage', all_data)]
      )
    ))
    resultList[[sm]]$delRate = as.numeric(sub(
      '\\%',
      '',
      sub(
        'mapped reads with deletion percentage = ',
        '',
        all_data[grep('mapped reads with deletion percentage', all_data)]
      )
    ))
    resultList[[sm]]$avgCoverage = as.numeric(sub(
      'X',
      '',
      sub(
        'mean coverageData = ',
        '',
        all_data[grep('mean coverageData', all_data)]
      )
    ))
  ...
  ezWrite.table(
    data.frame(SampleName = samples, Folder = samples),
    'sampleKey.txt',
    row.names = FALSE,
    col.names = FALSE
  )
  ...
  cmd = paste(
    'unset DISPLAY; qualimap multi-bamqc -d sampleKey.txt -outdir qualimap_MultiSample'
  )
  ezSystem(cmd)
  ...
  for (j in 2:length(sampleSummary)) {
    ...
    barplot(
      as.numeric(sampleSummary[[j]]),
      names.arg = sampleSummary[[1]],
      col = 'royalblue',
      main = names(sampleSummary)[j]
    )
    dev.off()
  }
  ...
  styleFiles <- file.path(
    ...
    c("fgcz.css", "DnaQC.Rmd", "fgcz_header.html", "banner.png")
  )
...
ezLibComplexity <- function(
  ...
  suppressWarnings(
    estimates$reads <- estimates$relative.size *
      total
  )
  plot(
