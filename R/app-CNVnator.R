...
annotateCNVs = function(file, param) {
  ...
  data = cbind(data, pos)
  data[['FeaturesInCnv']] = unlist(sapply(
    as.vector(featureInCNVs),
    paste,
    collapse = ','
  ))
  data[['GenesInCnv']] = unlist(sapply(
    as.vector(genesInCNVs),
    paste,
    collapse = ','
  ))
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          binSize = ezFrame(
            Type = "numeric",
            DefaultValue = "1000",
            Description = "set binSize"
          ),
          maxEVal = ezFrame(
            Type = "numeric",
            DefaultValue = "0.01",
            Description = "set maximum E-Value for CNV filtering"
          ),
          maxQ0 = ezFrame(
            Type = "numeric",
            DefaultValue = "0.1",
            Description = "set maximum fraction of low quality read mapping in CNV"
          )
        )
      }
    )
