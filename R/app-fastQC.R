  ...
  setRefClass(
    ...
    methods = list(
      ...
      generate_mm = function(script_paths = c(), log_paths = c(), output_dir = ".", dataset_id = NULL, ...) {
        md_path <- file.path(output_dir, paste0("mm_dataset_", dataset_id, ".md"))
        sh_path <- file.path(output_dir, paste0("mm_dataset_", dataset_id, ".sh"))
        writeLines(c(
          "## Materials & Methods",
          "",
        ...
        writeLines(c(
          "#!/bin/bash",
          "# Generated by EzAppFastqc$generate_mm()",
          "# This script is a placeholder produced by the FastQC MM override."
        ), sh_path)
        invisible(NULL)
      }
    )
