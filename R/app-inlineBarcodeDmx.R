...
ezMethodInlineBarcodeDmx <- function(input = NA, output = NA, param = NA) {
  ...
  indexInfo <- ezRead.table(
    input$meta[['indexFile']],
    row.names = NULL,
    sep = ','
  )
  ...
  repeat {
    ...
    for (i in 1:length(index)) {
      ...
      leftEnd <- vapply(endIndex(vp), .dummyFunction, c('endIndex' = 0))
      ...
      vp <- vmatchPattern(
        param$rightPattern,
        filteredReads,
        max.mismatch = param$maxMismatch
      )
      rightStart <- vapply(startIndex(vp), .dummyFunction, c('startIndex' = 0))
      rightStart[is.na(rightStart)] <- maxReadLength
      toSmall <- which(rightStart < (nchar(leftPattern)))
      rightStart[toSmall] <- maxReadLength
      filteredReads <- DNAStringSet(substr(
        filteredReads,
        nchar(leftPattern) + param$leftLinkerSize,
        rightStart
      ))
      filteredQual <- BStringSet(substr(
        filteredQual,
        nchar(leftPattern) + param$leftLinkerSize,
        rightStart
      ))
      filteredReads <- filteredReads[
        width(filteredReads) >= param$minReadLength &
          width(filteredReads) <= param$maxReadLength
      ]
      filteredID <- filteredID[
        width(filteredQual) >= param$minReadLength &
          width(filteredQual) <= param$maxReadLength
      ]
      filteredQual <- filteredQual[
        width(filteredQual) >= param$minReadLength &
          width(filteredQual) <= param$maxReadLength
      ]
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          barcodePos = ezFrame(
            Type = "numeric",
            DefaultValue = 8,
            Description = "StartPosition of Inline Barcode in Read1"
          ),
          leftLinkerSize = ezFrame(
            Type = "numeric",
            DefaultValue = 5,
            Description = "Size of 5-Prime Linker next to Inline Barcode"
          ),
          rightPattern = ezFrame(
            Type = "character",
            DefaultValue = "GTGTCAGTCACTTCCAG",
            Description = "3-Prime linker sequence for trimming"
          ),
          maxMismatch = ezFrame(
            Type = "numeric",
            DefaultValue = 1,
            Description = "Accepted mismatches in 3-Prime linker sequence"
          ),
          minReadLength = ezFrame(
            Type = "numeric",
            DefaultValue = 20,
            Description = "minimal read length after dmx, filtering and trimming"
          ),
          maxReadLength = ezFrame(
            Type = "numeric",
            DefaultValue = 50,
            Description = "maximal read length after dmx, filtering and trimming"
          )
        )
      }
    )
