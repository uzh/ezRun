...
ezMethodExceRptReport = function(input = NA, output = NA, param = NA) {
  ...
  plots = processSamples(
    samplePaths = samplePathsFilt,
...
readData = function(samplePathList, output.dir) {
  ...
  mapping.stats = matrix(
    0,
    nrow = length(samplePathList),
    ncol = 31,
    dimnames = list(
      1:length(samplePathList),
      c(
        "input",
        ...
        "repetitiveElements",
        "endogenous_gapped",
        "input_to_exogenous_miRNA",
        "exogenous_miRNA",
        "input_to_exogenous_rRNA",
        "exogenous_rRNA",
  ...
  qc.results = matrix(
    ...
    dimnames = list(
      ...
      c(
        ...
        "GenomeReads",
        "TranscriptomeReads",
        "TranscriptomeGenomeRatio",
        "TranscriptomeComplexity"
      )
    )
  )
  maxReadLength = 10000
  read.lengths = matrix(
    0,
    nrow = length(samplePathList),
    ncol = maxReadLength + 1,
    dimnames = list(1:length(samplePathList), 0:maxReadLength)
  )
  ...
  for (i in 1:length(samplePathList)) {
    ...
    if (length(x.start) > 0 && length(x.end) > 0) {
      tmp.start = strptime(
        unlist(strsplit(tmp.stats[x.start[1], 1], "Run started at "))[2],
        "%Y-%m-%d--%H:%M:%S"
      )
      tmp.end = strptime(
        unlist(strsplit(tmp.stats[x.end[1], 1], "Run completed at "))[2],
        "%Y-%m-%d--%H:%M:%S"
      )
      runTiming = data.frame(
        start = tmp.start,
        completed = tmp.end,
        duration = difftime(tmp.end, tmp.start),
        duration_secs = as.numeric(difftime(tmp.end, tmp.start, units = "secs"))
      )
      continue = T
    } else {
      continue = F
      ...
      printMessage(c(
        ...
        i,
        "/",
        length(samplePathList),
        "] WARNING: Incomplete run for sample \'",
        thisSampleID,
        "\', ignoring"
      ))
    }
    ...
    if (continue == T) {
      ...
      mapping.stats[
        i,
        match(tmp.stats[, 1], colnames(mapping.stats))
      ] = as.numeric(tmp.stats[, 2])
      ...
      if (file.exists(paste(samplePathList[i], ".qcResult", sep = ""))) {
        tmp.qc = read.table(
          paste(samplePathList[i], ".qcResult", sep = ""),
          stringsAsFactors = F,
          fill = T,
          header = F,
          sep = " ",
        ...
        qc.results[
          i,
          match(gsub(":$", "", tmp.qc[-1, 1]), colnames(qc.results))
        ] = as.numeric(tmp.qc[-1, 2])
      ...
      ##
      if (
        paste(thisSampleID, ".adapterSeq", sep = "") %in% dir(samplePathList[i])
      ) {
        tmp.seq = try(
          read.table(paste(
            samplePathList[i],
      ...
      if (
        paste(
          thisSampleID,
          ".clipped.trimmed.filtered.calibratormapped.counts",
          sep = ""
        ) %in%
          dir(samplePathList[i])
      ) {
        calibratorCounts = try(
          read.table(
            paste(
              samplePathList[i],
              "/",
      ...
      if ("readCounts_miRNAmature_sense.txt" %in% availableFiles) {
        ...
        miRNA_sense = cbind(
          miRNA_sense,
          ID = sapply(rownames(miRNA_sense), function(id) {
            multiID = unlist(strsplit(id, "\\|"))
            multiIDs = sapply(multiID, function(idPart) {
      ...
      if ("readCounts_miRNAmature_antisense.txt" %in% availableFiles) {
        miRNA_antisense = read.table(
          paste(
            samplePathList[i],
            "readCounts_miRNAmature_antisense.txt",
            sep = "/"
          ),
          header = T,
          sep = "\t",
        ...
        miRNA_antisense = cbind(
          miRNA_antisense,
          ID = sapply(rownames(miRNA_antisense), function(id) {
            multiID = unlist(strsplit(id, "\\|"))
            multiIDs = sapply(multiID, function(idPart) {
              unlist(strsplit(idPart, ":"))[1]
            })
      ...
      if ("readCounts_tRNA_sense.txt" %in% availableFiles) {
        tmp = read.table(
          paste(samplePathList[i], "readCounts_tRNA_sense.txt", sep = "/"),
          header = T,
          sep = "\t",
          comment.char = "",
          stringsAsFactors = F,
        ...
        tmp = cbind(
          tmp,
          ID = sapply(rownames(tmp), function(id) {
            unlist(strsplit(id, "-"))[2]
          })
        )
        tRNA_sense = ddply(tmp, "ID", function(mat) {
          c(
            as.numeric(mat[1, 1:2]),
            sum(mat$multimapAdjustedReadCount),
            sum(mat$multimapAdjustedBarcodeCount)
          )
        })
        colnames(tRNA_sense)[-1] = colnames(tmp)[1:4]
        tRNA_sense = tRNA_sense[
          order(tRNA_sense$multimapAdjustedReadCount, decreasing = T),
        ]
      }
      if ("readCounts_tRNA_antisense.txt" %in% availableFiles) {
        tmp = read.table(
          paste(samplePathList[i], "readCounts_tRNA_antisense.txt", sep = "/"),
          header = T,
          sep = "\t",
          comment.char = "",
          stringsAsFactors = F,
        ...
        tRNA_antisense = ddply(tmp, "ID", function(mat) {
          c(
            as.numeric(mat[1, 1:2]),
            sum(mat$multimapAdjustedReadCount),
            sum(mat$multimapAdjustedBarcodeCount)
        ...
        tRNA_antisense = tRNA_antisense[
          order(tRNA_antisense$multimapAdjustedReadCount, decreasing = T),
        ]
      }

      if ("readCounts_piRNA_sense.txt" %in% availableFiles) {
        piRNA_sense = read.table(
          paste(samplePathList[i], "readCounts_piRNA_sense.txt", sep = "/"),
          header = T,
          sep = "\t",
          comment.char = "",
          stringsAsFactors = F,
      ...
      if ("readCounts_piRNA_antisense.txt" %in% availableFiles) {
        piRNA_antisense = read.table(
          paste(samplePathList[i], "readCounts_piRNA_antisense.txt", sep = "/"),
          header = T,
          sep = "\t",
          comment.char = "",
          stringsAsFactors = F,
      ...
      makeGeneID = function(id) {
        bits = unlist(strsplit(id, ":"))
        geneNameBits = unlist(strsplit(bits[3], "-"))
        geneName = geneNameBits[1]
        if (length(geneNameBits) > 2) {
          geneName = paste(geneNameBits[-length(geneNameBits)], collapse = "-")
        }
        paste(geneName, bits[2], sep = ":")
      }
      if ("readCounts_gencode_sense.txt" %in% availableFiles) {
        tmp = read.table(
          paste(samplePathList[i], "readCounts_gencode_sense.txt", sep = "/"),
          header = T,
          sep = "\t",
          comment.char = "",
          stringsAsFactors = F,
        ...
        gencode_sense = ddply(tmp, "ID", function(mat) {
          c(
            as.numeric(mat[1, 1:2]),
            sum(mat$multimapAdjustedReadCount),
            sum(mat$multimapAdjustedBarcodeCount)
        ...
        gencode_sense = gencode_sense[
          order(gencode_sense$multimapAdjustedReadCount, decreasing = T),
        ]
      }
      if ("readCounts_gencode_antisense.txt" %in% availableFiles) {
        ...
        tmp = cbind(tmp, ID = sapply(rownames(tmp), makeGeneID))
        gencode_antisense = ddply(tmp, "ID", function(mat) {
          c(
            as.numeric(mat[1, 1:2]),
            sum(mat$multimapAdjustedReadCount),
            sum(mat$multimapAdjustedBarcodeCount)
        ...
        gencode_antisense = gencode_antisense[
          order(gencode_antisense$multimapAdjustedReadCount, decreasing = T),
        ]
      }

      if ("readCounts_circRNA_sense.txt" %in% availableFiles) {
        circRNA_sense = read.table(
          paste(samplePathList[i], "readCounts_circRNA_sense.txt", sep = "/"),
          header = T,
          sep = "\t",
          comment.char = "",
          stringsAsFactors = F,
      ...
      if ("EXOGENOUS_miRNA" %in% availableFiles) {
        ...
        if ("readCounts_miRNAmature_sense.txt" %in% dir(tmp.dir)) {
          exogenous_miRNA_sense = read.table(
            paste(tmp.dir, "readCounts_miRNAmature_sense.txt", sep = "/"),
            header = T,
            sep = "\t",
            comment.char = "",
            stringsAsFactors = F,
        ...
        if ("readCounts_miRNAmature_antisense.txt" %in% dir(tmp.dir)) {
          exogenous_miRNA_antisense = read.table(
            paste(tmp.dir, "readCounts_miRNAmature_antisense.txt", sep = "/"),
            header = T,
            sep = "\t",
            comment.char = "",
            stringsAsFactors = F,
      ...
      if ("EXOGENOUS_rRNA" %in% availableFiles) {
        ...
        if (
          "ExogenousRibosomalAlignments.result.taxaAnnotated.txt" %in%
            dir(tmp.dir)
        ) {
          exogenous_rRNA = try(
            read.table(
              paste(
                tmp.dir,
                "/ExogenousRibosomalAlignments.result.taxaAnnotated.txt",
                sep = ""
              ),
              sep = "\t",
              stringsAsFactors = F,
          ...
          i.toRemove = which(
            exogenous_rRNA$name == "Bacteria" & exogenous_rRNA$level == "genus"
          )
          if (length(i.toRemove) > 0) {
            exogenous_rRNA = exogenous_rRNA[-i.toRemove, ]
          }
          exogenous_rRNA_IDs = exogenous_rRNA$name
          taxonomyInfo.exogenous_rRNA = unique(rbind(
            taxonomyInfo.exogenous_rRNA,
            exogenous_rRNA[, 1:5]
          ))
        }
      }
      ...
      if ("EXOGENOUS_genomes" %in% availableFiles) {
        ...
        if (
          "ExogenousGenomicAlignments.result.taxaAnnotated.txt" %in%
            dir(tmp.dir)
        ) {
          exogenous_genomes = read.table(
            paste(
              tmp.dir,
              "/ExogenousGenomicAlignments.result.taxaAnnotated.txt",
              sep = ""
            ),
            sep = "\t",
            stringsAsFactors = F,
          ...
          i.toRemove = which(
            exogenous_genomes$name == "Bacteria" &
              exogenous_genomes$level == "genus"
          )
          if (length(i.toRemove) > 0) {
            exogenous_genomes = exogenous_genomes[-i.toRemove, ]
          }
          exogenous_genomes_IDs = exogenous_genomes$name
          taxonomyInfo.exogenous_genomes = unique(rbind(
            taxonomyInfo.exogenous_genomes,
            exogenous_genomes[, 1:5]
          ))
        }
      }

      # Update list of detected smallRNA IDs
      allIDs.calibrator = unique(c(
        allIDs.calibrator,
        as.character(calibratorCounts$calibratorID)
      ))
      ...
      allIDs.circularRNA = unique(c(
        allIDs.circularRNA,
        rownames(circRNA_sense)
      ))
      allIDs.exogenous_miRNA = unique(c(
        allIDs.exogenous_miRNA,
        exogenous_miRNA_IDs
      ))
      allIDs.exogenous_rRNA = unique(c(
        allIDs.exogenous_rRNA,
        exogenous_rRNA_IDs
      ))
      allIDs.exogenous_genomes = unique(c(
        allIDs.exogenous_genomes,
        exogenous_genomes_IDs
      ))

      sample.data[[i]] = list(
        "miRNA_sense" = miRNA_sense,
        "miRNA_antisense" = miRNA_antisense,
        "tRNA_sense" = tRNA_sense,
        "tRNA_antisense" = tRNA_antisense,
        "piRNA_sense" = piRNA_sense,
        "piRNA_antisense" = piRNA_antisense,
        "gencode_sense" = gencode_sense,
        "gencode_antisense" = gencode_antisense,
        "circRNA_sense" = circRNA_sense,
        "circRNA_antisense" = circRNA_antisense,
        "exogenous_miRNA_sense" = exogenous_miRNA_sense,
        "exogenous_rRNA" = exogenous_rRNA,
        "exogenous_genomes" = exogenous_genomes,
        "adapterSeq" = adapterSeq,
        "adapterConfidence" = adapterConfidence,
        "qcOutcome" = qcOutcome,
        "runTiming" = runTiming,
        "calibratorCounts" = calibratorCounts
      )
  ...
  read.lengths = read.lengths[,
    0:(max(as.numeric(colnames(read.lengths[,
      colSums(read.lengths) > 0,
      drop = F
    ]))) +
      1),
    drop = F
  ...
  ##
  allIDs = list(
    "calibrator" = allIDs.calibrator,
    "miRNA_sense" = allIDs.miRNA,
    "tRNA_sense" = allIDs.tRNA,
    "piRNA_sense" = allIDs.piRNA,
    "gencode_sense" = allIDs.gencode,
    "circRNA_sense" = allIDs.circularRNA,
    "exogenous_miRNA" = allIDs.exogenous_miRNA,
    "exogenous_rRNA" = allIDs.exogenous_rRNA,
    "exogenous_genomes" = allIDs.exogenous_genomes
  )
  ...
  run.duration = data.frame(
    runDuration_secs = rep(0, length(sample.data)),
    stringsAsFactors = F
  )
  rownames(run.duration) = names(sample.data)

  exprs.calibrator = matrix(
    0,
    ncol = length(sample.data),
    nrow = length(allIDs$calibrator),
    dimnames = list(allIDs$calibrator, names(sample.data))
  )

  exprs.miRNA = matrix(
    0,
    ncol = length(sample.data),
    nrow = length(allIDs$miRNA_sense),
    dimnames = list(allIDs$miRNA_sense, names(sample.data))
  )
  exprs.tRNA = matrix(
    0,
    ncol = length(sample.data),
    nrow = length(allIDs$tRNA_sense),
    dimnames = list(allIDs$tRNA_sense, names(sample.data))
  )
  exprs.piRNA = matrix(
    0,
    ncol = length(sample.data),
    nrow = length(allIDs$piRNA_sense),
    dimnames = list(allIDs$piRNA_sense, names(sample.data))
  )
  exprs.gencode = matrix(
    0,
    ncol = length(sample.data),
    nrow = length(allIDs$gencode_sense),
    dimnames = list(allIDs$gencode_sense, names(sample.data))
  )
  exprs.circRNA = matrix(
    0,
    ncol = length(sample.data),
    nrow = length(allIDs$circRNA_sense),
    dimnames = list(allIDs$circRNA_sense, names(sample.data))
  )
  exprs.exogenous_miRNA = matrix(
    0,
    ncol = length(sample.data),
    nrow = length(allIDs$exogenous_miRNA),
    dimnames = list(allIDs$exogenous_miRNA, names(sample.data))
  )
  ...
  exprs.exogenousRibosomal_specific = matrix(
    0,
    ncol = length(sample.data),
    nrow = tmp.nrow,
    dimnames = list(taxonomyInfo.exogenous_rRNA$ID, names(sample.data))
  )
  exprs.exogenousRibosomal_cumulative = matrix(
    0,
    ncol = length(sample.data),
    nrow = tmp.nrow,
    dimnames = list(taxonomyInfo.exogenous_rRNA$ID, names(sample.data))
  ...
  exprs.exogenousGenomes_specific = matrix(
    0,
    ncol = length(sample.data),
    nrow = tmp.nrow,
    dimnames = list(taxonomyInfo.exogenous_genomes$ID, names(sample.data))
  )
  exprs.exogenousGenomes_cumulative = matrix(
    0,
    ncol = length(sample.data),
    nrow = tmp.nrow,
    dimnames = list(taxonomyInfo.exogenous_genomes$ID, names(sample.data))
  ...
  for (i in 1:length(sample.data)) {
    ...
    if (!is.null(nrow(sample.data[[i]]$calibratorCounts))) {
      exprs.calibrator[
        match(
          sample.data[[i]]$calibratorCounts$calibratorID,
          rownames(exprs.calibrator)
        ),
        i
      ] = as.numeric(sample.data[[i]]$calibratorCounts$readCount)
    }

    exprs.miRNA[
      match(sample.data[[i]]$miRNA_sense$ID, rownames(exprs.miRNA)),
      i
    ] = as.numeric(sample.data[[i]]$miRNA_sense$multimapAdjustedReadCount)
    exprs.tRNA[
      match(sample.data[[i]]$tRNA_sense$ID, rownames(exprs.tRNA)),
      i
    ] = as.numeric(sample.data[[i]]$tRNA_sense$multimapAdjustedReadCount)
    exprs.piRNA[
      match(rownames(sample.data[[i]]$piRNA_sense), rownames(exprs.piRNA)),
      i
    ] = as.numeric(sample.data[[i]]$piRNA_sense$multimapAdjustedReadCount)
    exprs.gencode[
      match(sample.data[[i]]$gencode_sense$ID, rownames(exprs.gencode)),
      i
    ] = as.numeric(sample.data[[i]]$gencode_sense$multimapAdjustedReadCount)
    exprs.circRNA[
      match(rownames(sample.data[[i]]$circRNA_sense), rownames(exprs.circRNA)),
      i
    ] = as.numeric(sample.data[[i]]$circRNA_sense$multimapAdjustedReadCount)
    ## Exogenous miRNA
    if (!is.null(nrow(sample.data[[i]]$exogenous_miRNA))) {
      exprs.exogenous_miRNA[
        match(
          rownames(sample.data[[i]]$exogenous_miRNA),
          rownames(exprs.exogenous_miRNA)
        ),
        i
      ] = as.numeric(sample.data[[i]]$exogenous_miRNA$multimapAdjustedReadCount)
    }
    ## Exogenous rRNA
    if (!is.null(nrow(sample.data[[i]]$exogenous_rRNA))) {
      exprs.exogenousRibosomal_specific[
        match(
          sample.data[[i]]$exogenous_rRNA$ID,
          rownames(exprs.exogenousRibosomal_specific)
        ),
        i
      ] = as.numeric(sample.data[[i]]$exogenous_rRNA$readCount_direct)
      exprs.exogenousRibosomal_cumulative[
        match(
          sample.data[[i]]$exogenous_rRNA$ID,
          rownames(exprs.exogenousRibosomal_cumulative)
        ),
        i
      ] = as.numeric(sample.data[[i]]$exogenous_rRNA$readCount_inherited)
    }
    ## Exogenous Genomes
    if (!is.null(nrow(sample.data[[i]]$exogenous_genomes))) {
      exprs.exogenousGenomes_specific[
        match(
          sample.data[[i]]$exogenous_genomes$ID,
          rownames(exprs.exogenousGenomes_specific)
        ),
        i
      ] = as.numeric(sample.data[[i]]$exogenous_genomes$readCount_direct)
      exprs.exogenousGenomes_cumulative[
        match(
          sample.data[[i]]$exogenous_genomes$ID,
          rownames(exprs.exogenousGenomes_cumulative)
        ),
        i
      ] = as.numeric(sample.data[[i]]$exogenous_genomes$readCount_inherited)
    }
  }
  ...
  libSizes$successfully_clipped = mapping.stats[,
    colnames(mapping.stats) %in% c("successfully_clipped")
  ]
  libSizes$reads_used_for_alignment = mapping.stats[,
    colnames(mapping.stats) %in% c("reads_used_for_alignment")
  ]
  libSizes$all = rowSums(mapping.stats[,
    colnames(mapping.stats) %in% c("rRNA", "genome", "miRNA_exogenous_sense")
  ])
  libSizes$endogenous = rowSums(mapping.stats[,
    colnames(mapping.stats) %in% c("rRNA", "genome")
  ])
  ...
  libSizes$exogenous_rRNA = exprs.exogenousRibosomal_cumulative[
    rownames(exprs.exogenousRibosomal_cumulative) == "1",
  ]
  libSizes$exogenous_genomes = exprs.exogenousGenomes_cumulative[
    rownames(exprs.exogenousGenomes_cumulative) == "1",
  ]
  ...
  if (nrow(exprs.calibrator) > 0) {
    write.table(
      exprs.calibrator,
      file = paste(output.dir, "exceRpt_CALIBRATOR_ReadCounts.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.miRNA) > 0) {
    write.table(
      exprs.miRNA,
      file = paste(output.dir, "exceRpt_miRNA_ReadCounts.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.tRNA) > 0) {
    write.table(
      exprs.tRNA,
      file = paste(output.dir, "exceRpt_tRNA_ReadCounts.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.piRNA) > 0) {
    write.table(
      exprs.piRNA,
      file = paste(output.dir, "exceRpt_piRNA_ReadCounts.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.gencode) > 0) {
    write.table(
      exprs.gencode,
      file = paste(output.dir, "exceRpt_gencode_ReadCounts.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.circRNA) > 0) {
    write.table(
      exprs.circRNA,
      file = paste(output.dir, "exceRpt_circularRNA_ReadCounts.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.exogenousRibosomal_specific) > 0) {
    tmp = cbind(
      taxonomyInfo.exogenous_rRNA[
        match(
          rownames(exprs.exogenousRibosomal_specific),
          taxonomyInfo.exogenous_rRNA$ID
        ),
      ],
      exprs.exogenousRibosomal_specific
    )
  ...
  if (nrow(exprs.exogenousGenomes_specific) > 0) {
    tmp = cbind(
      taxonomyInfo.exogenous_genomes[
        match(
          rownames(exprs.exogenousGenomes_specific),
          taxonomyInfo.exogenous_genomes$ID
        ),
      ],
      exprs.exogenousGenomes_specific
    )
    write.table(
      ...
      file = paste(
        output.dir,
        "exceRpt_exogenousGenomes_taxonomySpecific_ReadCounts.txt",
        sep = "/"
      ),
      sep = "\t",
      row.names = F,
    ...
    write.table(
      ...
      file = paste(
        output.dir,
        "exceRpt_exogenousGenomes_taxonomyCumulative_ReadCounts.txt",
        sep = "/"
      ),
      sep = "\t",
      row.names = F,
  ...
  write.table(
    read.lengths,
    file = paste(output.dir, "exceRpt_ReadLengths.txt", sep = "/"),
    sep = "\t",
    col.names = NA,
    quote = F
  )
  ...
  ##
  adapterSeq = unlist(lapply(sample.data, function(l) {
    l$adapterSeq
  }))
  write.table(
    as.data.frame(adapterSeq),
    file = paste(output.dir, "exceRpt_adapterSequences.txt", sep = "/"),
    sep = "\t",
    col.names = NA,
    quote = F
  )
  ...
  write.table(
    mapping.stats,
    file = paste(output.dir, "exceRpt_readMappingSummary.txt", sep = "/"),
    sep = "\t",
    col.names = NA,
    quote = F
  )
  write.table(
    qc.results,
    file = paste(output.dir, "exceRpt_QCresults.txt", sep = "/"),
    sep = "\t",
    col.names = NA,
    quote = F
  )
  ...
  exprs.exogenous_miRNA.rpm = t(
    10^6 * t(exprs.exogenous_miRNA) / libSizes$exogenous_miRNA
  )
  ...
  if (nrow(exprs.exogenousRibosomal_specific) > 0) {
    exprs.exogenousRibosomal_specific.rpm = t(
      10^6 * t(exprs.exogenousRibosomal_specific) / libSizes$exogenous_rRNA
    )
    exprs.exogenousRibosomal_cumulative.rpm = t(
      10^6 * t(exprs.exogenousRibosomal_cumulative) / libSizes$exogenous_rRNA
    )
  }
  ...
  if (nrow(exprs.exogenousGenomes_specific) > 0) {
    exprs.exogenousGenomes_specific.rpm = t(
      10^6 * t(exprs.exogenousGenomes_specific) / libSizes$exogenous_genomes
    )
    exprs.exogenousGenomes_cumulative.rpm = t(
      10^6 * t(exprs.exogenousGenomes_cumulative) / libSizes$exogenous_genomes
    )
  }
  ...
  if (nrow(exprs.miRNA.rpm) > 0) {
    write.table(
      exprs.miRNA.rpm,
      file = paste(output.dir, "exceRpt_miRNA_ReadsPerMillion.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.tRNA.rpm) > 0) {
    write.table(
      exprs.tRNA.rpm,
      file = paste(output.dir, "exceRpt_tRNA_ReadsPerMillion.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.piRNA.rpm) > 0) {
    write.table(
      exprs.piRNA.rpm,
      file = paste(output.dir, "exceRpt_piRNA_ReadsPerMillion.txt", sep = "/"),
      sep = "\t",
      col.names = NA,
      quote = F
    )
  ...
  if (nrow(exprs.exogenousRibosomal_specific) > 0) {
    tmp = cbind(
      taxonomyInfo.exogenous_rRNA[
        match(
          rownames(exprs.exogenousRibosomal_specific.rpm),
          taxonomyInfo.exogenous_rRNA$ID
        ),
      ],
      exprs.exogenousRibosomal_specific.rpm
    ...
    tmp = cbind(
      taxonomyInfo.exogenous_rRNA[
        match(
          rownames(exprs.exogenousRibosomal_cumulative.rpm),
          taxonomyInfo.exogenous_rRNA$ID
        ),
      ],
      exprs.exogenousRibosomal_cumulative.rpm
  ...
  if (nrow(exprs.exogenousGenomes_specific) > 0) {
    tmp = cbind(
      taxonomyInfo.exogenous_genomes[
        match(
          rownames(exprs.exogenousGenomes_specific.rpm),
          taxonomyInfo.exogenous_genomes$ID
        ),
      ],
      exprs.exogenousGenomes_specific.rpm
    ...
    write.table(
      ...
      file = paste(
        output.dir,
        "exceRpt_exogenousGenomes_taxonomySpecific_ReadsPerMillion.txt",
        sep = "/"
      ),
      sep = "\t",
      row.names = F,
    ...
    tmp = cbind(
      taxonomyInfo.exogenous_genomes[
        match(
          rownames(exprs.exogenousGenomes_cumulative.rpm),
          taxonomyInfo.exogenous_genomes$ID
        ),
      ],
      exprs.exogenousGenomes_cumulative.rpm
    ...
    write.table(
      ...
      file = paste(
        output.dir,
        "exceRpt_exogenousGenomes_taxonomyCumulative_ReadsPerMillion.txt",
        sep = "/"
      ),
      sep = "\t",
      row.names = F,
...
PlotData = function(
  ...
  pdf(
    paste(output.dir, "exceRpt_DiagnosticPlots.pdf", sep = "/"),
    height = 10,
    width = 20
  )
  ...
  if (ncol(read.lengths) > 1) {
    ...
    ##
    tmp = melt(read.lengths)
    colnames(tmp) = c("sample", "length", "count")
    if (is.data.frame(sampleGroups)) {
      tmp$sampleGroup = sampleGroups[
        match(tmp$sample, sampleGroups$sampleID),
        2
      ]
    }
    tmp$sample <- factor(tmp$sample)
    maxX = min(c(100, max(tmp$length)))
    p = ggplot(tmp, aes(x = length, y = count, color = sample)) +
      geom_line(alpha = 1) +
      xlab("read length (nt)") +
      ylab("# reads") +
      ...
      scale_x_continuous(
        limits = c(15, maxX),
        minor_breaks = 1:maxX,
        breaks = seq(15, maxX, by = 5)
      ) +
      scale_colour_manual(values = getSampleColors(tmp$sample))
    if (nrow(read.lengths) > 30) {
      p = p + guides(colour = FALSE)
    }
    if (is.data.frame(sampleGroups)) {
      p = p + facet_wrap(~sampleGroup, ncol = 1)
    }
    print(p)
    ...
    ##
    tmp = melt(t(apply(read.lengths, 1, function(row) {
      row / sum(row)
    })))
    colnames(tmp) = c("sample", "length", "fraction")
    if (is.data.frame(sampleGroups)) {
      tmp$sampleGroup = sampleGroups[
        match(tmp$sample, sampleGroups$sampleID),
        2
    ...
    tmp$sample <- factor(tmp$sample)
    maxX = min(c(100, max(tmp$length)))
    p = ggplot(tmp, aes(x = length, y = fraction, colour = sample)) +
      geom_line(alpha = 1) +
      xlab("read length (nt)") +
      ylab("fraction of reads") +
      ggtitle("read-length distributions:\nnormalised read fraction") +
      scale_x_continuous(
        limits = c(15, maxX),
        minor_breaks = 1:maxX,
        breaks = seq(15, maxX, by = 5)
  ...
  p = ggplot(as.data.frame(tmp), aes(x = tmp)) +
    geom_histogram(
      breaks = seq(0, ceiling(max(tmp)), by = 0.1),
      fill = 'lightblue'
    ) +
    xlab("log10(# mapped reads)") +
    ylab("# samples") +
    ggtitle("Library size (all mapped reads)")
  ...
  toplot = melt(as.matrix(mapping.stats / mapping.stats$input))
  colnames(toplot) = c("Sample", "Stage", "ReadFraction")
  toplot$Stage = with(toplot, factor(Stage, levels = rev(levels(Stage))))
  toplot$Sample = factor(
    as.character(toplot$Sample),
    levels = rownames(mapping.stats)[sampleOrder]
  )
  if (is.data.frame(sampleGroups)) {
    toplot$sampleGroup = sampleGroups[
      match(toplot$Sample, sampleGroups$sampleID),
      2
    ]
  }
  p = ggplot(
    ...
    aes(
      ...
      group = Sample,
      fill = ReadFraction,
      label = sprintf("%1.1f%%", ReadFraction * 100)
    )
  ) +
    geom_tile() +
    ...
    theme(axis.text.x = element_text(angle = 40, hjust = 1.0, vjust = 1)) +
    ggtitle("fraction aligned reads\n(normalised by # input reads)")
  if (nrow(mapping.stats) < 50) {
    p = p + geom_text(size = 3)
  }
  if (is.data.frame(sampleGroups)) {
    p = p + facet_grid(~sampleGroup, scales = "free_x", space = "free_x")
  }
  print(p)
  ...
  if (max(mapping.stats$successfully_clipped) > 0) {
    ...
    toplot = melt(as.matrix(tmp / tmp$successfully_clipped)[, -1, drop = F])
    colnames(toplot) = c("Sample", "Stage", "ReadFraction")
    toplot$Stage = with(toplot, factor(Stage, levels = rev(levels(Stage))))
    toplot$Sample = factor(
      as.character(toplot$Sample),
    ...
    # save
    plotsList[[
      "fraction aligned reads (normalised by # adapter-clipped reads)"
    ]] = p
  }
  ...
  toplot = melt(as.matrix(
    mapping.stats / mapping.stats$reads_used_for_alignment
  )[, -c(1:7), drop = F])
  colnames(toplot) = c("Sample", "Stage", "ReadFraction")
  toplot$Stage = with(toplot, factor(Stage, levels = rev(levels(Stage))))
  toplot$Sample = factor(
  ...
  # save
  plotsList[[
    "fraction aligned reads (normalised by # non-contaminant reads)"
  ]] = p
  ...
  p = ggplot(
    toplot,
    aes(x = TranscriptomeReads, y = TranscriptomeGenomeRatio)
  ) +
    geom_text(
      label = toplot$Sample,
      hjust = 0,
  ...
  if (is.data.frame(sampleGroups)) {
    ...
    p = ggplot(
      toplot,
      aes(
        x = TranscriptomeReads,
        y = TranscriptomeGenomeRatio,
        colour = sampleGroup
      )
    ) +
      geom_text(
  ...
  p = p +
    scale_x_log10(breaks = 10^c(minX:maxX)) +
    coord_cartesian(xlim = c(10^(minX), 10^(maxX)), ylim = c(0, 1)) +
    geom_vline(xintercept = 100000, col = "red", alpha = 0.5) +
    ...
    ylab("# transcriptome reads / # genome reads") +
    xlab("# transcriptome reads (log10)") +
    ggtitle("QC result: overall")
  ...
  tmp.pass = tmp.mat[, 3] >= 100000
  tmp.mat[tmp.pass, 3] = "pass"
  tmp.mat[!tmp.pass, 3] = "fail"
  tmp.pass = tmp.mat[, 4] >= 0.5
  tmp.mat[tmp.pass, 4] = "pass"
  tmp.mat[!tmp.pass, 4] = "fail"

  toplot = cbind(melt(tmp.mat), Actual = melt(qc.results)[, 3])
  colnames(toplot)[1:3] = c("Sample", "Stage", "Value")
  toplot$Sample = factor(
    as.character(toplot$Sample),
    levels = rownames(mapping.stats)[sampleOrder]
  )
  ...
  p = ggplot(toplot, aes(y = Sample, x = Stage, fill = Value, label = Actual)) +
    scale_fill_manual(
      values = c("fail" = "red", "pass" = "palegreen", "1" = "lightgrey")
    ) +
    geom_label() +
    theme(
      plot.background = element_rect(fill = "white"),
      panel.background = element_rect(fill = rgb(0.97, 0.97, 0.97)),
      axis.text.x = element_text(angle = 20, hjust = 1, vjust = 1)
    ) +
    ggtitle("QC result: per-sample results") +
    xlab("") +
    ylab("")
  print(p)
  ...
  require(plyr)
  sampleTotals = matrix(NA, ncol = nrow(mapping.stats), nrow = 0)
  colnames(sampleTotals) = rownames(mapping.stats)
  if (nrow(exprs.miRNA) > 0) {
  ...
  if (nrow(exprs.gencode) > 0) {
    tmp = data.frame(
      biotype = sapply(rownames(exprs.gencode), function(id) {
        bits = unlist(strsplit(id, ":"))
        bits[length(bits)]
      }),
      exprs.gencode
    )
    tmp = ddply(tmp, "biotype", function(mat) {
      colSums(mat[, -1, drop = F])
    })
    rownames(tmp) = tmp[, 1]
    tmp = tmp[, -1, drop = F]
  ...
  sampleTotals = sampleTotals[
    order(apply(sampleTotals, 1, median, na.rm = T), decreasing = F),
    ,
    drop = F
  ]
  ...
  if (is.data.frame(sampleGroups)) {
    tmp$sampleGroup = sampleGroups[
      match(tmp$sampleID, sampleGroups$sampleID),
      2
    ]
  }
  p = ggplot(na.omit(tmp), aes(y = readCount, x = biotype, colour = biotype)) +
    ...
    guides(colour = FALSE) +
    coord_flip() +
    ggtitle("Biotypes: distributions, raw read-counts") +
    scale_color_manual(values = getSampleColors(tmp$biotype))
  if (is.data.frame(sampleGroups)) {
    p = p + facet_grid(~sampleGroup, scales = "free_x")
  }
  print(p)
  ...
  write.table(
    sampleTotals[
      order(apply(sampleTotals, 1, median, na.rm = T), decreasing = T),
      ,
      drop = F
    ],
    file = paste(output.dir, "exceRpt_biotypeCounts.txt", sep = "/"),
    sep = "\t",
    col.names = NA,
    quote = F
  )
  ...
  tmp = melt(as.matrix(apply(sampleTotals, 2, function(col) {
    col * 1000000 / sum(col)
  })))
  ...
  p = ggplot(
    na.omit(tmp),
    aes(y = readPerMillion, x = biotype, colour = biotype)
  ) +
    geom_hline(aes(yintercept = 1), linetype = "dashed") +
    geom_boxplot() +
    scale_y_log10(
    ...
    guides(colour = FALSE) +
    coord_flip() +
    ggtitle("Biotypes: distributions, normalised") +
    scale_color_manual(values = getSampleColors(tmp$biotype))
  if (is.data.frame(sampleGroups)) {
    p = p + facet_grid(~sampleGroup, scales = "free_x")
  }
  ...
  for (i in 1:ncol(sampleTotals)) {
    tmp[, i] = sampleTotals[, i] *
      1000000 /
      libSizes$reads_used_for_alignment[i]
  }
  ...
  tmp = as.matrix(rbind(
    tmp[1:N, , drop = F],
    other = colSums(tmp[-c(1:N), , drop = F])
  ))
  ...
  p = ggplot(
    na.omit(tmp),
    aes(y = readsPerMillion, x = sampleID, fill = biotype)
  ) +
    geom_bar(stat = "identity") +
    scale_fill_brewer(palette = "Paired") +
    theme(axis.text.x = element_text(angle = 50, hjust = 1.0, vjust = 1)) +
    ggtitle("Biotypes: per-sample, normalised") +
    ylab("reads per million reads used for alignment") +
    xlab("") +
    ylim(limits = c(0, 1E6)) #+scale_fill_discrete(rich.colors(10))
  if (is.data.frame(sampleGroups)) {
    p = p + facet_grid(~sampleGroup, scales = "free_x", space = "free_x")
  }
  ...
  tmp = as.matrix(apply(sampleTotals, 2, function(col) {
    col * 1000000 / sum(col)
  }))
  tmp = tmp[biotypeOrder, , drop = F]
  tmp = as.matrix(rbind(
  ...
  p = ggplot(
    ...
    xlab("") +
    scale_fill_brewer(palette = "Paired") #+scale_fill_manual( values = c(colorRampPalette( brewer.pal( 6 , "Paired" ) )(8), "grey") )
  if (is.data.frame(sampleGroups)) {
    p = p + facet_grid(~sampleGroup, scales = "free_x", space = "free_x")
  }
  suppressWarnings(print(p))
  ...
  if (nrow(exprs.miRNA) > 0) {
    ...
    p = ggplot(tmp, aes(y = abundance, x = sample, colour = sample)) +
      ...
      ylab("Read count") +
      ggtitle("miRNA abundance distributions (raw counts)") +
      scale_y_log10() +
      guides(colour = FALSE) +
      scale_color_manual(
        values = set_names(getSampleColors(tmp$sample), tmp$sample)
      )
    ...
    p = ggplot(tmp, aes(x = abundance, colour = sample)) +
      geom_density() +
      xlab("Read count") +
      ggtitle("miRNA abundance distributions (raw counts)") +
      scale_x_log10() +
  ...
  if (nrow(exprs.exogenousGenomes_specific) > 0) {
    ...
    par(oma = c(20, 2, 0, 0))
    barplot(
      exprs.exogenousGenomes_cumulative[1, , drop = F],
      las = 2,
      main = "Total # reads mapped to NCBI taxonomy"
    )
    ...
    if (ncol(exprs.exogenousGenomes_specific) > 1) {
      par(oma = c(8, 0, 0, 20))
      maxRow = 50
      if (nrow(exprs.exogenousGenomes_specific) < maxRow) {
        maxRow = nrow(exprs.exogenousGenomes_specific)
      }
      tmp.order = order(
        apply(
          t(
            t(exprs.exogenousGenomes_specific) /
              colSums(exprs.exogenousGenomes_specific)
          ),
          1,
          median
        ),
      ...
      colnames(tmp) = taxonomyInfo.exogenous_genomes[
        match(colnames(tmp), taxonomyInfo.exogenous_genomes$ID),
      ]$name
      heatmap.2(
        tmp,
        trace = "none",
        main = "top taxa nodes: specific normalised read count",
        symbreaks = F,
        col = rich.colors(50)
      )

      tmp.order = order(
        apply(t(t(exprs.exogenousGenomes_specific)), 1, median),
        decreasing = T
      )
      tmp = t(log10(
        exprs.exogenousGenomes_specific[tmp.order, ][1:maxRow, ] + 0.1
      ))
      colnames(tmp) = taxonomyInfo.exogenous_genomes[
        match(colnames(tmp), taxonomyInfo.exogenous_genomes$ID),
      ]$name
      heatmap.2(
        tmp,
        trace = "none",
        main = "top taxa nodes: specific absolute read count",
        symbreaks = F,
        col = rich.colors(50)
      )

      maxRow = 50
      if (nrow(exprs.exogenousGenomes_cumulative) < maxRow) {
        maxRow = nrow(exprs.exogenousGenomes_cumulative)
      }
      tmp.order = order(
        apply(
          t(t(exprs.exogenousGenomes_cumulative) / libSizes$exogenous_genomes),
          1,
          median
        ),
        decreasing = T
      ...
      tmp = t(log10(
        t(
          t(exprs.exogenousGenomes_cumulative) *
            1000000 /
            libSizes$exogenous_genomes
        )[tmp.order, ][1:maxRow, ] +
          0.1
      ))
      colnames(tmp) = taxonomyInfo.exogenous_genomes[
      ...
      heatmap.2(
        tmp,
        trace = "none",
        main = "top taxa nodes: cumulative normalised read count",
        symbreaks = F,
        col = rich.colors(50)
      )

      tmp.order = order(
        apply(t(t(exprs.exogenousGenomes_cumulative)), 1, median),
        decreasing = T
      )
      tmp = t(log10(
        exprs.exogenousGenomes_cumulative[tmp.order, ][1:maxRow, ] + 0.1
      ))
      colnames(tmp) = taxonomyInfo.exogenous_genomes[
        match(colnames(tmp), taxonomyInfo.exogenous_genomes$ID),
      ]$name
      heatmap.2(
        tmp,
        trace = "none",
        main = "top taxa nodes: cumulative absolute read count",
        symbreaks = F,
        col = rich.colors(50)
      )
    }
...
plotTree = function(
  ...
  if (what == "exogenousRibosomal") {
    nA$fillcolor <- sapply(counts_uniq * 10, function(val) {
      if (val > 100) {
        val = 100
      }
      rgb(100 - val, 100, 100 - val, maxColorValue = 100)
    })
  } else {
    nA$fillcolor <- sapply(counts_uniq * 10, function(val) {
      if (val > 100) {
      ...
      rgb(100 - val, 100 - val, 100, maxColorValue = 100)
    })
  }

  newNodeIDs = sapply(
    taxonomyInfo[match(as.numeric(nodes(rEG)), taxonomyInfo$ID), ]$name,
    function(id) {
      newID = unlist(strsplit(id, " "))
      if (length(newID) == 1) {
        id
      } else {
  ...
  ## edge parameters
  eA <- list(
    arrowsize = rep(0.1, length(names(rEG@edgeData))),
    arrowhead = rep("none", length(names(rEG@edgeData)))
  )
  eA <- lapply(eA, function(x) {
    names(x) <- names(rEG@edgeData)
    x
  })
...
## (Rob Kitchen)
plotExogenousTaxonomyTrees = function(
  counts,
  cumcounts,
  what,
  output.dir,
  ...
  counts.norm = apply(counts, 2, function(col) {
    col * 100 / sum(col)
  })
  cumcounts.norm = apply(cumcounts, 2, function(col) {
    col * 100 / col[1]
  })
  dim(counts)
  ...
  keepRows = sort(unique(c(
    keepRows,
    which(apply(cumcounts.norm, 1, max) >= minPercent)
  )))
  ...
  data_uniq = apply(counts, 2, function(col) {
    col * 100 / sum(col)
  })
  data_cum = apply(cumcounts, 2, function(col) {
    col * 100 / col[1]
  })
  ...
  rEG <<- new(
    "graphNEL",
    nodes = as.character(taxonomyInfo$ID),
    edgemode = "directed"
  )
  trim <- function(x) gsub("^\\s+|\\s+$", "", x)
  apply(taxonomyInfo[-1, ], 1, function(row) {
    from = trim(as.character(row[4]))
    if (from %in% taxonomyInfo$ID) {
      rEG <<- addEdge(
        trim(as.character(row[4])),
        trim(as.character(row[3])),
        rEG,
        1
      )
    }
  ...
  printMessage(c(
    "Plotting a taxonomy tree based on the average of all samples "
  ))
  pdf(
    file = paste(
      output.dir,
      "/exceRpt_",
      what,
      "_TaxonomyTrees_aggregateSamples.pdf",
      sep = ""
    ),
    height = 7,
    width = 15
  ...
  if (is.data.frame(sampleGroups)) {
    ...
    for (thisgroup in levels(as.factor(sampleGroups$sampleGroup))) {
      tmpDat_uniq = rowMeans(data_uniq[,
        match(
          sampleGroups[sampleGroups$sampleGroup %in% thisgroup, ]$sampleID,
          colnames(data_uniq)
        ),
        drop = F
      ])
      tmpDat_cum = rowMeans(data_cum[,
        match(
          sampleGroups[sampleGroups$sampleGroup %in% thisgroup, ]$sampleID,
          colnames(data_cum)
        ),
