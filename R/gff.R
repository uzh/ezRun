...
ezLoadFeatures = function(
  param = NULL,
  featureFile = param$ezRef["refFeatureFile"],
  types = NULL
) {
  ...
  gff = gff[
    !ezGrepl(c("CDS", "codon", "UTR", "protein"), gff$type, ignore.case = TRUE),
  ]
  if (getSuffix(featureFile) == "gtf") {
    gff$transcript_id = ezGffAttributeField(
      gff$attributes,
      field = "transcript_id",
      attrsep = "; *",
      valuesep = " "
    ...
    gff$gene_id = ezGffAttributeField(
      gff$attributes,
      field = "gene_id",
      attrsep = ";",
      valuesep = " "
    ...
    gff$gene_name = ezGffAttributeField(
      gff$attributes,
      field = "gene_name",
      attrsep = "; *",
      valuesep = " "
    ...
    gff$tss_id = ezGffAttributeField(
      gff$attributes,
      field = "tss_id",
      attrsep = "; *",
      valuesep = " "
...
ezGffAttributeField = function(x, field, attrsep = ";", valuesep = "=") {
  ...
  x <- str_extract(
    ...
    paste(
      field,
      paste0('"{0,1}[^"', attrsep, ']+"{0,1}', attrsep),
      sep = valuesep
    )
  )
...
extendGtfThreePrime <- function(gtf, extensionWidth, seqLengths) {
  ...
  chrIdx <- split(
    1:length(gtfUse),
    paste(as.vector(seqnames(gtfUse)), as.vector(strand(gtfUse)))
  )
  ...
  for (nm in names(chrIdx)) {
    ...
    if (grepl("+", nm, fixed = TRUE)) {
      ...
      rowIdxList <- tapply(
        (1:length(gtfChrom))[isExon],
        gtfChrom$transcript_id[isExon],
        function(x) {
          x
        }
      )
      ...
      toExtend <- c(
        which(gtfChrom$type %in% c("gene", "transcript", "three_prime_utr")),
        lastExonIdx
      )
      # trStarts which are the limits plus the chromosome end
      trStarts <- tapply(
        start(gtfChrom)[isExon],
        gtfChrom$transcript_id[isExon],
        min
      ) %>%
        c(seqLengths[chrName] + 1) %>%
        unique() %>%
        sort()
...
ezBuildAttributeField = function(x, format = "gtf") {
  if (format == "gtf") {
    ...
    stop("Format not supported. Use either 'gtf' or 'gff'.")
  }
}
...
gffTrimTranscripts = function(gff, maxLength = 100, start = TRUE) {
  ...
  gffListTrimmed = ezMclapply(
    gffList,
    function(x) {
      gffTrimSingleTranscript(x, maxLength = maxLength, start = start)
    },
    mc.cores = min(ezThreads(), 8)
  )
...
gffTrimSingleTranscript = function(gffSingle, maxLength = 100, start = TRUE) {
  ...
  stopifnot(
    length(gffSingle$start) > 0 &&
      (all(diff(gffSingle$start) > 0) || all(diff(gffSingle$start) < 0))
  )
  ## and place the order according the "start"
  gffSingle = gffSingle[
    order(gffSingle$start, decreasing = reverseOrder),
    ,
    drop = FALSE
  ]
  ...
  if (is.na(trimThis)) {
    ...
    if (maxLength == 0) {
      ...
      if (reverseOrder) {
        gffTrim[trimThis, "start"] = gffTrim[trimThis, "end"] -
          as.integer(maxLength - 1)
      } else {
        gffTrim[trimThis, "end"] = gffTrim[trimThis, "start"] +
          as.integer(maxLength - 1)
      }
    }
  }
...
groupGff = function(
  ...
  isTransSpliced = tapply(paste(gtf$seqid, gtf$strand), grouping, function(x) {
    length(unique(x)) > 1
  })
  if (any(isTransSpliced)) {
    ...
    if (skipTransSpliced) {
      ...
      stop(
        "transspliced elements exist in GTF: ",
        paste(tsNames, collapse = " ")
      )
    }
  }
  stopifnot(
    tapply(paste(gtf$seqid, gtf$strand), grouping, function(x) {
      length(unique(x))
    }) ==
      1
  )
  start = tapply(gtf$start, grouping, function(x) {
    min(x)
  })
  end = tapply(gtf$end, grouping, function(x) {
    max(x)
  })
...
gffGroupToRanges = function(gtf, grouping, skipTransSpliced = FALSE) {
  gtfGrouped = groupGff(
    gtf,
    grouping = grouping,
    skipTransSpliced = skipTransSpliced
  )
...
getExonNumber = function(gtf) {
  ...
  stopifnot(
    tapply(locusId, gtf$transcript_id, function(x) {
      length(unique(x))
    }) ==
      1
  ) ## every transcript must occur only in one locus!
  exonNumbers = rep(NA, nrow(gtf))
  use = gtf$type == "exon"
  exonNumbers[use] = unsplit(
    tapply(gtf$start[use], gtf$transcript_id[use], order),
    gtf$transcript_id[use]
  )
  return(exonNumbers)
}
...
writePresplicedGtf <- function(
  ...
  attrNew = ezBuildAttributeField(
    gtfBoth[, c("gene_id", "gene_name", "transcript_id", "tss_id")],
    "gtf"
  )
...
getTranscriptSequences = function(
  param = NULL,
  genomeFn = NULL,
  featureFn = NULL
) {
  ...
  txdb = makeTxDbFromGFF(featureFn, dataSource = "FGCZ", taxonomyId = NA)
...
ezUtrSequences = function(gtf, chromSeqs) {
  if (is.null(gtf$transcript_id)) {
    gtf$transcript_id = ezGffAttributeField(
      gtf$attributes,
      field = "transcript_id",
      attrsep = "; *",
      valuesep = " "
  ...
  for (nm in names(chromSeqs)) {
    ...
    if (nrow(utr3) > 0) {
      seqs = as.character(Views(
        chromSeqs[[nm]],
        start = utr3$start,
        end = utr3$end,
        names = utr3$transcript_id
      ))
...
getTranscriptGcAndWidth <- function(
  param = NULL,
  genomeFn = NULL,
  featureFn = NULL
) {
  ...
  data <- data %>%
    ...
    summarise_all(sum) %>%
    mutate(gc = signif(gcCount / featWidth, digits = 4)) %>%
    dplyr::select(-gcCount)
  return(data)
}
...
.checkGtfForExons = function() {
  ...
  gtf$Parent = ezGffAttributeField(
    x = gtf$attributes,
    field = "transcript_id",
    attrsep = "; *",
    valuesep = " "
...
trimGRanges <- function(x, width = 100, start = TRUE) {
  ...
  if (is.na(trimThis)) {
    ...
    if (trimThis > 1) {
      ...
      xTrim <- c(
        xTrim,
        resize(x[trimThis], fix = ifelse(start, "start", "end"), width = width)
      )
    } else {
      xTrim <- resize(
        x[trimThis],
