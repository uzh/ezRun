...
ezMethodSGSeq = function(input = NA, output = NA, param = NA) {
  ...
  if (!file.exists(txdbDatabase)) {
    txDbObject <- makeSGSeqtxdbObject(
      param$refBuild,
      param$refFeatureFile,
      txdbDatabase
    )
  ...
  usedChr <- scanBamHeader(bamFile)[[1]]$text[
    names(scanBamHeader(sampleInfoComplete$file_bam[1])[[1]]$text) == "@SQ"
  ]
  ...
  ### get counts
  varCounts <- getSGVariantCounts(
    rowRanges(sgfc_ucscV),
    sample_info = sampleInfoComplete
  )
  countValues <- counts(varCounts)
  vid <- variantID(varCounts)
  eid <- eventID(varCounts)
  inputForDexSeq <- data.frame(
    paste(eid, vid, sep = ":"),
    countValues,
    stringsAsFactors = FALSE
  )
