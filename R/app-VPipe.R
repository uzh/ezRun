...
ezMethodVPipe <- function(
  ...
  for (i in rownames(dataset)) {
    ...
    dest <- file.path(
      samples[i, 'ID'],
      samples[i, 'InternalNumber'],
      'raw_data'
    )
    dir.create(dest)
    system(paste(
      'ln -s',
      input$getFullPaths("Read1")[i],
      file.path(dest, basename(input$getFullPaths("Read1")[i]))
    ))
    system(paste(
      'ln -s',
      input$getFullPaths("Read2")[i],
      file.path(dest, basename(input$getFullPaths("Read2")[i]))
    ))
  }
  setwd('..')
  ...
  cmd <- paste(
    'export TMPDIR=/scratch/tmp; ./vpipe --use-conda -p --keep-going --rerun-incomplete --cores',
    param$cores,
    ' 2>&1 | tee',
    paste0(orderId, '.log')
  )
  ...
  for (j in 1:nrow(samples)) {
    cmd <- paste(
      ...
      file.path(
        '../results',
        samples[j, 'ID'],
        samples[j, 'InternalNumber'],
        '*'
      ),
      file.path(samples[j, 'ID'], samples[j, 'InternalNumber'])
    )
    ezSystem(cmd)
    cmd <- paste(
      'mv',
      file.path(
        samples[j, 'ID'],
        samples[j, 'InternalNumber'],
        'references/ref_ambig_dels.fasta'
      ),
      paste0(
        file.path(oDir, aDir),
        '/',
    ...
    cmd <- paste(
      ...
      file.path(
        samples[j, 'ID'],
        samples[j, 'InternalNumber'],
        'references/ref_majority_dels.fasta'
      ),
      paste0(
        file.path(oDir, cDir),
        '/',
  ...
  for (j in 1:nrow(samples)) {
    cmd <- paste(
      'bedtools coverage -a /srv/gstore/projects/p24680/gffs/artic.expected_amplicons.bed -b',
      file.path(
        '..',
        samples[j, 'ID'],
        samples[j, 'InternalNumber'],
        'variants/SNVs/REF_aln_indelqual.bam'
      ),
      '-mean -bed >',
      file.path(
        'bedtools_coverage',
        paste0(samples[j, 'ID'], '_', samples[j, 'InternalNumber'], '.bed')
      )
    )
    ezSystem(cmd)
    cmd <- paste(
      ...
      file.path(
        'samtools_depth',
        paste0(samples[j, 'ID'], '_', samples[j, 'InternalNumber'], '.tsv')
      )
    )
    ezSystem(cmd)
  }
