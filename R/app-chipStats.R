...
ezMethodChipStats = function(
  ...
  pngLinkList = ezMclapply(
    dataset$"BAM [File]",
    Create_ChIP_QCPlots_ind,
    param = param,
    gff = gff,
    maxX = 20,
    range = c(1, 100),
    ...
    mc.set.seed = FALSE
  ) ## REFAC: pngLinkList = ezMclapply() might not work properly
  ...
  plotCmd = expression({
    ...
    barplot(
      ...
      cex.names = 0.9,
      xlab = 'SSD',
      main = 'SSD of Coverage'
    )
    par(mar = c(5, 4, 4, 2) + 0.1)
  })
  ssdCoverageLink = ezImageFileLink(
    plotCmd,
    file = 'ssdCoveragePlot.png',
    width = 800,
    height = 800
  )
  ...
  titles[[
    "Frequency of identical Reads per Start Position"
  ]] = "Frequency of identical Reads per Start Position"
  addTitle(doc, titles[[length(titles)]], 2, id = titles[[length(titles)]])
  ...
  plotCmd = expression({
    plot(
      enrichmentData[[1]][, 1],
      log2(enrichmentData[[1]][, 2]),
      type = 'l',
      xlim = c(0, 20),
      col = cols[1],
      lwd = 2,
      main = 'Multi Sample Enrichment-Plot',
      xlab = 'Normalized Enrichment Level of Reads',
      ylab = 'Frequency (log2)'
    )
    for (i in 2:length(enrichmentData)) {
      lines(
        enrichmentData[[i]][, 1],
        log2(enrichmentData[[i]][, 2]),
        col = cols[i],
        lwd = 2
      )
    }
  ...
  comprehensiveEnrichmentLink = ezImageFileLink(
    plotCmd,
    file = "ComprehensiveEnrichmentPlot.png",
    width = 800,
    height = 800
  )
  ...
  clusterLink = ezImageFileLink(
    plotCmd,
    file = 'ClusteringAllBins.png',
    width = 800,
    height = 800
  )
  remove(tmp)
  ...
  addFlexTable(
    doc,
    ezGrid(cbind(comprehensiveEnrichmentLink, ssdCoverageLink, clusterLink))
  )
  ...
  setRefClass(
    ...
    methods = list(
      initialize = function() {
        ...
        name <<- "EzAppChipStats"
        appDefaults <<- rbind(
          estimatedFragmentSize = ezFrame(
            Type = "numeric",
            DefaultValue = '200',
            Description = "estimated size of the DNA fragments"
          ),
          flank = ezFrame(
            Type = "integer",
            DefaultValue = "200",
            Description = "flanking around the transcription start sites used for plotting"
          )
        )
      }
    )
...
Create_ChIP_QCPlots_ind = function(
  file,
  param,
  maxX = 20,
  gff = gff,
  ...
  pngLinkList$enrichmentLink = MakeEnrichmentPlot(
    myBam,
    param$paired,
    param$estimatedFragmentSize
  )
...
galp2gal = function(galp) {
  system(
    'echo Function galp2gal, conversion of paired-end data into single end data... \n'
  )
  ...
  if (length(idx) < length(gal)) {
    warning(sprintf(
      "%d read-pairs with end larger than chromosome length are discarded",
      length(gal) - length(idx) + 1
    ))
  }
...
fragmentSize = function(myBam) {
  ...
  sample_fragmentSize = fragmentSize[which(
    fragmentSize > minQuantile & fragmentSize < maxQuantile
  )]
  ...
  plotCmd = expression({
    ...
    plot(
      d,
      main = names(myBam),
      xlab = paste0("Median of FragmentSize:", medianFS)
    )
    polygon(d, col = "red", border = "black")
    legend("topright", c(names(myBam)))
  })
  fragmentLink = ezImageFileLink(
    plotCmd,
    file = paste0(names(myBam), "_fragmentSize.png")
  ) ## REFAC: won't names(myBam) produces several files with the same plot?
...
## @describeIn Create_ChIP_QCPlots_ind
MakeEnrichmentPlot = function(
  myBam,
  isPaired = F,
  estimatedFragmentSize = 200
) {
  ...
  plotCmd = expression({
    ...
    write.table(
      data[[1]],
      paste(names(myBam), '_EnrichedCoverage.txt', sep = ''),
      quote = F,
      row.names = F,
      sep = '\t'
    )
  })
  enrichmentLink = ezImageFileLink(
    plotCmd,
    file = paste0(names(myBam), "_SingleEnrichmentPlot.png")
  ) ## REFAC: won't names(myBam) produces several files with the same plot?
...
MakeCpGDensityPlot = function(
  ...
  if (build %in% bsgenome_table$build) {
    bsgenome_table = unlist(bsgenome_table[
      which(bsgenome_table$build == build),
    ])
    ...
    plotCmd = expression({
      Repitools::cpgDensityPlot(
        myBam,
        organism = get(bsgenome_table[2]),
        w.function = "none",
        seq.len,
        cols = c("black"),
        xlim = c(0, 30),
    ...
    densityLink = ezImageFileLink(
      plotCmd,
      file = paste0(names(myBam), "_cpgDensityPlot.png")
    )
...
getBSgenomes = function() {
  ...
  bs_packages = listall_availablePackages[grep(
    "^BSgenome.",
    listall_availablePackages
  )]
  organism = strsplit2(bs_packages, "\\.")[, 2]
  build = strsplit2(bs_packages, "\\.")[, 4]
  bsgenome_table = data.frame(
    bs_packages,
    organism,
    build,
    stringsAsFactors = F
...
StartPosTableFunction = function(myBam, maxX = 20) {
  ...
  startPosTable = table(table(paste(
    seqnames(myBam[[1]]),
    start(myBam[[1]]),
    strand(myBam[[1]]),
    sep = '_'
  ...
  plotCmd = expression({
    plot(
      x = as.numeric(names(startPosTable)),
      y = log2(startPosTable),
      type = 'l',
      ylab = "Frequency (log2)",
      xlab = "Reads_per_Start_Position",
      xlim = c(1, maxX),
      main = names(myBam),
      lwd = 3
    )
  ...
  startPosLink = ezImageFileLink(
    plotCmd,
    file = paste0(names(myBam), "_StartPositionPlot.png")
  ) ## REFAC: won't names(myBam) produces several files with the same plot?
...
CoverageVarFunction = function(myBam) {
  ...
  write.table(
    myCov,
    file = paste(names(myBam), "_ssdCoverage.txt", sep = ""),
    quote = F,
    row.names = F,
    col.names = F,
    sep = ""
...
createTSSPlot = function(myBam, gff, flank, name, range = c(1, 100)) {
  ...
  for (i in 1:ncol(positionData)) {
    posDataCounts[, i] = MyTable(
      shrinkToRange(positionData[!is.na(positionData[, i]), i], range),
      range
    )
  }
  ...
  plotCmd = expression({
    ...
    title(
      ...
      sub = "",
      col.sub = "blue",
      xlab = "Relative Genomic Position around TSS",
...
binMyBam = function(cov, binLength, sampleName) {
  ...
  write.table(
    MyBins,
    paste0(sampleName, '_', binLength, '_BinData.txt'),
    col.names = F,
    quote = F,
    sep = '\t'
  )
