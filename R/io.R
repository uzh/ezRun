...
ezWrite.table = function(
  ...
  if (row.names) {
    if (col.names) {
      write.table(
        matrix(c(head, colnames(values)), nrow = 1),
        file = file,
        sep = sep,
        quote = quote,
        col.names = FALSE,
...
ezListAsKable <- function(my_list, ...) {
  ezFrame(Value = my_list |> unlist(recursive = FALSE)) |>
...
ezInteractiveTableRmd = function(
  values,
  filter = "top",
  digits = NULL,
  columnsToRound = sapply(values, typeof) == "double",
