...
ezMethodTrinity = function(
  ...
  if (param$paired) {
    ...
    libOpt = switch(
      param$strandMode,
      sense = "--SS_lib_type FR",
      antisense = "--SS_lib_type RF",
      both = ""
    )
  } else {
    read1 = paste(trimmedInput$getColumn("Read1"), collapse = ",")
  ...
  cmd = paste(
    ...
    "--max_memory",
    paste0(param$ram, "G"),
    "--bflyCalculateCPU", ## "--bflyHeapSpaceMax", paste0(round(as.numeric(param$ram)/4), "G"),
    "--CPU",
    ezThreads(),
    libOpt,
    param$trinityOpt,
  ...
  cmd = paste(
    ...
    pathTranscripts,
    ">",
    basename(output$getColumn("Stats"))
  )
  ezSystem(cmd)
  ...
  for (nm in trimmedInput$getNames()) {
    ...
    if (param$paired) {
      readOpt = paste(
        "--left",
        sampleDs$getColumn("Read1"),
        "--right",
        sampleDs$getColumn("Read2")
      )
    } else {
  ...
  ezSystem(cmd)
  cmd = paste(
    "$Trinity/util/misc/contig_ExN50_statistic.pl",
  ...
  ezSystem(paste(
    "mv",
    paste0(abundancePrefix, ".isoform.counts.matrix"),
    basename(output$getColumn("Abundance Counts"))
  ))
  ezSystem(paste(
    "mv",
    paste0(abundancePrefix, ".isoform.TPM.not_cross_norm"),
    basename(output$getColumn("Abundance TPM"))
  ))
  ezSystem(paste(
    "mv",
    paste0(abundancePrefix, ".isoform.TMM.EXPR.matrix"),
    basename(output$getColumn("Abundance TMM"))
  ))
    ...
    methods = list(
      initialize = function() {
        ...
        appDefaults <<- rbind(
          trinityOpt = ezFrame(
            Type = "character",
            DefaultValue = "--min_kmer_cov 2",
            Description = "trinity commandline options"
          )
        )
      }
    )
