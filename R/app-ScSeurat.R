...
ezMethodScSeurat <- function(input = NA, output = NA, param = NA,
  ...
  if (ezIsSpecified(param$sctype.enabled) && (param$sctype.enabled == TRUE || param$sctype.enabled == "true")) {
    tryCatch({
      ...
      futile.logger::flog.info("scType annotation completed successfully using wrapper")
  ...
  if (ezIsSpecified(param$AzimuthPanHuman) && (param$AzimuthPanHuman == TRUE || param$AzimuthPanHuman == "true")) {
    tryCatch({
      ...
      futile.logger::flog.info("Azimuth Pan-Human annotation completed successfully")
      
    }, error = function(e) {
      futile.logger::flog.error("Azimuth Pan-Human annotation failed: %s", e$message)
    })
  } else {
    futile.logger::flog.info("Azimuth Pan-Human annotation disabled, skipping...")
  }
