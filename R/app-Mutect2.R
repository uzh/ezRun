...
ezMethodMutect2 = function(input = NA, output = NA, param = NA) {
  ...
  if (param$snpEffDB == 'mm39') {
    cmd <- paste(
      ...
      param$dataDir,
      "-csvStats",
      paste0(sampleName, ".snpeff.csv"),
      "-s",
      paste0(sampleName, ".snpeff.html"),
      param$snpEffDB,
      filteredOutFile,
      ">",
      annotatedOutFile
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          snpEffDB = ezFrame(
            Type = "character",
            DefaultValue = "",
            Description = "snpEffDB Name"
          ),
          TumorOnlyMode = ezFrame(
            Type = "logical",
            DefaultValue = FALSE,
            Description = "Run in tumor only mode"
          )
        )
      }
    )
