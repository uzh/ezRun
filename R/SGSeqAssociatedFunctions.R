...
makeSGSeqtxdbObject <- function(refBuil, refFeat, txdbDatabase) {
  ...
  txDbObject <- makeTxDbFromGFF(
    txdbGtfOrigin,
    format = "gtf",
    organism = organismForDB,
...
generateFlattenedFileForSGeq <- function(x) {
  ...
  buildExonRowsForFlattenedFile <- function(test) {
    ...
    if (length(geneToAnnotate) > 0) {
      geneNameToMap <- AnnotationDbi::mget(
        x = geneToAnnotate,
        envir = org.Mm.egSYMBOL,
        ifnotfound = NA
      )[[1]]
    } else {
  ...
  write.table(
    finalFlattened,
    'dexSeqFlattenedFileForSgSeq.txt',
    row.names = FALSE,
    col.names = FALSE,
    quote = FALSE,
    sep = "\t"
