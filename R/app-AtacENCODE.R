...
ezMethodAtacENCODE <- function(input = NA, output = NA, param = NA) {
  ...
  cmd <- paste(
    'caper run /usr/local/ngseq/src/atac-seq-pipeline/atac.wdl -i atac.json --singularity'
  )
  system(cmd)
...
createJson <- function(input, param) {
  ...
  atac_qc[["atac.genome_tsv"]] <- paste0(
    "https://storage.googleapis.com/encode-pipeline-genome-data/genome_tsv/v4/",
    refBuild,
    '.tsv'
  )
