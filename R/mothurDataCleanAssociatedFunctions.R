...
prepareFilesLocallyForMothur <- function(sushiInputDataset, param) {
  for (i in (1:nrow(sushiInputDataset))) {
    ...
    file1PathInDatset <- paste0(
      param$dataRoot,
      "/",
      sushiInputDataset$`Read1 [File]`[i]
    )
    file2PathInDatset <- paste0(
      param$dataRoot,
      "/",
      sushiInputDataset$`Read2 [File]`[i]
    )
...
createStepConvTableForKableExtra <- function(x) {
  ...
  fillEmptyDF <- function(inDF, totRows) {
    ...
    presRows <- nrow(inDF)
    df <- data.frame(
      matrix(ncol = ncols, nrow = totRows - presRows),
  ...
  rawSummaryTableFilled <- lapply(rawSummaryTable, function(x) {
    fillEmptyDF(x, totRows = finNumRows)
  })
