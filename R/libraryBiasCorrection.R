ezCorrectBias = function(
  ...
  useForNorm = gc >= gcCore[1] &
    gc <= gcCore[2] &
    logWidth >= widthCore[1] &
    logWidth <= widthCore[2]
  useForNorm = useForNorm & apply(counts > minCount, 1, mean) > minPresFraction

  require(DESeq2)
  dds = DESeqDataSetFromMatrix(
    countData = round(counts),
    colData = data.frame(names = colnames(counts)),
    ~names
  )
  ...
  binMatrix = tapply(
    (1:nGenes)[useForFit],
    list(logWidth = lwClasses[useForFit], gc = gcClasses[useForFit]),
    identity
  )
  binMatrixTemplate = matrix(
    0,
    nrow = nrow(binMatrix),
    ncol = ncol(binMatrix),
  ...
  gcmDef = (c(gcBreaks[1], gcBreaks) +
    c(gcBreaks, gcBreaks[length(gcBreaks)])) /
    2
  ...
  lwmDef = (c(logWidthBreaks[1], logWidthBreaks) +
    c(logWidthBreaks, logWidthBreaks[length(logWidthBreaks)])) /
    2
  ...
  return(list(
    ...
    logOffset = logOffset,
    nGenesForNorm = sum(useForNorm),
    nGenesForFit = sum(useForFit),
    gcShrinked = gcShrinked,
    logWidthShrinked = logWidthShrinked,
    binScoresFilled = binScoresFilled
  ))
}
