...
ezMethodSCTrajectoryInference <- function(
  input = NA,
  output = NA,
  param = NA,
  ...
  filePath <- file.path(
    "/srv/gstore/projects",
    sub("https://fgcz-(gstore|sushi).uzh.ch/projects", "", dirname(objectURLs)),
    "sce_h5"
  )
  ...
  if (file.exists(filePath)) {
    ...
    filePath <- file.path(
      "/srv/gstore/projects",
      sub(
        "https://fgcz-(gstore|sushi).uzh.ch/projects",
        "",
        dirname(objectURLs)
      ),
      "scData.rds"
  ...
  #Prepare the data
  dyno_dataset <- wrap_expression(
    expression = t(as.matrix(logcounts)),
  ...
  for (i in 1:length(TI_method)) {
    ...
    myModel$model[[1]] <- myModel$model[[1]] %>%
      add_dimred(
        dyndimred::dimred_mds,
        expression_source = dyno_dataset$expression
      )
  ...
  styleFiles <- file.path(
    ...
    c("fgcz.css", "SCTrajectoryInference.Rmd", "fgcz_header.html", "banner.png")
  )
  file.copy(from = styleFiles, to = ".", overwrite = TRUE)
  rmarkdown::render(
    input = "SCTrajectoryInference.Rmd",
    envir = new.env(),
    output_dir = ".",
