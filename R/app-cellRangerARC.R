...
ezMethodCellRangerARC <- function(input = NA, output = NA, param = NA) {
  ...
  RNADataDir <- input$getColumn("RNADataDir") %>%
    strsplit(",") %>%
    unlist() %>%
    sort() %>% ## support multiple komma-separated entreies
  ...
  cmd <- paste(
    ...
    ifelse(
      ezIsSpecified(param$excludeIntrons) && param$excludeIntrons,
      "--gex-exclude-introns",
      ""
    )
  )
  ...
  file.rename(
    file.path(cellRangerARCFolder, "outs"),
    basename(output$getColumn("ResultDir"))
  )
...
createARCLibraryFile <- function(RNADataDir, ATACDataDir) {
  #}, sampleName, peakName) {
  libraryFn <- "ARC-library.csv"
  sampleName <- list.dirs(RNADataDir, recursive = FALSE) %>%
    basename() %>%
    unique()
  peakName <- list.dirs(ATACDataDir, recursive = FALSE) %>%
    basename() %>%
    unique()
  ...
  rnaLib <- tibble(
    fastqs = normalizePath(RNADataDir),
    sample = sampleName,
    library_type = "Gene Expression"
  )
  atacLib <- tibble(
    fastqs = normalizePath(ATACDataDir),
    sample = peakName,
...
getCellRangerARCReference <- function(param) {
  ...
  if (ezIsSpecified(param$controlSeqs) | ezIsSpecified(param$secondRef)) {
    ...
    refDir <- sub(
      "\\.gtf$",
      paste0("_10XARC_SC_", cellRangerBase, "_Index"),
