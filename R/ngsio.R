...
loadCountDataset <- function(input, param) {
  ...
  if (!columnName %in% colnames(x1)) {
    return(list(
      error = paste0(
        "Specified column name not found in data!<br>columnName: '",
        columnName,
        "'\n",
  ...
  if (dataFeatureLevel == "isoform" && param$featureLevel == "gene") {
    ...
    seqAnnoDFData <- ezFeatureAnnotation(param, pull(x["id"]), dataFeatureLevel)
  ...
  if (ezIsSpecified(param$ezRef@refBuild)) {
    seqAnnoDFFeature <- ezFeatureAnnotation(
      param,
      rownames(counts),
      param$featureLevel
    )
  ...
  if (ezIsSpecified(param$correctBias) && param$correctBias) {
    ...
    counts <- ezCorrectBias(
      counts,
      gc = seqAnnoDFFeature$gc,
      width = seqAnnoDFFeature$featWidth
    )$correctedCounts
  }
  ...
  if (ezIsSpecified(param$runRUV) && as.logical(param$runRUV)) {
    ...
    ruvCorr = RUVs(
      counts,
      cIdx = rownames(counts),
      k = as.integer(param$kRUVFactors),
      scIdx = differences,
      epsilon = 10
    )
  ...
  seqAnno <- makeGRangesFromDataFrame(
    seqAnnoDFFeature,
  ...
  rawData <- SummarizedExperiment(
    ...
    metadata = list(
      isLog = FALSE,
      featureLevel = param$featureLevel,
      type = "Counts",
      countName = columnName,
      param = param
    )
  )
...
.makeSeqAnnoFromCounts <- function(x1, identifier) {
  seqAnnoDFFeature <- data.frame(
    row.names = x1[[identifier]],
...
loadSCCountDataset <- function(input, param) {
  ...
  if (length(input$getNames()) > 1L) {
    ...
    sce <- SingleCellExperiment::cbind(
  ...
  if (toupper(param$scProtocol) == "SMART-SEQ2") {
    ...
    if (ezIsSpecified(param$correctBias) && param$correctBias) {
      countMatrix <- ezCorrectBias(
        countMatrix,
        gc = seqAnno$gc,
        width = seqAnno$featWidth
      )$correctedCounts
    }
    ...
    countMatrixFn <- list.files(
      path = input$getFullPaths("CountMatrix"),
      pattern = "\\.mtx(\\.gz)*$",
      recursive = TRUE,
      full.names = TRUE
    )
    sce <- read10xCounts(dirname(countMatrixFn), col.names = TRUE)
    seqAnnoDF <- ezFeatureAnnotation(param, rownames(sce), dataFeatureLevel)
    seqAnno <- makeGRangesFromDataFrame(seqAnnoDF, keep.extra.columns = TRUE)
...
load10xSC_seurat <- function(input, param) {
  ...
  geneID <- read_tsv(
    gzfile(paste0(input$getFullPaths("CountMatrix"), "/features.tsv.gz")),
    col_names = FALSE
  )
  if (grepl("FeatBarcoding", param$appName)) {
    counts_antibodies <- counts[["Antibody Capture"]]
    colnames(counts_antibodies) <- paste(
      input$getNames(),
      colnames(counts_antibodies),
      sep = "___"
    )
...
load10xSpatialData <- function(input, param) {
  ...
  data_raw <- read10xRaw(file.path(
    input$getFullPaths("ResultDir"),
  ...
  if (length(missingBarcodes) > 0) {
    data_slide_info$slide <- data_slide_info$slide[
      !(data_slide_info$slide$barcode %in% missingBarcodes),
    ]
  }

  data_obj <- createSlide(count_mat = data_raw, slide_info = data_slide_info)
  scDataRaw <- convertToSpatialSeurat(
    data_obj,
    image_dir = file.path(input$getFullPaths("ResultDir"), "spatial"),
    filter_matrix = FALSE
  )
  ...
  if (param$spotClean) {
    ...
    img = Read10X_Image(
      file.path(input$getFullPaths("ResultDir"), "spatial"),
      image.name = "tissue_hires_image.png"
    )
    param$imageEnlargementFactor <- img@scale.factors$hires /
      img@scale.factors$lowres
  ...
  scData <- RenameCells(
    scData,
    new.names = paste(input$getNames(), colnames(scData), sep = "___")
  )
...
convertToSpatialSeurat <- function(
  slide_obj,
  image_dir,
  slice = "slice1",
  filter_matrix = TRUE
...
readNcProResult = function(ncproDir) {
  ...
  rawData = list(
    counts = as.matrix(allData),
    presentFlag = allData > 0,
    seqAnno = seqAnno,
    featureLevel = "isoform",
    type = "Counts",
    countName = "ncpro-counts",
    isLog = FALSE,
    types = types
  )
}
...
filterFastqByBam = function(
  ...
  gc()
  fqFiltered = removeReadsFromFastq(
    fqFiles,
    bamReads,
    fqOutFiles = fqOutFiles,
    doGzip = doGzip
  )
  return(fqFiltered)
}
...
ezReadBamFileAsGRanges <- function(
  bamfile,
  bamindex = bamfile,
  chromosomes = NULL,
  ...
  gr <- GenomicRanges::GRanges(
    seqnames = chroms2use,
    ranges = IRanges(
      start = rep(1, length(chroms2use)),
      end = chrom.lengths[chroms2use]
    )
  )
  if (!remove.duplicate.reads) {
    if (pairedEndReads) {
      data.raw <- GenomicAlignments::readGAlignmentPairs(
        bamfile,
        index = bamindex,
        param = Rsamtools::ScanBamParam(which = range(gr), what = what)
      )
    } else {
      data.raw <- GenomicAlignments::readGAlignments(
        bamfile,
        index = bamindex,
        param = Rsamtools::ScanBamParam(which = range(gr), what = what)
      )
  ...
  if (length(data.raw) == 0) {
    if (pairedEndReads) {
      stop(paste0(
        "No reads imported. Does your file really contain paired end reads? Try with 'pairedEndReads=FALSE'"
      ))
    }
  ...
  if (pairedEndReads) {
    ...
    if (min.mapq > 0) {
      ...
      if (any(is.na(mapq.mask))) {
        warning(paste0(
          bamfile,
          ": Reads with mapping quality NA (=255 in BAM file) found and removed. Set 'min.mapq=NULL' to keep all reads."
        ))
      }
  ...
  data <- data[
    as.vector(seqnames(data)) %in% seqlevels(data)[!is.na(seqlengths(data))]
  ]
  data <- keepSeqlevels(data, as.character(unique(seqnames(data))))
  if (length(na.seqlevels) > 0) {
    warning(
      "Dropped seqlevels because no length information was available: ",
