      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          ...
          Niche_resolution = ezFrame(
            Type = "numeric",
            DefaultValue = 0.6,
            Description = "Value of the Niche resolution parameter, use a value above (below) 1.0 if you want to obtain a larger (smaller) number of communities."
          ),
...
ezMethodSpatialSeuratHD <- function(
  input = NA,
  output = NA,
  param = NA,
  ...
  dataDir <- file.path(
    dirname(input$getFullPaths("CountMatrix")),
    'binned_outputs'
  )
  if (param$binSize == 8) {
    ...
    stop(
      "Only bin sizes of 8 and 16 or even numbers between 10-100 are supported if the parameter --custom-bin-size was used for SpaceRanger before"
    )
  }
...
runBasicProcessingHD <- function(scData, input, featInfo, param, BPPARAM) {
  ...
  scData[[myAssay]] <- AddMetaData(
    object = scData[[DefaultAssay(scData)]],
  ...
  if (nrow(scData@meta.data) < 50000) {
    ...
    scData$seurat_cluster.projected <- factor(
      scData$seurat_cluster.projected,
      levels(scData$seurat_cluster.sketched)
    )
