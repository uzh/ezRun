...
bam2fastq <- function(
  ...
  if (isTRUE(OUTPUT_PER_RG)) {
    ...
    toFns <- file.path(
      OUTPUT_DIR,
      str_replace(fastqFns, "_1\\.fastq$", "_R1.fastq")
    )
...
.ezMethodBam2Fastq_PerRG <- function(input = NA, output = NA, param = NA) {
  ...
  output$setColumn(
    ...
    file.path(
      getwd(),
      str_c(output$getNames(), "_R1.fastq")
    )
  )
  if (param$paired) {
    output$setColumn(
      ...
      file.path(
        getwd(),
        str_c(output$getNames(), "_R2.fastq")
      )
    )
  } else {
  ...
  fastqsToRemove <- allFastqFns[
    !(basename(allFastqFns) %in%
...
.ezMethodBam2Fastq_NotPerRG <- function(input = NA, output = NA, param = NA) {
  if (!is(output, "EzDataset")) {
    ...
    outputMeta[["Read1"]] <- file.path(
      getwd(),
      str_c(rownames(outputMeta), "_R1.fastq")
    )
    outputMeta[["Read1 [File]"]] <- NULL
    if (param$paired) {
      outputMeta[["Read2"]] <- file.path(
        getwd(),
        str_c(rownames(outputMeta), "_R2.fastq")
      )
...
ezMethodBam2Fastq <- function(
  ...
  if (isTRUE(OUTPUT_PER_RG)) {
    output <- .ezMethodBam2Fastq_PerRG(
      input = input,
      output = output,
      param = param
    )
  } else {
    output <- .ezMethodBam2Fastq_NotPerRG(
      input = input,
      output = output,
      param = param
    )
...
ezMethodSubsampleFastq <- function(
  input = NA,
  output = NA,
  param = NA,
  n = 1e6
  ...
  output$setColumn(
    "Read Count",
    ifelse(input$getColumn("Read Count") < n, input$getColumn("Read Count"), n)
  )
  ...
  parallel::mclapply(
    samples,
    function(sm, input, output, param, n) {
      subsampleFastqFile(
        input$getFullPaths("Read1")[sm],
        output$getColumn("Read1")[sm],
        nReads = n
      )
      if (param$paired) {
        subsampleFastqFile(
          input$getFullPaths("Read2")[sm],
