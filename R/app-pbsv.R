...
ezMethodPbsv <- function(
  ...
  cmd = paste(
    "/srv/GT/software/SMRTtools/SMRT_Link_v10/smrtcmds/bin/pbsv discover",
    param$afOptions
  )
  ...
  PbsvCallCmd = paste(
    "/srv/GT/software/SMRTtools/SMRT_Link_v10/smrtcmds/bin/pbsv call --log-file",
    PbsvLogFile,
    "--types",
    param$types,
    "--min-sv-length",
  ...
  styleFiles <- file.path(
    ...
    c("fgcz.css", "Pbsv.Rmd", "fgcz_header.html", "banner.png")
  )
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          ReadOpt = ezFrame(
            Type = "character",
            DefaultValue = "HIFI",
            Description = "input read types: SUBREAD, CCS, HIFI. Default is HIFI"
          )
        )
      }
    )
