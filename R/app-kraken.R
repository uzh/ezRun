...
ezMethodKraken = function(input=NA, output=NA, param=NA, htmlFile="00index.html"){
  ...
  if (param$paired){
    ...
    cmd = paste("/usr/local/ngseq/stow/kraken2-2.0.7-beta/bin/kraken2 -db", dbOpt, "--paired", "--confidence", conOpt, "--minimum-base-quality", phredOpt,
           "--outout", paste0(sampleName,".txt"), "--threads", ezThreads(), param$cmdOptions, readOpt, "1> ", paste0(sampleName,".log"))
    ezSystem(cmd)
    ...
    read1 = trimmedInput$getColumn("Read1")
    readOpt = paste(read1)
    cmd = paste("/usr/local/ngseq/stow/kraken2-2.0.7-beta/bin/kraken2 -db", dbOpt, "--confidence", conOpt, "--minimum-base-quality", phredOpt,
           "--outout", paste0(sampleName,".txt"), "--threads", ezThreads(), param$cmdOptions, readOpt, "1> ", paste0(sampleName,".log"))
    ezSystem(cmd)
    cmd = paste("/usr/local/ngseq/stow/KronaTools-2.7/bin/ktImportTaxonomy -q 2 -t 3", paste0(sampleName,".txt"), "-o", paste0(sampleName,".html"))
    ezSystem(cmd)
