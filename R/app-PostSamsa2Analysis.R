...
ezMethodPostSamsa2Analysis = function(
  input = NA,
  output = NA,
  param = NA,
  ...
  for (directory in c("orgresults/", "funcresults/")) {
    ...
    for (i in as.list(
      dataset[
        dataset[, grepl(group, names(dataset))] == refGroup,
      ]$annotationFileRefSeq..File.
    )) {
      factor_list <- c(
        factor_list,
        unlist(strsplit(i, split = '/', fixed = TRUE))[3]
      )
    }
    ...
    for (i in factor_list2) {
      list_of_files_torename <- c(
        list_of_files_torename,
        list.files(path = directory, pattern = i)
      )
    }
    list_of_files_torename <- list_of_files_torename[-1]
    print(list_of_files_torename)
    add_prefix <- function(x, path = directory, sep = "_") {
      paste(paste0(directory, "control"), x, sep = sep)
    }
    file.rename(
      paste0(directory, list_of_files_torename),
      add_prefix(list_of_files_torename)
    )
  }
  ...
  for (directory in c("orgresults/", "funcresults/")) {
    ...
    for (i in as.list(
      dataset[
        dataset[, grepl(group, names(dataset))] == sampleGroup,
      ]$annotationFileRefSeq..File.
    )) {
      factor_list <- c(
        factor_list,
  ...
  samsa2RscriptsToBeExecCmd1 <- paste(
    "bash /usr/local/ngseq/src/samsa2/R_scripts/run_all_Rscripts.sh orgresults/"
  )
  ezSystem(samsa2RscriptsToBeExecCmd1)
  samsa2RscriptsToBeExecCmd2 <- paste(
    "bash /usr/local/ngseq/src/samsa2/R_scripts/run_all_Rscripts.sh funcresults/"
  )
  ...
  makeRmdReport(
    param = param,
    org_Shannon_Simpson = org_Shannon_Simpson,
    func_Shannon_Simpson = func_Shannon_Simpson,
    org_div_graph = org_div_graph,
    func_div_graph = func_div_graph,
    org_PCA = org_PCA,
    func_PCA = func_PCA,
    org_heatmap = org_heatmap,
    func_heatmap = func_heatmap,
    org_combined = org_combined,
    func_combined = func_combined,
    org_volcano = org_volcano,
    func_volcano = func_volcano,
    org_deseq_results = org_deseq_results,
    func_deseq_results = func_deseq_results,
    output = output,
    rmdFile = markdownFile,
    reportTitle = "Samsa2 Report"
  )
}
