...
ezMethodFastqScreen <- function(
  ...
  if (
    "Species" %in%
      input$colNames &&
      grepl("^Human|^Homo", input$getColumn("Species")[1])
  ) {
    param[["virusCheck"]] <- T
  }
  ...
  ## map to adapters ----
  rawScreenResult = getFastqScreenStats(
    param,
    confFile = FASTQSCREEN_ADAPTER_CONF,
    inputRaw$getFullPaths("Read1"),
    workDir = "rawReads"
  )

  procScreenResult = getFastqScreenStats(
    param,
    confFile = FASTQSCREEN_GENOMICDNA_RIBORNA_CONF,
    inputProc$getFullPaths("Read1"),
    workDir = "procReads"
  )
  if (param[["virusCheck"]]) {
    unmappedFiles = gsub(
      ".fastq.gz$",
      ".tagged_filter.fastq.gz",
      file.path("procReads", basename(inputProc$getFullPaths("Read1")))
    )
    names(unmappedFiles) = inputProc$getNames()
    virusResult <- map_and_count_virus(
      param,
      unmappedFiles,
      workDir = "virusResult"
    )
  ...
  refseqResult = map_and_count_refseq(
    param,
    inputProc$getFullPaths("Read1"),
    workDir = "refseqResult",
  ...
  if (param$paired) {
    ...
    for (i in 1:length(sampleNames)) {
      ...
      reads <- sread(fq)
      consMatrix <- consensusMatrix(reads, as.prob = TRUE)
      consMatrix <- consMatrix[
        rownames(consMatrix) %in% c('A', 'C', 'G', 'T'),
      ]
  ...
  makeRmdReport(
    ...
    param = param,
    input = input,
    rawScreenResult = rawScreenResult,
    procScreenResult = procScreenResult,
    virusResult = virusResult,
    rRNAstrandResult = rRNAstrandResult,
    krakenResult = krakenResult,
    refseqResult = refseqResult,
    PWMs = PWMs,
    rmdFile = "FastqScreen.Rmd",
    reportTitle = paste("Fastq Screen", param$name)
  )
  return("Success")
}
...
getFastqScreenStats <- function(
  param,
  confFile = NULL,
  files,
  workDir = "fastqScreenTmp"
) {
  dir.create(workDir)
  cmd <- paste(
    "fastq_screen",
  ...
  for (nm in names(files)) {
    ...
    if (is.na(emptyLinePos)) {
      ## no hits found!
      emptyLinePos = 3
    }
    x <- ezRead.table(
      file.path(workDir, resultFile),
      skip = 1,
      stringsAsFactors = F,
      nrows = emptyLinePos - 3
    ) ## first line and
    fastqData[[nm]] = x
  }
  return(fastqData)
...
get_rRNA_Strandness <- function(param, input) {
  ...
  for (nm in names(countFiles)) {
    ...
    countData <- countData[
      countData$AlignmentScore == bestScores[countData$ReadID],
      ,
      drop = FALSE
    ]
    countData <- countData[
      countData$AlignmentScore >= param$minAlignmentScore,
      ,
      drop = FALSE
    ]
...
runKraken <- function(param, input) {
  ...
  for (nm in names(r1Files)) {
    ...
    if (length(readLines(resultFile)) > 0) {
      ...
      data = ezFrame(
        "readPercentage" = numeric(0),
        "nreads_clade" = numeric(0),
        "nreads_taxon" = numeric(0),
        "rankCode" = character(0),
        "ncbi" = character(0),
        "name" = numeric(0)
      )
    }
...
map_and_count_virus <- function(
  param,
  files,
  workDir = "virusResult",
  readCount = countReadsInFastq(files)
) {
  dir.create(workDir)
  countFiles <- character()
  for (nm in names(files)) {
    ...
    cmd <- paste(
      "bowtie2",
      "-x",
      REFSEQ_pathogenicHumanViruses_REF,
      " -U ",
      files[nm],
      bowtie2options,
      "-p",
      ...
      "--no-unal --no-hd",
      "2> ",
      paste0(workDir, "/", nm, "_bowtie2.err"),
      "| cut -f1,3,12",
      " |sed s/AS:i://g",
      ">>",
      countFiles[nm]
  ...
  countList = collectBowtie2Output(
    param,
    countFiles,
    readCount,
    virusResult = T
...
map_and_count_refseq <- function(
  ...
  if (file.exists(outputCountFile) && file.size(outputCountFile) > 0) {
    ...
    system(paste(
      'join -j 1 -o 1.1,1.2,1.3,2.2',
      completeOutputCountFile,
      readToReadGroupFile,
      '>',
      outputCountFile
    )) #ezSystem thinks that it fails
    file.remove(
      completeOutputCountFile,
      bamFile,
      inputFastq,
...
collectBowtie2Output <- function(
  ...
  tax2name <- read.table(
    sub("Sequence.*", "Annotation/tax2name.txt", REFSEQ_mRNA_REF),
    header = F,
    stringsAsFactors = F,
    sep = "\t",
  ...
  for (nm in names(countFiles)) {
    ...
    if (nrow(countData) > 0) {
      if (virusResult) {
        countData$species <- substr(
          sub("_NC_[0-9].*", "", countData$RefSeqID),
          1,
          30
        )
      } else {
      ...
      uniqSpeciesHitsPerRead <- names(speciesHitsPerRead)[
        lengths(speciesHitsPerRead) == 1
      ]
      ### Result UniqHits:
      uniqSpeciesHits <- sort(
        table(unlist(speciesHitsPerRead[uniqSpeciesHitsPerRead])),
        decreasing = T
      )
      ### Results MultipleHits:
      multipleSpeciesHitsPerRead <- countData[
        !(countData$ReadID %in% uniqSpeciesHitsPerRead),
      ]
      by <- paste(
        multipleSpeciesHitsPerRead$ReadID,
      ...
      multipleSpeciesHits <- sort(
        table(tapply(multipleSpeciesHitsPerRead$species, by, unique)),
        decreasing = T
      )
      ...
      if (length(uniqSpeciesHits) == 0) {
        topSpeciesUniq <- integer(min(
          param$nTopSpecies,
          length(multipleSpeciesHits)
        ))
        names(topSpeciesUniq) <- names(multipleSpeciesHits)[
          1:min(
            param$nTopSpecies,
            length(multipleSpeciesHits)
          )
      ...
      multipleSpeciesHits[setdiff(
        names(topSpeciesUniq),
        names(multipleSpeciesHits)
      )] <- 0
      ...
      x <- ezFrame(
        UniqueSpeciesPercent = signif(
          100 * as.matrix(topSpeciesUniq) / readCount[nm],
          digits = 4
        ),
        MultipleSpeciesPercent = signif(
          100 * as.matrix(topSpeciesMultiple) / readCount[nm],
          digits = 4
        ),
        row.names = taxNames
      )
...
makeScatterplot <- function(dataset, colname1, colname2) {
  if (
    colname1 %in%
      colnames(dataset) &&
      colname2 %in% colnames(dataset) &&
      nrow(dataset) > 1
  ) {
    if (
      !all(dataset[[colname1]] == 0) &&
        !any(is.na(dataset[[colname1]])) &&
        !all(dataset[[colname2]] == 0) &&
        !any(is.na(dataset[[colname2]]))
    ) {
