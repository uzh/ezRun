...
cellxgene_annotation <- function(scData, param) {
  ...
  if (ezIsSpecified(param$cellxgene) && ezIsSpecified(param$column_name_of_cell_label)){
    ...
    unharmonised_metadata <- get_unharmonised_metadata(curated_single_cell_experiment_object)
    ...
    ## mapping
    scData.anchors <- FindTransferAnchors(reference = seurat.combined.sct, query = scData, dims = 1:30,
                                          reference.reduction = "pca")
    
    if (!cell_label_author %in% colnames(seurat.combined.sct@meta.data)) {
      stop("The specified column name for cell labels does not exist in the reference object's metadata.")
    }
    
    predictions.scData <- TransferData(anchorset = scData.anchors, refdata = seurat.combined.sct@meta.data[[cell_label_author]], dims = 1:30)
