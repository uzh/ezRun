...
autoEstContTfidfMin <- function(sc, tfidfMin) {
  while (tfidfMin > 0) {
    scOut <- try({
      autoEstCont(sc, tfidfMin = tfidfMin, forceAccept = T, doPlot = FALSE)
    })
...
addAmbientEstimateToSeurat <- function(scData, rawDir = NULL, param = NULL) {
  ...
  sce <- SingleCellExperiment(
    assays = list(counts = GetAssayData(scData, layer = "counts")),
    colData = scData@meta.data
  )
  sce$clusters <- Idents(scData)
  ## decontx
  ctsClean <- decontX(sce, z = sce$clusters)
  ctsClean <- decontXcounts(ctsClean)
  contaminationFraction <- (colSums2(counts(sce)) - colSums2(ctsClean)) /
    colSums2(counts(sce))
  scData <- AddMetaData(
    scData,
    metadata = contaminationFraction,
    col.name = paste0("DecontX_contFrac")
  )
  ## SoupX
  if (!is.null(rawDir) && file.exists(rawDir)) {
    if (param$cellbender) {
      tod <- checkAndCleanAntibody(Seurat::Read10X_h5(
        file.path(dirname(rawDir), 'cellbender_filtered_seurat.h5'),
        use.names = FALSE
      ))
      ...
      if (!is.null(param$featInfo_h5)) {
        ...
        !is.null(param$featuresPath) && file.exists(param$featuresPath)
      ) {
        ...
        countMatrixToUse <- if (
          dirname(param$cellrangerCountFiltDir) !=
            dirname(param$cellrangerCountRawDir)
        ) {
        ...
        if (
          !file.exists(featuresFile) &&
            grepl("CellRangerMulti", countMatrixToUse)
        ) {
        ...
        if (file.exists(featuresFile)) {
          ...
          warning(paste0(
            "Could not find features.tsv.gz file at: ",
            featuresFile
          ))
    ...
    rownames(tod) <- gsub(
      "_",
      "-",
      uniquifyFeatureNames(ID = featInfo$ensemblID, names = featInfo$name)
    )
    ...
    if (length(class(sc)) == 2L) {
      if (!is.null(sc$fit) && "rho" %in% colnames(sc$metaData)) {
        ...
        scData <- AddMetaData(
          scData,
          metadata = contaminationFraction,
          col.name = paste0("SoupX_contFrac")
        )
      }
    }
  }
