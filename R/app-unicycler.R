      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          pathToLong = ezFrame(
            Type = "character",
            DefaultValue = "",
            Description = "Specify path to the FASTA or FASTQ file for your long reads."
          ),
          unicyclerOpt = ezFrame(
            Type = "character",
            DefaultValue = "",
            Description = "Predefine any options not already specified by default. By default is empty for genome assembly"
          )
        )
      }
    )
