...
ezMethodBamPreview = function(
  ...
  bamMeta[["Read Count"]] = ceiling(
    bamMeta[["Read Count"]] / param$subsampleReads
  )
  ...
  for (nm in input$getNames()) {
    setwdNew(nm)
    mappingApp$run(
      input = input$subset(nm),
      output = bamOutput$subset(nm),
      param = bamParam
    ) ## TODO: Read Count of the bamOutput should be set by the mapping app.
    setwd("..")
  }
  param$dataRoot = ""
