...
makeSTARsoloCmd = function(
  ...
  stopifnot(
    "Could not find barcode whitelist" = file.exists(barcodeInclusionListPath)
  )
...
getBarcodeInclusionListPath <- function(param) {
  soloCBwhitelist = list(
    SC3Pv1 = paste0(
      Sys.getenv("CellRanger"),
      '/lib/python/cellranger/barcodes/737K-april-2014_rc.txt'
    ),
    SC3Pv2 = paste0(
      Sys.getenv("CellRanger"),
    ...
    SC3Pv3 = paste0(
      Sys.getenv("CellRanger"),
      '/lib/python/cellranger/barcodes/3M-february-2018.txt'
    ),
    SC3Pv4 = paste0(
      Sys.getenv("CellRanger"),
      '/lib/python/cellranger/barcodes/3M-3pgex-may-2023_TRU.txt'
    )
  )
  ...
  soloWhiteListPathVariation <- sprintf(
    "%s_TRU.txt",
    file_path_sans_ext(soloWhiteListPath)
  )
...
writeVelocytoFeature <- function(subFeature, fromDir, destDir) {
  ...
  file.copy(from = Sys.glob(file.path(fromDir, "*.tsv.gz")), to = destSubDir)
  file.copy(
    from = file.path(fromDir, paste0(subFeature, ".mtx.gz")),
...
writeGenericFeature <- function(outputDir, subRawDir, subFilteredDir) {
  ...
  file.copy(
    from = Sys.glob(file.path(filtOutputDir, "*.gz")),
    to = subFilteredDir
  )
}
...
getSTARSoloReference <- function(param) {
  ...
  if (ezIsSpecified(param$controlSeqs)) {
    ...
    refDir <- sub(
      "\\.gtf$",
      paste0("_STARSolo_", starSoloBase, "_Index"),
  ...
  cmd <- paste(
    ...
    genomeLocalFn,
    "--outTmpDir",
    tempfile(pattern = "", fileext = "_STARtmp", tmpdir = getwd()),
    "--sjdbGTFfile",
    gtfFile,
    "--sjdbOverhang 150",
    "--runThreadN",
