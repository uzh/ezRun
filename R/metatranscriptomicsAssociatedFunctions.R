...
metagMetatrDifferentialAnalysis_tableAndPlots <- function(
  countMatrix,
  designMatrix,
  group,
  sampleGroup,
  ...
  filteredMatrix <- filterCountsForDiffExpr(
    countMatrix,
    designMatrix,
    group,
    sampleGroup,
  ...
  if (mode == "expression") {
    ...
    samplesGroup1 <- rownames(designMatrix[
      designMatrix[[group]] == sampleGroup,
    ])
...
orgFuncHeatmapPrep <- function(listOfAnnotatedAbundTable, diffTableFunc) {
  ...
  topDiff <- grep(
    ...
    rownames(diffTableFunc$tableToReport)[1:20],
    invert = T,
    value = T
  )
