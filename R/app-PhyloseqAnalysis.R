...
ezMethodPhyloSeqAnalysis = function(
  input = NA,
  output = NA,
  param = NA,
  ...
  if (isGroupThere) {
    ...
    if (N == 1) {
      ...
      sample_data(physeqObjectNoTreeUnfilt)[, "dummy"] <- sample_data(
        physeqObjectNoTreeUnfilt
      )[, 1]
    } else {
      areThereMultVar <- TRUE
    }
    ### also ensure the groups are factor
    sample_data(physeqObjectNoTreeUnfilt)@.Data <- lapply(
      sample_data(physeqObjectNoTreeUnfilt)@.Data,
      as.factor
    )
  } else {
    areThereMultVar <- FALSE
  ...
  ### Filtering step
  physeqFullObject <- phyloSeqPreprocess(
    physeqObjectNoTreeUnfilt,
    rawCount,
    sampleFraction
  )
  ...
  makeRmdReport(
    param = param,
    output = output,
    physeqFullObject = physeqFullObject,
    QCChimeraObject = QCChimeraObject,
    physeqObjectNoTreeUnfilt = physeqObjectNoTreeUnfilt,
    rmdFile = markdownFile,
    reportTitle = "16S Phyloseq Report"
  )
  ...
  setRefClass(
    ...
    methods = list(
      initialize = function() {
        ...
        appDefaults <<- rbind(
          taxonomicRank = ezFrame(
            Type = "character",
            DefaultValue = "Phylum",
            Description = "Rank to be highlighted in the plots"
          ),
          numTopRanks = ezFrame(
            Type = "numeric",
            DefaultValue = "15",
            Description = "Number of top OTUs for the plots"
          ),
          rawCount = ezFrame(
            Type = "numeric",
            DefaultValue = "15",
            Description = "OTUs with fewer than these counts in less than the fraction of samples below will be removed."
          ),
          sampleFraction = ezFrame(
            Type = "numeric",
            DefaultValue = "15",
            Description = "Minimum fraction of samples for which the raw count threshold above applies"
          )
        )
      }
    )
