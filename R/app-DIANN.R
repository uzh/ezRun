...
ezMethodDIANN = function(
  ...
  write_yaml(
    list(
      params = param,
      registration = list(workunit_id = "0000", container_id = "1111")
    ),
    "work/params.yml"
  )
  ## get the input
  ...
  cmd <- paste0(
    "scp ",
    "fgcz-r-033:",
    param$`03b_additional_fasta_database_path`,
    " work/input/",
    basename(param$`03b_additional_fasta_database_path`)
  )
  ezSystem(cmd)
  cmd <- paste0(
    "scp ",
  ...
  write_csv(
    ds[, c("Resource", "Relative Path", "Name", "Grouping Var", "File")],
    "work/dataset.csv"
  )
  ...
  ezSystem(paste(
    "mv",
    "work/out-DIANN_quantB",
    output$getColumn("DIANN Quant")
  ))
