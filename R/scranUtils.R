...
scranIntegration <- function(sceList, method = c("None", "MNN")) {
  ...
  decList <- lapply(sceList, function(x) {
    metadata(x)$dec
  })
  decList <- lapply(decList, function(x) {
    x[order(x$bio, decreasing = TRUE), ]
  })
  universe <- Reduce(intersect, lapply(decList, rownames))
  bioMatrix <- do.call(
    cbind,
    lapply(decList, function(x) {
      x[universe, "bio"]
    })
  )
  ...
  if (method == "None") {
    sce <- SingleCellExperiment(
      assays = list(
        counts = do.call(cbind, lapply(rescaled, assay, "counts")),
    ...
    sce <- do.call(
      batchelor::fastMNN,
      c(
        lapply(rescaled, function(x) {
          x[chosen, ]
        }),
        k = 20,
        d = 50,
        BSPARAM = IrlbaParam(deferred = FALSE)
      )
    )
    assay(sce, "counts") <-
      do.call(
...
scranPosMarkers <- function(sce) {
  ...
  for (cluster in names(markers_any)) {
    markersPerCluster <- data.frame(
      gene_name = rownames(markers_any[[cluster]]),
      markers_any[[cluster]],
      row.names = NULL
    )
...
scranDiffGenes <- function(sce, param) {
  ...
  summed <- aggregateAcrossCells(
    sce,
    id = colData(sce)[, c("ident", "Sample_name")]
  )
  ...
  if (param$DE.regress == "Batch") {
    design <- model.matrix(
      ~ factor(Batch) + factor(Condition),
      data = colData(summed.filt)
    )
  ...
  for (cluster in names(de.results)) {
    diffGenesPerCluster <- data.frame(
      gene_name = rownames(de.results[[cluster]]),
      de.results[[cluster]],
      row.names = NULL
    )
    diffGenesPerCluster <- diffGenesPerCluster[
      which(diffGenesPerCluster$FDR < 0.05),
    ]
