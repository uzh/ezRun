...
ezMethodMetatranscriptomeAnalysis = function(
  input = NA,
  output = NA,
  param = NA,
  ...
  listOfAnnotatedAbundTable <- lapply(
    annotationFiles,
    convertDiamondAnnotationToAbund
  )
  listOfAnnotatedAbundTableOrg <- lapply(
    listOfAnnotatedAbundTable,
    function(x) x[["orgAbundDF"]]
  )
  listOfAnnotatedAbundTableFunc <- lapply(
    listOfAnnotatedAbundTable,
    function(x) x[["funcAbundDF"]]
  )
  orgDFforHeatmap <- listOfAbundMerge(listOfAnnotatedAbundTableOrg, sampleNames)
  funcDFforHeatmap <- listOfAbundMerge(
    listOfAnnotatedAbundTableFunc,
    sampleNames
  )
  ...
  styleFiles <- file.path(
    ...
    c(
      "fgcz.css",
      "metatranscriptomicsAnalysis.Rmd",
      "fgcz_header.html",
      "banner.png"
    )
  )
  ...
  rmarkdown::render(
    input = "metatranscriptomicsAnalysis.Rmd",
    envir = new.env(),
    output_dir = ".",
  ...
  setRefClass(
    ...
    methods = list(
      initialize = function() {
        ...
        appDefaults <<- rbind(
          numberOfTopNCategories = ezFrame(
            Type = "integer",
            DefaultValue = "20",
