...
ezMethodMpileup = function(input = NA, output = NA, param = NA) {
  ...
  bamFilesClean = ezMclapply(
    names(bamFiles),
    function(sampleName) {
      javaCall = paste0(
        "java",
        " -Djava.io.tmpdir=. -Xmx",
        floor(param$ram / nBamsInParallel),
        "g"
      )
      ...
      initialize = function() {
        ...
        appDefaults <<- rbind(
          region = ezFrame(
            Type = "character",
            DefaultValue = "",
            Description = "should analysis be done on a region of the genom only chr:start-end"
          ),
          mpileupOptions = ezFrame(
            Type = "character",
            DefaultValue = "",
            Description = "options to mpileup command"
          ),
          callOptions = ezFrame(
            Type = "character",
            DefaultValue = "",
            Description = "options to call command"
          ),
          filterOptions = ezFrame(
            Type = "character",
            DefaultValue = "",
            Description = "options to filter command"
          ),
