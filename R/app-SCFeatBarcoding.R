...
ezMethodSCFeatBarcoding <- function(
  input = NA,
  output = NA,
  param = NA,
  ...
  scData.singlet <- addCellCycleToSeurat(
    scData.singlet,
    param$refBuild,
    BPPARAM
  )
  ...
  if (species == "Human" | species == "Mouse") {
    cells_AUC <- cellsLabelsWithAUC(
      GetAssayData(scData.singlet, layer = "counts"),
      species,
      param$tissue,
      BPPARAM = BPPARAM
    )
    singler.results <- cellsLabelsWithSingleR(
      GetAssayData(scData.singlet, layer = "counts"),
      Idents(scData.singlet),
      species
    )
    for (r in names(singler.results)) {
      scData.singlet[[paste0(r, "_single")]] <- singler.results[[
        r
      ]]$single.fine$labels
      scData.singlet[[paste0(r, "_cluster")]] <- singler.results[[
        r
  ...
  metadata(sce.singlets)$param$name <- paste(
    metadata(sce.singlets)$param$name,
  ...
  makeRmdReport(
    dataFiles = dataFiles,
    rmdFile = "SCFeatBarcoding.Rmd",
    reportTitle = metadata(sce.singlets)$param$name
  )
