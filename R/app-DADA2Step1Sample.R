...
ezMethodDADA2Step1Sample = function(
  input = NA,
  output = NA,
  param = NA,
  ...
  if (isPaired) {
    ...
    DADA2mainSeqTabObj <- DADA2CreateSeqTab(
  ...
  fStep <- list()
  countFilt1 <- DADA2mainSeqTabObj$out[,
    dimnames(DADA2mainSeqTabObj$out)[[2]] == "reads.in"
  ]
  countFilt2 <- DADA2mainSeqTabObj$out[,
    dimnames(DADA2mainSeqTabObj$out)[[2]] == "reads.out"
  ]
  fStep[[1]] <- data.frame(
    Freq = countFilt1,
    sample = sampleNames,
  ...
  fStep[[3]] <- data.frame(
    Freq = sapply(DADA2mainSeqTabObj$dadaObj, getN),
    sample = sampleNames,
    fStep = filterSteps[3]
  )
  ...
  countOTUs <- data.frame(
    DADA2mainSeqTabObj$fullTableOfOTUsNoChimObj,
    stringsAsFactors = F
  )
