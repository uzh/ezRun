...
ezMethodMothurStep1SampleReport = function(
  input = NA,
  output = NA,
  param = NA,
  ...
  dataset = input$meta
  relevantColumns <- gsub(
    " \\[File\\]",
    "",
    grep("File", colnames(dataset), value = T)
  )
  ...
  copyLoopOverFiles <- function(x) {
    lapply(x, function(x) {
      ezSystem(paste("cp", file.path(DEMO_DATA_ROOT, x), "./"))
    })
  }
  ...
  plotFromMothurSumm <- function(x) {
    ...
    DFforPlot <- data.frame(
      percentile = xAxis,
      length = tableDF$nbases,
      ambigs = tableDF$ambigs,
      homopol = tableDF$polymer,
      numSeqs = as.factor(tableDF$numSeqs),
      sample = colWithSampleNames,
      stringsAsFactors = F
    )
    caz <- melt(DFforPlot)
    plot <- ggplot(
      caz,
      aes(x = reorder(numSeqs, value), y = value, color = variable)
    ) +
      geom_point() +
    ...
    finalPlot <- plot +
      scale_x_discrete(
        breaks = quantile(
          as.numeric(as.character(caz$numSeqs)),
          probs = seq(0, 1, 0.05),
          type = 1
        )
      )
  ...
  summaryTablesToReport <- grep(
    "stepConvergence",
    summaryTablesToReport,
    invert = T,
    value = T
  )
  ...
  rmarkdown::render(
    input = "MothurStep1SampleReport.Rmd",
    envir = new.env(),
    output_dir = ".",
