...
countDensGGPlot <- function(
  cts,
  xmin = min(cts, na.rm = TRUE) - 5,
  xmax = max(cts, na.rm = TRUE) + 5,
  alpha = 0.3,
  colors,
  main = "all transcripts"
) {
  cts[cts < 0] = 0
  cts = log2(cts)
  data = data.frame(
    signal = unlist(cts),
    sampleName = rep(colnames(cts), each = nrow(cts)),
    stringsAsFactors = F
  )
  xvar = 'signal'
  split = 'sampleName'
  ...
  p = p +
    ylab("density of transcripts") +
    xlab("log2 expression") +
    ggtitle(paste(main, "(", nrow(cts), ")"))
  p = p +
    theme_bw() +
    theme(
      plot.background = element_blank(), #Remove default GRID
  ...
  p = p +
    theme(
      axis.line.x = element_line(color = "black"), #Draw AxisLines
...
ezMdsPlot = function(signal, sampleColors, main) {
  ...
  plot(
    ...
    main = main,
    xlim = c(1.2 * min(mds$x), 1.2 * max(mds$x)),
    ylim = c(1.2 * min(mds$y), 1.2 * max(mds$y)),
    col = sampleColors
  )
  text(
    mds$x,
...
ezMdsGG2 <- function(
  ...
  if (ncol(design) > 1L) {
    p <- ggplot(toPlot, aes(`Leading logFC dim1`, `Leading logFC dim2`)) +
      geom_point(
        aes(colour = colnames(design)[1], shape = colnames(design)[2]),
        size = 3
      ) +
      ggtitle(main) +
      theme_half_open() +
    ...
    p <- ggplot(toPlot, aes(`Leading logFC dim1`, `Leading logFC dim2`)) +
      geom_point(aes(colour = colnames(design)[1]), size = 3) +
      ggtitle(main) +
      theme_half_open() +
      background_grid()
    if (isTRUE(addLabels)) {
...
fragLengthReadMidDensityPlot = function(
  fl,
  file = NULL,
  xlim = c(-500, 500),
  bw = 5
  ...
  plot(
    ...
    main = paste(
      sm,
      "shift:",
      signif(fl$fragSizeMean, digits = 4),
      "+-",
  ...
  legend(
    "bottomright",
    c("Forward Reads", "Reverse Reads", "Shifted Reverse Reads"),
