...
countQcScatterPlots = function(
  ...
  if (!is.null(rowData(rawData)$gc)) {
    gcTypes = data.frame(
      "GC < 0.4" = as.numeric(rowData(rawData)$gc) < 0.4,
  ...
  if (!is.null(rowData(rawData)$featWidth)) {
    widthTypes = data.frame(
      "width < 500nt" = as.numeric(rowData(rawData)$featWidth) < 500,
  ...
  narrowPlots <- list()
  nPlots <- c()
  for (factorName in head(colnames(design), 4)) {
    ## take the first 4 factors
    for (factorLevel in unique(design[, factorName])) {
      ...
      narrowPlots[[plotName]][["std"]] <-
        ezScatter(
          y = signal[, idx],
          isPresent = isPresent[, idx],
          types = types,
          lim = signalRange,
          xlab = paste("Avg of", factorLevel),
          ylab = NULL,
          mode = "ggplot2"
        )
      if (!is.null(gcTypes)) {
...
makeTestScatterData <- function(param, se, types = NULL) {
  ...
  if (!is.null(param$pValueHighlightThresh)) {
    significants = rowData(se)$pValue <= param$pValueHighlightThresh &
    ...
    if (!is.null(param$log2RatioHighlightThresh)) {
      if (!is.null(rowData(se)$log2Ratio)) {
        ...
        types$Significants = types$Significants &
          rowData(se)$log2Effect >= param$log2RatioHighlightThresh
      }
    }
  }
