...
ezMethodVisiumQC <- function(input = NA, output = NA, param = NA,
    ...
    if(param$visiumType == 'SD'){
        ...
        for (j in 1:nrow(dataset)){
            ...
            if (file.exists(file.path(samplePath, "spatial", "tissue_positions.csv"))) {
                ...
                tissueFile <- file.path(samplePath, "spatial", "tissue_positions_list.csv")
            }
            if (file.exists(file.path(samplePath, "spatial", "tissue_hires_image.png"))) {
                imageFile <- file.path(samplePath, 
            ...
            metadata(slide_obj)$slide$tissue <- as.numeric(as.character(metadata(slide_obj)$slide$tissue))
            
            myPlots[[sampleName]] <- list()
            myPlots[[sampleName]][['Overview']] <- visualizeSlide(slide_obj, title = paste(sampleName, "overview"))
            myPlots[[sampleName]][['Tissue']] <- visualizeHeatmap(slide_obj, "tissue", title = paste(sampleName, "tissue"))
            myPlots[[sampleName]][['Signal']] <- visualizeHeatmap(slide_obj, "total_counts", title = paste(sampleName, "total_counts"))
        }
