...
kable(finalListOfSummaryTables["PreClusteredAndChimeraSummary"]$PreClusteredAndChimeraSummary$mergedTable, escape=TRUE, row.names=FALSE, format="html") %>%
  ...
  scroll_box(width = "100%", height = "600px")
grid.arrange(arrangeGrob(chimeraPlotsToReport), nrow=length(chimeraPlotsToReport))
```

### OTUs convergence summary
```{r stepConvergence, echo=FALSE}
kable(finalListOfSummaryTables["stepConvergenceSummary"]$stepConvergenceSummary$mergedTable, escape=TRUE, row.names=FALSE, format="html") %>%
  kable_styling(bootstrap_options = c("striped", "bordered"),
                full_width = T) %>%
  group_rows(index = finalListOfSummaryTables["stepConvergenceSummary"]$stepConvergenceSummary$aboveHeader) %>%
  scroll_box(width = "100%", height = "600px")
```

### OTUs saturation tables and plots
```{r otuSaturPlot, echo=FALSE}
kable(finalListOfSummaryTables["OTUsCountTable"]$OTUsCountTable$mergedTable, escape=TRUE, row.names=FALSE, format="html") %>%
  kable_styling(bootstrap_options = c("striped", "bordered"),
                full_width = T) %>%
  group_rows(index = finalListOfSummaryTables["OTUsCountTable"]$OTUsCountTable$aboveHeader) %>%
  scroll_box(width = "100%", height = "600px")
