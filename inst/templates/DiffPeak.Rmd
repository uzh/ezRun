...
DESeq2 differential peak analysis performed across all possible pairwise comparisons. Peak count were normalized using a variance-stabilizing transformation (VST) to mitigate the dependency of the variance on the mean expression level.
...
* a volcano plot with the top 10 up- and down-regulated genes associated with the peaks. Differentially expressed genes (DEGs) are called at an FDR (false discovery rate) of 5% (Benjamini-Hochberg adjusted p-value < 0.05) and effect size (absolute log2 fold change cutoff) of 1.
...
Based on top 500 DEGs for each comparison and filtered for known housekeeping genes reported in the [HRT atlas](https://housekeeping.unicamp.br/homePageGlobal.html).
...
enrichrTable <- tibble(Sample = names(topGenes),
