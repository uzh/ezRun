...
if(!is.na(plateMatrix)){
  ...
  for(plateName in names(plateMatrix)){
    for(colname in names(plateMatrix[[1]])){
      dataHeatMap <- log10(plateMatrix[[plateName]][[colname]])
      medianPlotVal <- median(dataHeatMap, na.rm = TRUE)
      p <- plot_ly(z=dataHeatMap, x=colnames(dataHeatMap),
                   y=rownames(dataHeatMap), type="heatmap",
                   #colors = colorRamp(c("black", "white")),
                   zmin=medianPlotVal-log10(2),#medianPlotVal/2, 
                   zmax=medianPlotVal+log10(2),#medianPlotVal*2,
                   hoverinfo="text", text=matrix(paste0("10^", format(dataHeatMap, digits=3), "=", 10^dataHeatMap), ncol=7),
                     #10^dataHeatMap,#paste0("10^", dataHeatMap, "=", 10^dataHeatMap),
                   width = 500*(1 + sqrt(5))/2, height = 500) %>%
        layout(xaxis=list(autotick = FALSE, dtick=1),
