...
### Heatmaps of reads/concentration perl cell on the plate
Sequenced reads or concentration per cell, on the plate layout.

```{r plate layout, echo=FALSE}
...
if(!is.na(plateMatrix)){
  for(plateName in names(plateMatrix)){
    for(colname in names(plateMatrix[[1]])){
      dataHeatMap <- plateMatrix[[plateName]][[colname]]
      plot_ly(z=dataHeatMap, x=colnames(dataHeatMap),
