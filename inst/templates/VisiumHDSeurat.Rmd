...
scData_unfiltered <- ezLoadRobj("scData_unfiltered")
...
```{r upset_plot, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
## Select columns with at least one flag for UpSet display
...
if (ncol(qcFrame) > 1) {
  ...
  cat(paste0("**", n_flagged, " bins/cells** removed by: ", colnames(qcFrame)))
} else {
...
for (eachCluster in levels(posMarkers$cluster)) {
  ...
  # Select top 20% of median nFeatures per cluster, clamped between 50 and 500
  nFeatures_cluster <- median(scData@meta.data[[nFeatureCol]][scData@meta.data$seurat_clusters == eachCluster], na.rm = TRUE)
  n_markers <- nrow(markersPerCluster)
  n_to_use <- max(50, min(500, round(nFeatures_cluster * 0.20)))
  n_to_use <- min(n_to_use, n_markers)
  markersPerCluster <- head(markersPerCluster, n_to_use)
