...
```{r conserved markers plots, fig.width=8, fig.height=length(levels(sce$seurat_clusters))*0.8}
...
top5 <- conservedMarkers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_fc)
...
most_diffGenes <- unique(diffGenes$gene)[1:50]
#in case the unique genes are less than 50 avoids NAs
most_diffGenes <- most_diffGenes[!is.na(most_diffGenes)]
plotHeatmap(sce, order_columns_by = "Condition", features = most_diffGenes, fontsize=8)
```


```{r diff genes plots, fig.height=30, fig.width=16}
