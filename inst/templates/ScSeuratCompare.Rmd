...
dittoSeq::dittoBarPlot(scData, group.by = scData$Sample, var = param$CellIdentity)
...
# Get both conditions
all_conditions <- sccomp_result$count_data[[1]] |>
  dplyr::pull(Condition) |>
  unique()

# The one that's not in the comparison name is the reference
comparison_group <- gsub("Condition", "", comparison)  # Gets "Wildlings"
reference_group <- all_conditions[!all_conditions %in% comparison_group]

# Create clear title
plot_title <- paste0(comparison_group, " vs ", reference_group)
