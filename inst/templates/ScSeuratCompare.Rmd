...
# Load scData with error handling
tryCatch({
  scData <- ezLoadRobj("scData", nthreads = param$cores)
}, error = function(e) {
  message("Could not load scData: ", e$message)
})

# Load sccomp results
if (ezRobjExists("sccomp_results")) {
  sccomp_res <- ezLoadRobj("sccomp_results")
}
```
...
# AggregateExpression mangles names: replaces _ with - and prepends 'g' if
# the name starts with a digit. Build the transformed names for joining.
sample_meta$AggName <- gsub("_", "-", sample_meta$SampleID)
starts_digit <- grepl("^[0-9]", sample_meta$AggName)
sample_meta$AggName[starts_digit] <- paste0("g", sample_meta$AggName[starts_digit])
...
# Join using AggName (AggregateExpression-transformed names)
pca_df <- dplyr::left_join(
  pca_res$pca_df, sample_meta,
  by = c("SampleID" = "AggName")
)
# Use original sample names for labels
pca_df$Label <- pca_df$SampleID.y
pca_df$Label[is.na(pca_df$Label)] <- pca_df$SampleID[is.na(pca_df$Label)]
...
for (cl in clusterNames) {
  ...
  tryCatch({
    ...
    pca_df_cl <- dplyr::left_join(
      pca_cl$pca_df, sample_meta,
      by = c("SampleID" = "AggName")
    )
    pca_df_cl$Label <- pca_df_cl$SampleID.y
    pca_df_cl$Label[is.na(pca_df_cl$Label)] <- pca_df_cl$SampleID[is.na(pca_df_cl$Label)]
...
tryCatch({
  ezDoHeatmap(scData, features=top5$gene)
}, error = function(e) {
...
for (clusterName in clusterNames) {
  ...
  if (nrow(dt) > 0) {
    ...
    print(ezDoHeatmap(subset(scd, downsample = 2000), genesToPlot, slot="scale.data") + ggtitle(paste0("Cell type: ", clusterName)))
  }
...
if(exists('sccomp_res')){
  ...
  # Fix: sccomp_boxplot() internally calls sccomp_replicate() which uses
  # sccomp_stan_models_cache_dir (defaults to ~/.sccomp_models) as cache.
  # SUSHI jobs run as trxcopy whose home may not have cached compiled models.
  # Redirect to the package stan/ dir where pre-compiled .rds caches exist.
  assignInNamespace("sccomp_stan_models_cache_dir",
                    system.file("stan", package = "sccomp"),
                    ns = "sccomp")
