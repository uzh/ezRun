...
```{r, eval=length(unique(sce$Condition))>1}
top5 <- conservedMarkers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_avg_fc)
var_heigth <- nrow(top5)*0.6
```


```{r consMarkers plots, eval=length(unique(sce$Condition))>1, results='asis', fig.width=8, fig.height=var_heigth}
...
cat(paste0("After identifying common cell types across conditions, we can look for genes that change in different conditions for cells of the same type. The comparison made was ", paste(unique(sce$Condition), collapse = " vs ")))
