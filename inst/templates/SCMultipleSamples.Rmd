...
# data10X <- Read10X(paste0(metadata(sce)$output$getFullPaths('ResultDir'), "/filtered_feature_bc_matrix"))
# data10X_GeneExp <- data10X$`Gene Expression`
# cellCycleFn <- list.files(path=paste0(metadata(sce)$output$getFullPaths('ResultDir'), "/filtered_feature_bc_matrix"),
...
plotReducedDim(sce, dimred="UMAP_NOCORRECTED", colour_by="CellCycle", label_format = c("UMAP_1","UMAP_2"))
```
...
plotReducedDim(sce, dimred =  "UMAP_NOCORRECTED", colour_by = "Batch", label_format = c("UMAP_1","UMAP_2"))
...
```{r UMAP and barplot Condition, eval=length(unique(sce$Batch)) > 2 ,fig.width=8, fig.height=5}
#if there are only two samples these plots are the same as the previous ones
plotReducedDim(sce, dimred =  "UMAP_NOCORRECTED", colour_by = "Condition", label_format = c("UMAP_1","UMAP_2"))
...
```{r UMAP per Condition after correction, eval=length(unique(sce$Batch)) > 2}
...
```{r heatmap, fig.width=8, fig.height=length(levels(sce$seurat_clusters))*0.8}
...
```{r consMarkers table, eval=length(unique(sce$Condition))>1, results='asis'}
...
```{r consMarkers plots, eval=length(unique(sce$Condition))>1, results='asis', fig.width=8, fig.height=length(levels(sce$seurat_clusters))*0.8}
...
cat("\n")
cat(paste0("After identifying common cell types across conditions, we can look for genes that change in different conditions for cells of the same type. The comparison made was ", unique(sce$Condition), collapse = " vs "))
cat("\n")
...
```{r diffGenes heatmap, eval=length(unique(sce$Condition))>1, results='asis', fig.height=6, fig.width=12, fig.align='center'}
...
```{r diffGenes violin plots, eval=length(unique(sce$Condition))>1, fig.height=30, fig.width=16}
