...
```{r gene filtering}
cat("genes removed:", sum(!rowData(sce.unfiltered)$is.expressed))
cat("genes kept:", sum(rowData(sce.unfiltered)$is.expressed))
```
...
```{r}
plotReducedDim(sce, dimred="TSNE", colour_by="seurat_clusters", text_by="seurat_clusters", 
               text_size=5, add_legend=FALSE)
plotReducedDim(sce, dimred="UMAP", colour_by="seurat_clusters", text_by="seurat_clusters", 
               text_size=5, add_legend=FALSE)
```
...
cells_prop = cellsProportion(sce, groupVar1 = "ident", groupVar2 = "Plate")
...
```{r uninteresting variation, fig.width=12, fig.height=12}
if (!is.null(colData(sce)$CellCycle)){
  plotColData(sce, x="ident", y="CellCycle", colour_by="ident") + ggtitle("Cell cycle vs ident")
}
sce$logSum = log10(sce$sum)
...
gridExtra::grid.arrange(
  ...
  plotReducedDim(sce, dimred="UMAP", colour_by="logSum", text_by="seurat_clusters", text_size=5, add_legend=FALSE),
  plotColData(sce, x="ident", y="detected", colour_by="ident", add_legend=FALSE) + 
    ggtitle("Detected genes vs ident"),
  plotReducedDim(sce, dimred="UMAP", colour_by="logDetected", text_by="seurat_clusters", 
                 text_size=5, add_legend=FALSE),
  plotColData(sce, x="ident", y="subsets_Mito_percent", colour_by="ident", add_legend=FALSE) + 
    ggtitle("Mito percentage vs ident"),
  plotReducedDim(sce, dimred="UMAP", colour_by="subsets_Mito_percent", text_by="seurat_clusters", 
                 text_size=5, add_legend=FALSE),
  ncol=2)
if (!identical(param$controlSeqs, character(0))) {
...
genesToPlot <- c(gsub("_", "-", param$controlSeqs), unique(top5$gene))
genesToPlot <- intersect(genesToPlot, rownames(sce))
...
pheatmap(geneMeans[genesToPlot, ],fontsize_row = 6)

plotDots(sce, features=genesToPlot, group="seurat_clusters")
```
