...
```{r low quality cells}
num.qc.lib <- sum(sce.unfiltered$qc.total_counts)
num.qc.nexprs <- sum(sce.unfiltered$qc.total_features)
removed <- sum(sce.unfiltered$discard)

kable(tibble("QC metric" =c("Library Size", "Expressed genes", "Total removed", "Cells remaining"),
             "Number of cells"= c(num.qc.lib, num.qc.nexprs, removed, ncol(sce))),
      row.names=FALSE,
...
```{r wilcoxon test, eval=is.null(param$SCT.regress), results='asis'}
...
```{r LR test, eval=!is.null(param$SCT.regress), results='asis'}
