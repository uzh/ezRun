...
variable <- rowData(sce)[rowData(sce)$vst.variable == TRUE,]
top10 <- rownames(variable[order(variable$vst.variance.standardized, decreasing=TRUE),][1:10,])
...
plotTSNE(sce, colour_by="detected") + ggtitle("Genes detected")
plotTSNE(sce, colour_by="sum") + ggtitle("Library size")
plotTSNE(sce, colour_by="subsets_Mito_percent") + ggtitle("Michondrial percentage")

if(!is.null(colData(sce)$CellCycle) && 
   !any(is.na(colData(sce)$CellCycle))){
...
```{r singler ScoreHeatmap,  eval= (param$species != "other"), fig.width=8, results='asis'}
...
cat("\n")
p1 <- plotScoreHeatmap(singler.results.single)
p1
p2 <- plotScoreHeatmap(singler.results.cluster, clusters=rownames(singler.results.cluster))
p2
```

```{r singler labelsTSNE,  eval= (param$species != "other"), fig.width=12, results='asis'}
cat("The single cell (left) and cluster (right) annotations are also shown on the TSNEs. Place the mouse over the cells to get information such as their TSNE coordinates, original cluster, the cells name and the label assigned by SingleR. You can also zoom in specific areas of the TSNE by dragging and drop with the mouse.\n")
