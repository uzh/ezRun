...
# Load Seurat object
seuratObj <- ezLoadRobj("seurat_object", nthreads = param$cores)

# Load SCEVAN results
resultsList <- ezLoadRobj("scevan_results_list", nthreads = param$cores)

# Load multi-sample results if available
if (ezRobjExists("scevan_multi_sample_results")) {
  multiSampleResults <- ezLoadRobj("scevan_multi_sample_results", nthreads = param$cores)
} else {
