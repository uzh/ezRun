...
var_height = shrinkToRange(length(unique(singler.single.labels))*0.5, c(6, 20))
```

```{r singler ScoreHeatmap,  eval= !is.null(singler.results), fig.height=var_height, fig.width=10, results='asis'}
...
cat("\n")
plotScoreHeatmap(singler.results.single) #, treeheight_row=0) ... maybe treeheight_row=0 caused heatmaps not showing up
plotScoreHeatmap(singler.results.cluster, show_colnames = TRUE) #,  treeheight_row=0)
```

again my code

```{r , fig.height=16, fig.width=8}
str(singler.results.single)
plot(1:10)
p <- plotScoreHeatmap(singler.results.single) #, treeheight_row=0) ... maybe treeheight_row=0 caused heatmaps not showing up
ggsave("singler-per-cell-score-heatmap.pdf", plot=p)
print(p)
p
p<- plotScoreHeatmap(singler.results.cluster, show_colnames = TRUE) #,  treeheight_row=0)
ggsave("singler-per-cluster-score-heatmap.pdf", plot=p)
print(p)
p
plot(10:1)
```
...
```{r , fig.height=16, fig.width=8, results='as.is'}
