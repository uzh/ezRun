...
allCellsMeta$qc.doublet <- allCellsMeta$doubletClass %in% "doublet"
## this is an empty hull having only the meta data
...
```{r QC plots, fig.height=4, fig.width=16}
VlnPlot(scData.unfiltered, features=c("nCount_RNA", "nFeature_RNA", "percent_mito", "percent_riboprot"), group.by="Sample", ncol = 4)
```
...
FeatureScatter(scData.unfiltered, feature1="nCount_RNA", feature2="percent_mito", group.by="discard")
```
...
UpSetR::upset(fromList(qcList))

kable(tibble("QC metric" =c("Library Size", "Expressed genes", "Mitochondrial genes", "Ribosomal protein genes", "Total removed", "Cells remaining"),
             "Number of cells"= c(sum(allCellsMeta$qc.lib), sum(allCellsMeta$qc.nexprs), sum(allCellsMeta$qc.mito), sum(allCellsMeta$qc.riboprot), sum(!allCellsMeta$useCell), ncol(scData))),
      row.names=FALSE,
...
VlnPlot(scData, features=c("nCount_RNA", "nFeature_RNA", "percent_mito", "percent_ribo"), group.by="Sample", ncol = 4)
```

```{r QC plot mito after filtering, fig.align='center'}
FeatureScatter(scData, feature1="nCount_RNA", feature2="percent_mito")
```
...
VlnPlot(scData, features ="nCount_RNA") + ggtitle("Number of UMIs vs ident") + NoLegend()
...
VlnPlot(scData, features ="percent_ribo") + ggtitle("Ribosomal percentage vs ident") + NoLegend()
FeaturePlot(scData, features="percent_mito", label = TRUE)
