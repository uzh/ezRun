...
Number of bins filtered by the individual quality criterions.
...
ggplot(plot_df, aes(x = seurat_clusters, 
  ...
  theme(legend.position="none", 
        ...
        axis.title.y=element_text("# bins"),
...
df <- df |> as.data.frame(check.names = FALSE, stringsAsFactors = FALSE) |> rename("seurat_clusters"="Cluster")
...
df <- as.data.frame(posMarkers)
display_cols <- intersect(c("pct.1","pct.2","avg_log2FC","p_val","p_val_adj","diff_pct"),
                          names(df))
...
datatable(
  ...
  formatSignif(display_cols, digits = 3) |>
...
```{r dotplot, fig.width  = 15, fig.height = `r genesToPlot`}
