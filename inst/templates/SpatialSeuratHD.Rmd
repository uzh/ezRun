...
lambda <- 0.8 #Influence of the neighborhood. Larger values yield more spatially coherent domains
...
scData.unfiltered_spatial <- ezLoadRobj("scData.unfiltered")
...
filterStats <- list(
  "Total removed (thresholding)"= sum(!allCellsMeta$useCell),
  "Total removed (manual)" = sum(allCellsMeta$useCell) - ncol(scData),
  "Bins remaining" = sum(ncol(scData))
)

ezListAsKable(filterStats, col.names=c("", "Number of Bins"))
```
:::
:::
...
stopifnot(param$npcs <= length(stdev))
...
plot_df$used <- factor(plot_df$rank <= param$npcs, levels=c(TRUE, FALSE))
...
ggplot(plot_df, aes(x = cumu, 
  ...
  geom_vline(xintercept = cumu[param$npcs], color = "grey50", linetype = "dashed") +
  

  labs(
    title    = "Percent variance explained by the principal components",
    subtitle = paste0("number of PCs used: ", param$npcs),
  ...
  annotate(
    "text",
    x = cumu[param$npcs], y = max(plot_df$pct),
    label = paste0("Variance used: ", sprintf("%.2f", cumu[param$npcs]), "%"),
...
ezInteractiveTableRmd(df, rowNames = FALSE, title="number of bins", filter="none")|>
...
scData <- RunBanksy(scData,
                    ...
                    slot = "data", 
                    features = "variable",
                    k_geom = 50 #Local neighborhood size. Larger values will yield larger domains
                    )
...
##### Communities of spatially adjacent spots
...
banksy_niches <- tibble(
  ...
  add_row(
    ...
    Definition = "Community of spatially adjacent spots with neighborhood-smoothed expression.",
    `Key params` = "k_geom, λ",
    `Practical tip` = "Represents a tissue microenvironment."
  ) |>
  add_row(
    Concept = "k_geom (local neighborhood size)",
    Definition = "How many nearest spatial neighbors define a spot’s niche.",
    `Key params` = "Integer (e.g., 6–12 for HD bins)",
    `Practical tip` = "↑k = smoother/merge risk; ↓k = sharper/border-preserving."
  ) |>
  add_row(
    Concept = "λ (neighbor influence)",
    Definition = "How strongly neighbors affect each spot’s expression.",
...
```{r number of cells in each cluster banksy, fig.width=18, fig.height=6}
if(!('Batch' %in% colnames(scData@meta.data))){
    scData@meta.data$Batch <- scData@meta.data$Sample
}

cellIdents_perSample <- scData@meta.data[,c('banksy_cluster', 'Batch')]
cellIdents_perSample$Batch <- factor(cellIdents_perSample$Batch)
...
df <- cellsProportion(scData, groupVar1 = "banksy_cluster", groupVar2 = "Batch") 
df <- df |> as.data.frame(check.names = FALSE, stringsAsFactors = FALSE) |>
...
boxplot_data_nCount <- FetchData(scData, vars = c(nCountCol, "banksy_cluster"))
#boxplot_data_nCount$banksy_cluster <- factor(boxplot_data_nCount$banksy_cluster, levels = sort(as.numeric(levels(boxplot_data_nCount$banksy_cluster))))
plot1 <- ggplot(boxplot_data_nCount, aes(x = banksy_cluster, y = .data[[nCountCol]], fill = banksy_cluster))
plot1 <- plot1 + geom_boxplot(outlier.size = 0.5, alpha = 0.9)
plot1 <- plot1 + scale_fill_viridis_d(option = "turbo")  
plot1 <- plot1 + ggtitle("Number of UMIs vs cluster") + xlab("Cluster") + ylab(nCountCol) + theme_minimal(base_size = 14) + theme(legend.position = "none")

boxplot_data_nFeature <- FetchData(scData, vars = c(nFeatureCol, "banksy_cluster"))
#boxplot_data_nFeature$banksy_cluster <- factor(boxplot_data_nFeature$banksy_cluster, levels = sort(as.numeric(levels(boxplot_data_nFeature$banksy_cluster))))
plot2 <- ggplot(boxplot_data_nFeature, aes(x = banksy_cluster, y = .data[[nFeatureCol]], fill = banksy_cluster))
plot2 <- plot2 + geom_boxplot(outlier.size = 0.5, alpha = 0.9)
plot2 <- plot2 + scale_fill_viridis_d(option = "turbo")  
plot2 <- plot2 + ggtitle("Number of genes vs cluster") + xlab("Cluster") + ylab(nFeatureCol) + theme_minimal(base_size = 14) + theme(legend.position = "none")

boxplot_data_mito <- FetchData(scData, vars = c("percent_mito", "banksy_cluster"))
#boxplot_data_mito$banksy_cluster <- factor(boxplot_data_mito$banksy_cluster, levels = sort(as.numeric(levels(boxplot_data_mito$banksy_cluster))))
plot3 <- ggplot(boxplot_data_mito, aes(x = banksy_cluster, y = percent_mito, fill = banksy_cluster))
plot3 <- plot3 + geom_boxplot(outlier.size = 0.5, alpha = 0.9)
plot3 <- plot3 + scale_fill_viridis_d(option = "turbo")  
plot3 <- plot3 + ggtitle("Mitochondrial percentage vs cluster") + xlab("Cluster") + ylab("percent_mito") + theme_minimal(base_size = 14) + theme(legend.position = "none")
plot1 + plot2 + plot3 + plot_layout(nrow = 3, ncol = 1)
...
DotPlot(scData, features=rev(genesToPlot)) + 
  coord_flip() +
  scale_y_discrete(limits = levels(Idents(scData))) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
```
