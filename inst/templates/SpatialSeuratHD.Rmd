...
nCountCol <- colnames(scData[[]])[grep(paste0('^nCount_Spatial.*'), colnames(scData[[]]))]
nFeatureCol <- colnames(scData[[]])[grep(paste0('^nFeature_Spatial.*'), colnames(scData[[]]))]
...
plot1 <- plotColData(
  xxAll,
  x = "Sample",
  y = "nCount_Spatial.016um",
  colour_by = "discard"
) +
  scale_y_log10() +
  labs(
    title = "UMI counts",
    y = "UMI counts per spot (0.16 µm bins)"
  ) +
...
.pretty_title <- function(f) switch(
  f,
  "nCount_Spatial.016um"    = "UMI per spot (0.16 µm bins)",
  "nFeature_Spatial.016um"  = "Detected genes",
...
.fill_label <- function(f) switch(
  f,
  "nCount_Spatial.016um"    = "UMI counts",
  "nFeature_Spatial.016um"  = "Genes",
  "percent_mito"            = "Mito %",
...
sp_plot <- function(obj, feature, pt.size.factor = 2,
...
# Redone plots
plot5 <- sp_plot(scData, "nCount_Spatial.016um")
plot6 <- sp_plot(scData, "nFeature_Spatial.016um")
plot7 <- sp_plot(scData, "percent_mito")
...
boxplot_data_nCount <- FetchData(scData, vars = c("nCount_Spatial", "seurat_clusters"))
...
plot1 <- ggplot(boxplot_data_nCount, aes(x = seurat_clusters, y = nCount_Spatial, fill = seurat_clusters))
plot1 <- plot1 + geom_boxplot(outlier.size = 0.5, alpha = 0.9)
plot1 <- plot1 + scale_fill_viridis_d(option = "turbo")  
plot1 <- plot1 + ggtitle("Number of UMIs vs cluster") + xlab("Cluster") + ylab("nCount_Spatial") + theme_minimal(base_size = 14) + theme(legend.position = "none")

boxplot_data_nFeature <- FetchData(scData, vars = c("nFeature_Spatial", "seurat_clusters"))
...
plot2 <- ggplot(boxplot_data_nFeature, aes(x = seurat_clusters, y = nFeature_Spatial, fill = seurat_clusters))
plot2 <- plot2 + geom_boxplot(outlier.size = 0.5, alpha = 0.9)
plot2 <- plot2 + scale_fill_viridis_d(option = "turbo")  
plot2 <- plot2 + ggtitle("Number of genes vs cluster") + xlab("Cluster") + ylab("nFeature_Spatial") + theme_minimal(base_size = 14) + theme(legend.position = "none")
