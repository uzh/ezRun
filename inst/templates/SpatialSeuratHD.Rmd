...
DefaultAssay(banksyObj) <- "BANKSY"

banksyObj <- RunPCA(banksyObj, 
                 assay = "BANKSY", 
                 reduction.name = "pca.banksy", 
                 features = rownames(banksyObj), 
                 npcs = param$npcs)
...
banksyObj <- FindNeighbors(banksyObj, 
...
banksyObj <- FindClusters(banksyObj, 
...
#### Analysis using "local neighborhood size" and "influence of the neighborhood"

`r paste("Resolution:", param$resolution)`

```{r banksy plot spatial, fig.width=6, fig.height=6}
Idents(banksyObj) <- "banksy_cluster"

n_clusters_banksy <- nlevels(Idents(banksyObj))
show_leg_banksy   <- n_clusters <= 20

p_spatialNiches <- SpatialDimPlot(banksyObj, 
...
banksy_cells <- CellsByIdentities(banksyObj)

p_niches <- SpatialDimPlot(banksyObj,
