...
```{r picard plot, echo=FALSE}
plateMatrix <- plateStatistics(cd, colname="PCT_RIBOSOMAL_BASES")
...
for(plateName in names(plateMatrix)){
  ...
  vals <- unique(scales::rescale(c(matrix2Plot)))
  o <- order(vals, decreasing = FALSE)
  cols <- scales::col_numeric("RdYlGn", domain = NULL)(vals)
  colz <- setNames(data.frame(vals[o], cols[o]),
                   NULL)

  heatmapPlate(matrix2Plot, log10 = FALSE, center = FALSE, title=plateName,
               colorscale=colz)
