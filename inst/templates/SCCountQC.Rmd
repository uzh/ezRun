...
cell_info_scater <- cell_info
cell_info_scater$perc_mito <- cell_info_scater$perc_mito/100
scData <- CreateSeuratObject(raw.data = ctsFilt, min.cells = 5,
                             project = param$name, meta.data=cell_info_scater)
if(param$scProtocol == "smart-Seq2"){
...
```{r VlnPlot, echo=FALSE, fig.width=10, fig.height=ifelse(nchar(scData@project.name) > 20, 10, 7), eval=doSeurat && !debug}
VlnPlot(object=scData, features.plot = c("nGene", "nUMI", "perc_mito"),
        nCol=3, 
        x.lab.rot=ifelse(nchar(scData@project.name) > 20, TRUE, FALSE))
```

```{r seurat filter and normalize, echo=FALSE, eval=doSeurat && !debug}
