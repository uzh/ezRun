...
l <- htmltools::tagList()
for(plateName in names(plateMatrix)){
  for(colname in setdiff(names(plateMatrix[[plateName]]), c("PF_READS", "nGenesDetected"))){
    matrix2Plot <- plateMatrix[[plateName]][[colname]]
    ...
    p1 <- heatmapPlate(matrix2Plot, log10 = FALSE, center = FALSE,
                       title=paste(plateName, colname, sep=": "), 
                       colors="RdYlGn")
    cd2 <- data.frame(PF_READS=c(plateMatrix[[plateName]]$PF_READS),
                      ...
                      metricOriginal=c(plateMatrix[[plateName]][[colname]])
                      )
    if(colname == "PF_READS_ALIGNED"){
      cd2 <- transform(cd2, nGenesDetected=nGenesDetected/PF_READS)
    }
    if(colname %in% c("PF_MISMATCH_RATE", "PCT_ADAPTER", "MEDIAN_5PRIME_BIAS",
                      "MEDIAN_3PRIME_BIAS", "PERCENT_DUPLICATION")){
      p2 <- plot_ly(data = cd2, x = ~PF_READS, y = ~nGenesDetected, 
                  color = ~metric, type="scatter", mode = 'markers', 
                  colors="RdYlGn") %>% 
              layout(xaxis=list(type="log"), 
                     yaxis=list(title="# of Genes Detected"))
    }else{
      p2 <- plot_ly(data = cd2, x = ~PF_READS, y = ~nGenesDetected, 
                  color = ~metric, type="scatter", mode = 'markers', 
                  colors="RdYlGn") %>%
    ...
    p3 <- plot_ly(data = cd2, x = ~PF_READS, y = ~metricOriginal,
                  color = ~metric, type="scatter", mode = 'markers',
                  colors="RdYlGn") %>%
            layout(xaxis=list(type="log"),
