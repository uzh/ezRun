...
```{r picard plot, echo=FALSE, fig.width=24, fig.height=8}
plateMatrix <- plateStatistics(cd, colname=c("PF_READS", "nGenesDetected",
                                             "PCT_RIBOSOMAL_BASES"))

plateName <- names(plateMatrix)[1]
#for(plateName in names(plateMatrix)){
  
  matrix2Plot <- plateMatrix[[plateName]]$PCT_RIBOSOMAL_BASES
  ...
  matrix2Plot[matrix2Plot > med_matrix2Plot+log10(2)] <- med_matrix2Plot+log10(2)
  matrix2Plot[matrix2Plot < med_matrix2Plot-log10(2)] <- med_matrix2Plot-log10(2)
  
  p1 <- heatmapPlate(matrix2Plot, log10 = FALSE, center = FALSE, title=plateName,
                     colors="RdYlGn")
  cd2 <- data.frame(PF_READS=c(plateMatrix[[plateName]]$PF_READS),
                    nGenesDetected=c(plateMatrix[[plateName]]$nGenesDetected),
                    metric=c(matrix2Plot),
                    metricOriginal=c(plateMatrix[[plateName]]$PCT_RIBOSOMAL_BASES))
  p2 <- plot_ly(data = cd2, x = ~PF_READS, y = ~nGenesDetected, color = ~metric,
                type="scatter", mode = 'markers', colors="RdYlGn"
                ) %>%
    layout(xaxis=list(type="log"),
           yaxis=list(type="log"))
  p3 <- plot_ly(data = cd2, x = ~PF_READS, y = ~metricOriginal, color = ~metric,
                type="scatter", mode = 'markers', colors="RdYlGn"
                ) %>%
    layout(xaxis=list(type="log"),
           yaxis=list(type="log"))
