...
# sce <- readRDS("/export/local/scratch/gtan/dev/p2497-SCCountQC/sce.rds")
...
```{r gene detection rates, fig.width=8, fig.height=8, echo=FALSE, message=FALSE, warning=FALSE, eval=param$scProtocol=="smart-Seq2"}
scePC = sce
rowData(scePC) = NULL
...
```{r picard setup, echo=FALSE, eval=param$scProtocol=="smart-Seq2"}
cd <- data.frame(colData(sceAllCells), check.names = FALSE)
cd$nGenesDetected <- Matrix::colSums(assays(sceAllCells)$counts > 3)
```

```{r picard plot, echo=FALSE, fig.width=20, fig.height=6, eval=param$scProtocol=="smart-Seq2"}
...
```{r metrics, echo=FALSE, eval=param$scProtocol=="smart-Seq2"}
...
The raw count matrix is available here:
