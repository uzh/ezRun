...
# sce <- readRDS("/srv/gstore/projects/p2886/SCCountQC_30041_2018-10-01--14-24-05/B9P9_NPCHP13_SCCountQC/sce.rds")

debug <- FALSE
param <- metadata(sce)$param
```

```{r scater, include=FALSE}
## ----------------------------------------------
## scater sce
...
mito <- rownames(sce[as.character(seqnames(rowRanges(sce))) %in% c("chrM", "MT")])
...
is.mito <- as.character(seqnames(rowRanges(sceFixName))) %in% c("chrM", "MT")
...
```{r gene detection rates, fig.width=8, fig.height=8, echo=FALSE, message=FALSE, warning=FALSE, eval=evalPicard && !debug}
scePC = sce
...
```{r seurat variable gene, echo=FALSE, eval=!debug}
scData <- FindVariableGenes(object=scData)
```

The number of variable genes: `r length(x = scData@var.genes)`.

```{r scale and PCA calculation, echo=FALSE, message=FALSE, results='hide', eval=!debug}
...
```{r plot PCA, echo=FALSE, fig.width=10, fig.height=15, eval=!debug}
VizPCA(object = scData, pcs.use = 1:6)
```

```{r PCHeatmap, echo=FALSE, fig.width=10, fig.height=30, eval=!debug}
PCHeatmap(object=scData, pc.use=1:20, cells.use=500, do.balanced=TRUE, 
    label.columns=FALSE, use.full=FALSE)
```

```{r JackStraw, echo=FALSE, fig.width=10, fig.height=25, eval=!debug}
...
```{r PCElbowPlot, echo=FALSE, eval=!debug}
