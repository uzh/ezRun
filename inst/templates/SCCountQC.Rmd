...
# sce <- readRDS("/srv/gstore/projects/p2497/SCCountQC_24763_2018-03-23--22-14-43/20171222.A-SiCSeq_SCs_P5_SCCountQC/sce.rds")
...
sceSeurat <- sceAllCells
rownames(sceSeurat) <- make.unique(rowData(sceSeurat)$gene_name)
min_genes=500 ## Input parameter
countsSeurat <- assays(sceSeurat)$counts
if(param$scProtocol == "smart-Seq2"){
  thr=50000 ## Input parameter
  countsSeurat <- countsSeurat[ ,Matrix::colSums(countsSeurat) > thr]
}
...
```{r JackStraw, echo=FALSE, fig.width=10, fig.height=15, warning=FALSE}
scData <- JackStraw(object=scData, num.replicate=100, display.progress=FALSE)
JackStrawPlot(object=scData, PCs = 1:12)
```
