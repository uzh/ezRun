...
SpatialPlot(scData.unfiltered_spatial, group.by = "discard") + 
  labs(title="Filtered bins/cells") +
  theme_void() +
  theme(legend.position = "top", # to be consistent with other diagnostic plots
...
filterStats <- list(
  ...
  "Bins/cells remaining" = sum(ncol(scData))
)
...
Number of bins/cells filtered by the individual quality criteria.
```{r low quality BinsCells, fig.align='center'}
...
if (length(qcList) > 1){
  UpSetR::upset(fromList(qcList), nsets = 5, mainbar.y.label="bins/cells")
}
```
:::
...
qcStats <- ezFrame("Number of bins/cells"=integer(0), 
...
if (identical(param$binSize, "segmented_outputs")) {
  ...
  cat("> **Note:** This tab is only for *segmented outputs* and isn’t applicable to bin-based results.\n")
}
...
scData <- FindClusters(scData,
                          ...
                          resolution = param$clusterResolution/4,
                          verbose = F)
```
...
`r paste("Resolution:", param$clusterResolution/4)`
