...
write.csv(x = resultMetaCore, file = metaCoreFile, sep = "\t")
```
...
#### How to use Metacore: quick start
 ...
 1. [Right Click and Save File to Download Expression File for MetaCore](`r metacoreFile`)
  ...
  for (onto in names(enrichResult)) {
    ...
    for (sig in names(enrichResult[[onto]])) {
      if (is.na(enrichResult[[onto]][[sig]][1][[1]])) {
        ...
        erp[["dotplot"]] <- enrichplot::dotplot(
...
if (exists("enrichResultGSEA")) {
  for (onto in names(enrichResultGSEA)) {
    ...
    if (is.na(enrichResultGSEA[[onto]][1][[1]])) {
      ...
      erp[["dotplot"]] <- enrichplot::dotplot(
        object = er, showCategory = 20, title = paste(onto)) + 
        scale_y_discrete(labels = er@result$Label) + 
        theme(axis.text.y = element_text(vjust = -0.01, size = 8))
