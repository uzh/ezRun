...
## This report requires objects: output, deResult
require(knitr)
require(kableExtra)
...
```{r cluster, echo=FALSE, eval=!debug, message=FALSE, fig.width=max(8,0.1*ncol(se)), fig.height=10, fig.retina=2, warning=FALSE}
...
```{r enricher compute, echo=FALSE, eval=TRUE, message=FALSE}
enrichResult <- ezEnricher(param, se)
```

#### dotplot
```{r dotplot gsea, echo=FALSE, eval=TRUE}
...
#### enrichMap
```{r enrichMap gsea, echo=FALSE, eval=TRUE}
for(onto in names(enrichResult)){
  for(sig in names(enrichResult[[1]])){
    if(nrow(enrichResult[[onto]][[sig]]@result) == 0L){
      next
    }else{
...
#### cnetplot
```{r cnetplot gsea, echo=FALSE, eval=TRUE}
for(onto in names(enrichResult)){
  for(sig in names(enrichResult[[1]])){
    if(nrow(enrichResult[[onto]][[sig]]@result) == 0L){
      next
...
```{r gsea compute, echo=FALSE, message=FALSE}
enrichResult <- ezGSEA(param, se)
```

#### dotplot
```{r dotplot, echo=FALSE, eval=TRUE}
for(onto in names(enrichResult)){
  if(nrow(enrichResult[[onto]]@result) == 0L){
      next
  }else{
    p <- dotplot(enrichResult[[onto]], showCategory=20, 
                 title=paste(onto, sig))
    print(p)
  }
}
...
#### enrichMap
```{r enrichMap, echo=FALSE, eval=TRUE}
for(onto in names(enrichResult)){
    if(nrow(enrichResult[[onto]]@result) == 0L){
      next
    }else{
      enrichMap(enrichResult[[onto]], n=50, main=paste(onto, sig))
    }
}
```

#### cnetplot
```{r cnetplot, echo=FALSE, eval=TRUE}
for(onto in names(enrichResult)){
    if(nrow(enrichResult[[onto]]@result) == 0L){
      next
    }else{
      cnetplot(enrichResult[[onto]],
             foldChange=setNames(rowData(se)$log2Ratio, rowData(se)$gene_name),
             main=paste(onto, sig))
    }
}
