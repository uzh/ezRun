...
if(param$featureLevel == 'gene'){
  resultLoaded <- tibble::rownames_to_column(resultLoaded, "gene_id")
  resultLoaded <- resultLoaded[resultLoaded$isPresent, ]
  resultLoaded <- resultLoaded[resultLoaded$pValue <= param$pValThreshGO,]
  resultLoaded <- dplyr::left_join(resultLoaded, egr, by = c("gene_name"="SYMBOL"))
  resultMetaCore <- dplyr::select(resultLoaded,c("gene_name", "gene_id", "entrez_id", "log2 Ratio", "pValue", "fdr"))
} else {
  resultLoaded <- tibble::rownames_to_column(resultLoaded, "transcript_id")
  resultLoaded <- resultLoaded[resultLoaded$isPresent, ]
  resultLoaded <- resultLoaded[resultLoaded$pValue <= param$pValThreshGO,]
  resultLoaded <- dplyr::left_join(resultLoaded, egr, by = c("gene_name"="SYMBOL"))
  resultMetaCore <- dplyr::select(resultLoaded,c("transcript_id", "gene_name", "gene_id", "entrez_id", "log2 Ratio", "pValue", "fdr"))
}
```
...
Using MetaCore requires an account. One can be obtained by emailing <sequencing@fgcz.ethz.ch>.
 ...
 1. [Click to Download Expression File for MetaCore](`r metacoreFile$resultMetaCoreZip`)
...
For these non-core species, use Entrez gene IDs. 
