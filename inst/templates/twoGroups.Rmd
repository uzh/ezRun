...
if(Sys.which("phantomjs") == ""){
  ## phantomjs has to be in the PATH
  Sys.setenv("PATH"=paste(PHANTOMJS_DIR, Sys.getenv("PATH"), sep=":"))
...
```{r scatter comparison, echo=FALSE, eval=TRUE, message=FALSE}
...
```{r volcano p-value, echo=FALSE, eval=TRUE, message=FALSE}
if (isTRUE(isTwoGroup)){
...
```{r scatter all pairs, echo=FALSE, results='asis', message=FALSE}
## All pairs
if (!isTRUE(isTwoGroup)){
  plotCmd = expression({
...
```{r cluster, echo=FALSE, eval=!debug, message=FALSE, fig.width=max(8,4+0.1*ncol(se)), fig.height=10, fig.retina=2, warning=FALSE}
...
```{r goseq interactive table, echo=FALSE, eval=!debug, message=FALSE}
...
```{r enricher compute, echo=FALSE, eval=!debug, message=FALSE}
...
#### enrichMap
```{r enrichMap gsea, echo=FALSE, eval=!debug, fig.width=14, fig.height=14}
for(onto in names(enrichResult)){
  for(sig in names(enrichResult[[1]])){
...
#### cnetplot
```{r cnetplot gsea, echo=FALSE, eval=!debug, fig.width=14, fig.height=14}
for(onto in names(enrichResult)){
  for(sig in names(enrichResult[[1]])){
    if(nrow(enrichResult[[onto]][[sig]]@result) == 0L){
      next
