...
if (isTRUE(isTwoGroup)) {
  ...
  if (!isTRUE(hasMassiveFeats)) {
    ...
    scatterHtml <- paste0(param$comparison, "-scatter.html")
     DT::saveWidget(as_widget(p_scatter), scatterHtml)
  }
}
```
...
if (isTRUE(isTwoGroup)) {
  ...
  if (isFALSE(hasMassiveFeats)) {
    ...
    volcanoHtml <- paste0(param$comparison, "-volcano.html")
     DT::saveWidget(as_widget(p_volcano), volcanoHtml)
  }
}
```
...
```{r prepare ora plots, echo=FALSE, message=FALSE, eval=!debug & isTRUE(param$runGO), warning=FALSE}
enrichResultPlots <- list()
if (isTRUE(param$runGO)) {
  for (onto in names(enrichResult)) {
    enrichResultPlots[[onto]] <- list()
    for (sig in names(enrichResult[[onto]])) {
      if (ezIsEmptyEnrichResult(enrichResult[[onto]][[sig]])) {
        next
      } else {
        erp <- list()
        ...
        geneLabels <- enrichInput$seqAnno[sigGeneIds, "gene_name"]
        ## fall back to the gene id when a gene has no symbol (e.g. some lncRNAs),
        ## otherwise cnetplot builds a data.frame with NA/empty row names
        naLabel <- is.na(geneLabels) | geneLabels == ""
        geneLabels[naLabel] <- sigGeneIds[naLabel]
        names(log2Ratio) <- geneLabels
        log2Ratio <- sort(log2Ratio, decreasing = T)
        ## cnetplot (enrichplot >= 1.32) errors out when the enrichResult holds no
        ## category passing its significance cutoff (0 items) -- keep the rest of
        ## the report instead of aborting it.
        erp[["cnet"]] <- tryCatch(
          enrichplot::cnetplot(x = er, foldChange = log2Ratio),
          error = function(e) {
            ezWrite("cnetplot skipped for ", onto, "/", sig, ": ", conditionMessage(e))
            NULL
          }
        )
...
```{r ora overview, echo=F, message=F, eval=!debug & isTRUE(param$runGO), results='asis',warning=FALSE}
  ...
  for (goDomain in names(enrichResult)){
    for (resName in names(enrichResult[[goDomain]])){
      ...
      er <- enrichResult[[goDomain]][[resName]]
      ora.overview[testName, "Number of Significant Terms"] =
        if (ezIsEmptyEnrichResult(er)) 0L else
          nrow(er@result[er@result$p.adjust <= param$fdrThreshORA, ])
    }
  }
...
```{r tabs with ora plots, results='asis', echo = FALSE, eval = isTRUE(param$runGO), fig.height=8, fig.width=8, fig.align = "left", message=FALSE, warning=FALSE}
  for (onto in names(enrichResult)) {
    cat("###", onto, "{.tabset}", "\n")
    for (sig in names(enrichResult[[onto]])) {
      cat("#####", sig, "{.tabset}", "\n")
      if (ezIsEmptyEnrichResult(enrichResult[[onto]][[sig]])) {
        ...
        print(erp$barplot)
        if (!is.null(erp$cnet)) print(erp$cnet)
        print(kable(erp$df,
...
```{r prepare gsea plots, echo=FALSE, message=FALSE, eval=!debug & isTRUE(param$runGO), warning=FALSE}
enrichResultGSEAPlots <- list()
  for (onto in names(enrichResultGSEA)) {
    enrichResultGSEAPlots[[onto]] <- list()
    if (ezIsEmptyEnrichResult(enrichResultGSEA[[onto]])) {
      next
    } else {
      erp <- list()
      ...
      names(log2Ratio) <- geneLabels
      log2Ratio <- sort(log2Ratio, decreasing = T)
      ## keep the report alive if cnetplot cannot build a graph (see ORA note above)
      erp[["cnet"]] <- tryCatch(
        enrichplot::cnetplot(x = er, foldChange = log2Ratio),
        error = function(e) {
          ezWrite("cnetplot skipped for GSEA ", onto, ": ", conditionMessage(e))
          NULL
        }
      )
...
```{r gsea overview, echo=F, message=F, eval=!debug & isTRUE(param$runGO), results = 'asis', warning=FALSE}
  ...
  for (i in 1:length(names(enrichResultGSEA))){
    ...
    er <- enrichResultGSEA[[i]]
    gsea.overview[testName, "Number of Significant Pathways"] =
      if (ezIsEmptyEnrichResult(er)) 0L else nrow(er@result)
  }
