...
```{r scatter comparison, echo=FALSE, eval=FALSE}
...
```{r volcano p-value, echo=FALSE, eval=FALSE}
if (isTRUE(isTwoGroup)){
...
```{r volcano FDR, echo=FALSE, warning=FALSE, eval=FALSE}
if(isTRUE(isTwoGroup)){
...
```{r pvalue hist, echo=FALSE, results='hide'}
plotCmd = expression({
...
pdfName = paste0(param$comparison, "-pValueHist.pdf")
pdf(file=pdfName, height=7, width=7*1.4)
eval(plotCmd)
dev.off()
...
[Plot in pdf](`r pdfName`)

```{r intra group, echo=FALSE}
