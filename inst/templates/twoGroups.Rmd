---
title: "`r paste('Two groups analysis:')`"
...
```{r prepare plots, echo=FALSE, message=FALSE, eval=!debug}
    ...
    for(sig in names(enrichResult[[onto]])){
      if(is.na(enrichResult[[onto]][[sig]][1][[1]])){
        ...
        for (i in 1:length(er@result$Description)) {
          ...
          if (nchar(desc) < 30) {
            next
          } else if (nchar(desc) > 30 & nchar(syn) <= 30 & !is.na(syn)) {
            er@result$Description[i] <- syn
          } else if (nchar(desc) > 30 & nchar(syn) > 30 | is.na(syn)) {
            er@result$Description[i] <- substr(er@result$Description[i], 1, 30)
          }
        }
        
        erp[["dotplot"]] <- dotplot(er, showCategory = 20,title=paste(onto, sig))
...
```{r tabs with enrichment plots, results='asis', echo = FALSE, fig.height=8, fig.width=8}
for (onto in names(enrichResult)) {
  cat("####", onto,"{.tabset}","\n")
  for (sig in names(enrichResult[[onto]])) {
      cat("#####", sig,"{.tabset}","\n")
  ...
  for(onto in names(enrichResultGSEA)){
    enrichResultGSEAPlots[[onto]] = list()
      if(is.na(enrichResultGSEA[[onto]][1][[1]])){
        ...
        for (i in 1:length(er@result$ID)) {
          syn <- GO.db::GOTERM[[er@result$ID[i] ]]
          syn <- syn@Term
          if (is.na(syn)) {
            next
          } else if (nchar(syn) < 30) {
            er@result$Description[i] <- syn
          } else if (nchar(syn) > 30) {
            er@result$Description[i] <- substr(syn, 1, 30)
          }
        }
        
        erp[["dotplot"]] <- dotplot(er, showCategory = 20, title=paste(onto))
        erp[["ridgeplot"]] <- ridgeplot(er)
...
for (onto in names(enrichResultGSEA)) {
  cat("####", onto,"{.tabset}","\n")
      if (is.na(enrichResultGSEA[[onto]][1][[1]])){
        ...
        print( erp$gseaplot )
        print( kable(erp$df, 
                     escape=FALSE, row.names=FALSE, format = "html") %>% 
