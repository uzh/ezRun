...
```{r cluster, echo=FALSE, eval=TRUE, message=FALSE, fig.width=max(8,0.1*ncol(se)), fig.height=10, fig.retina=2}
...
```{r render go cluster, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
if (sum(use) > param$minGenesForClustering){
  if (!is.null(clusterResult$GO)){
    ## GO Cluster tables
