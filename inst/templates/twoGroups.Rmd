  ...
  for(onto in names(enrichResult)){
    ...
    for(sig in names(enrichResult[[onto]])){
      if(is.na(enrichResult[[onto]][[sig]][1][[1]])){
        ...
        er@result$Synonym <- NULL
        l <- NULL
        for (i in 1:length(er@result$Description)) {
          desc <- er@result$Description[i]
          syn <- Synonym(GOTERM[[er@result$ID[i]]])
          syn <- syn[grep("GO:", syn, invert = T)]
          syn <- syn[order(nchar(syn))][1]
          if (nchar(desc) < 35) {
            er@result$Synonym[i] <- paste(er@result$ID[i],desc)
            l <- c(l,paste0(er@result$ID[i],"\n",desc))
          } else if (nchar(desc) > 35 & nchar(syn) <= 35 & !is.na(syn)) {
            er@result$Synonym[i] <- paste(er@result$ID[i],syn)
            l <- c(l,paste0(er@result$ID[i],"\n",syn))
          } else if (nchar(desc) >= 35 & nchar(syn) > 35 | is.na(syn)) {
            er@result$Synonym[i] <- paste(er@result$ID[i],substr(er@result$Description[i], 1, 35))
            l <- c(l,paste0(er@result$ID[i],"\n",substr(er@result$Description[i], 1, 35)))
          }
        }
        labels[[onto]][[sig]] <- l
        ...
        dotplot.labels <- labels[[onto]][[sig]]
        erp[["dotplot"]] <- dotplot(er, showCategory = 20, title=paste(onto, sig)) + scale_y_discrete(labels = dotplot.labels) + theme(axis.text.y = element_text(vjust=-0.01, size = 8))
        ...
        erp[["cnet"]] <- cnetplot(er, foldChange = log2Ratio)
...
if (doGo(param, seqAnno)){
  enrichResultGSEA <- ezGSEA(enrichInput)
  resFns <- c()
}
```
  ...
  for(onto in names(enrichResultGSEA)){
    enrichResultGSEAPlots[[onto]] = list()
      if(is.na(enrichResultGSEA[[onto]][1][[1]])){
        ...
        dotplot.labels <- labels[[onto]]
        erp[["dotplot"]] <- dotplot(er, showCategory = 20, title=paste(onto)) + scale_y_discrete(labels = dotplot.labels) + theme(axis.text.y = element_text(vjust=-0.01, size = 8))
        erp[["ridgeplot"]] <- ridgeplot(er, showCategory = 20) + scale_y_discrete(labels = dotplot.labels) + theme(axis.text.y = element_text(vjust=-0.01, size = 8))
