...
if (doGo(param, seqAnno)){
    if (doEnrichr(param)){
      ...
      enrichrLinks = ezMatrix("", rows=selectionNames, cols=c("Number of Genes", "External", "Precomputed"))
      ...
      for (mySel in selectionNames){
        ...
        if (!is.null(genesToUse) && length(genesToUse) > 3 && param$doPrecomputeEnrichr) {
          ...
          if (length(resList) > 0) {
            ...
            enrichrLinks[mySel, "Precomputed"] = ezLink(link=enrichrTablePath,
                                                      label="Report",
                                                      target="_blank")
          } else {
...
if (doGo(param, seqAnno)){
  enrichResult <- ezEnricher(enrichInput)
  ...
  for(onto in names(enrichResult)){
    for(sig in names(enrichResult[[onto]])){
      if(is.na(enrichResult[[onto]][[sig]][1][[1]])){
        ...
        enrichResultReduced[[onto]][[sig]]@result$geneID <- enrichResultReduced[[onto]][[sig]]@result$geneName
         enrichResultPlots[[paste0(onto,"_",sig,"_","df")]] <- as.data.frame(enrichResult[[onto]][[sig]]@result[,c(1:3,5,6,10)])
        enrichResultPlots[[paste0(onto,"_",sig,"_","table")]] <- datatable(enrichResult[[onto]][[sig]]@result[,c(1:3,5,6,10)], filter = "top", 
                                                                        class = 'cell-border stripe', rownames = FALSE, caption = '') %>%
                                                                formatStyle(columns = colnames(.$x$data), `font-size` = "12px") %>% 
                                                                formatSignif(c("pvalue","p.adjust"), digits = 3)
        enrichResultPlots[[paste0(onto,"_",sig,"_","dotplot")]] <- dotplot(enrichResultReduced[[onto]][[sig]], showCategory = 20,
                     title=paste(onto, sig))
        enrichResultPlots[[paste0(onto,"_",sig,"_","lfc")]] <- seqAnno$log2Ratio[match(enrichInput$selections[[sig]], seqAnno$gene_id)]
        names(enrichResultPlots[[paste0(onto,"_",sig,"_","lfc")]]) <- seqAnno$gene_name[match(enrichInput$selections[[sig]], seqAnno$gene_id)]
        enrichResultPlots[[paste0(onto,"_",sig,"_","cnet")]] <- cnetplot(enrichResultReduced[[onto]][[sig]], foldChange = enrichResultPlots[[paste0(onto,"_",sig,"_","lfc")]])
...
for (onto in 1:length(enrichResult)) {
  cat("####",names(enrichResult)[onto],"{.tabset}","\n")
  for (sig in 1:length(names(enrichResult[[onto]]))) {
      cat("#####",gsub("Genes","",names(enrichResult[[onto]])[sig]),"{.tabset}","\n")
      if (is.na(enrichResult[[onto]][[sig]][1][[1]])){
        next
      } else {
        cat(paste0("[", paste(names(enrichResult)[onto], names(enrichResult[[onto]])[sig], "hypergeometric.txt", sep="_"), "](", paste(names(enrichResult)[onto], names(enrichResult[[onto]])[sig], "hypergeometric.txt", sep="_"), ")"))
        print( kable(enrichResultPlots[[ paste0(names(enrichResult)[onto], "_", names(enrichResult[[onto]])[sig], "_", "df") ]], 
                     escape=FALSE, row.names=FALSE, format = "html") %>% kable_styling(bootstrap_options = "striped", full_width = F, fixed_thead = T,) %>%
                scroll_box(width = "800px", height = "500px") )
        #print( enrichResultPlots[[ paste0(names(enrichResult)[onto], "_", names(enrichResult[[onto]])[sig], "_", "table") ]] )
        print( enrichResultPlots[[ paste0(names(enrichResult)[onto], "_", names(enrichResult[[onto]])[sig], "_", "dotplot") ]] )
        print( enrichResultPlots[[ paste0(names(enrichResult)[onto], "_", names(enrichResult[[onto]])[sig], "_", "cnet") ]] )
        cat('\n\n')
    }
  }
}
