...
### Association test
This show whether up and down-regulated genes are associated with GC content and gene length.
...
sigTable <- data.frame("Up-regulation"=isUp,
                       ...
                       check.names=FALSE)
testTable <- data.frame(row.names=apply(expand.grid(names(sigTable), 
                                                    names(biasTable)), 
                                        1, paste, collapse=": "),
                        check.names=FALSE)
for(rowName in names(sigTable)){
  for(colName in names(biasTable)){
    res.fisher <- fisher.test(biasTable[[colName]], sigTable[[rowName]],
                              alternative="two.sided")
    testTable[paste(rowName, colName, sep=": "), "overlapping genes"] = 
      paste(sum(biasTable[[colName]] & sigTable[[rowName]]),
            sum(sigTable[[rowName]]), sep="/")
    testTable[paste(rowName, colName, sep=": "), "odds ratio"] = 
      res.fisher$estimate
    testTable[paste(rowName, colName, sep=": "), "p-value"] = res.fisher$p.value
  }
}
t <- kable(format(testTable, digits=4, scientific=TRUE), "html", 
           caption=paste("Association test:",
                         sum(isUp), "up-regulated and",
                         sum(isDown), "down-regulated genes")) %>%
     kable_styling(bootstrap_options = "striped", full_width = F, 
                   position = "left") %>%
     footnote(number = c("The significant (p <= 1e-3) rows are in red.",
                         "The overlapping genes column is the number of overlapping genes over the total number of genes with the GC content or gene width criteria.")
     )
