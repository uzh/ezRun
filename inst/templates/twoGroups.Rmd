...
if (isTRUE(param$runGO)) {
  if (doEnrichr(param)) {
    ...
    for (mySel in selectionNames) {
      ...
      if(mySel == 'upGenes')
        enrichrLinks[mySel, "CutOffs"] <- paste(enrichrLinks[mySel, "CutOffs"], ", log ratio > ", param$log2RatioThreshGO)
      if(mySel == 'downGenes')
        enrichrLinks[mySel, "CutOffs"] <- paste(enrichrLinks[mySel, "CutOffs"], ", log ratio < ", -param$log2RatioThreshGO)
...
* Gene Selection: `r paste0("p <= ", param$pValThreshGO, ", Up: log ratio >", param$log2RatioThreshGO, "/Down: log ratio < ", -param$log2RatioThreshGO)`

* Candidate Terms: `r paste("fdr <=", param$fdrThresORA)`
...
if (isTRUE(param$runGO)) {
    ...
    for (resName in names(enrichResult[[goDomain]])){
              ...
              ora.overview[testName, "Number of Significant Terms"] = 
                nrow(enrichResult[[goDomain]][[resName]]@result[
                  enrichResult[[goDomain]][[resName]]@result$p.adjust <= param$fdrThresORA, ])
    }
  }
  ...
  cat("\n\n")
  cat("[Download Excel file of complete ORA results](`r oraFile`)")
  cat("\n\n")
}
```
...
* Candidate Terms: `r paste("fdr <=", param$fdrThresGSEA)`
  ...
  paste(
    kable(
      ...
      kable_styling(
        ...
        position = "left"))
  cat("\n\n")
  cat("[Download Excel file of complete GSEA results](`r gseaFile`)")
  cat("\n\n")
}
```
