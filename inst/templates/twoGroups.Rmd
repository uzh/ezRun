    ...
    for (sig in names(enrichResult[[onto]])) {
      if (is.na(enrichResult[[onto]][[sig]][1][[1]])) {
        ...
        erp[["df"]] <- as.data.frame(
          er@result[ , c("ID", "Description", "GeneRatio", "pvalue",
                         "p.adjust", "geneName")])
        erp[["table"]] <- datatable(
          data = er@result[, c("ID", "Description", "GeneRatio", "pvalue",
                               "p.adjust", "geneName")],
        ...
        erp[["dotplot"]] <- dotplot(
          object = er, showCategory = 20, title = paste(onto, sig)) + 
        ...
        log2Ratio <- enrichInput$seqAnno$log2Ratio[
          enrichInput$seqAnno$gene_id %in% sigGeneIds]
        names(log2Ratio) <- enrichInput$seqAnno[sigGeneIds, "gene_name"]
        log2Ratio <- sort(log2Ratio, decreasing = T)
        erp[["cnet"]] <- cnetplot(
          x = er, foldChange = log2Ratio, 
...
oraFile <- "ORA_results.xlsx"
for (onto in names(enrichResultPlots)) {
  for (sig in names(enrichResultPlots[[onto]])) {
    dfOut <- enrichResultPlots[[onto]][[sig]]$df
    if (!file.exists(oraFile)) {
      xlsx::write.xlsx(
        x = dfOut[dfOut$pvalue < 0.05, ],
        file = oraFile, sheetName = paste0(onto, "_", sig),
        row.names = FALSE)
    } else {
      xlsx::write.xlsx(
        x = dfOut[dfOut$pvalue < 0.05, ],
...
##### Description
The  Over Representation Analysis (ORA) gives an estimate of whether a set of selected genes  is enriched for genes in specific categories. It takes as input a subset of genes passing certain p-value/fold-change thresholds  (i.e.,differentially expressed genes). **It is recommended when the difference between groups is large (e.g., 500+ genes above p-value/fold-change thresholds).** There are many implementations of this test: in Sushi we use the R package Goseq. Further details: [Visit the goseq bioconductor page](https://bioconductor.org/packages/release/bioc/html/goseq.html)
  ...
  for (resName in names(enrichResult[[goDomain]])){
            ...
            ora.overview[testName, "Number of Significant Pathways"] = 
              nrow(enrichResult[[goDomain]][[resName]]@result[
                enrichResult[[goDomain]][[resName]]@result$pvalue < 0.05, ])
  }
}
...
[Download Excel file of complete ORA results](`r oraFile`)
...
# Print out a final combined xlsx 
gseaFile <- "GSEA_results.xlsx"
for (onto in names(enrichResultPlots)) {
  dfOut <- enrichResultGSEAPlots[[onto]]$df
  if (!file.exists(gseaFile)) {
    xlsx::write.xlsx(
      x = dfOut[dfOut$pvalue < 0.05, ],
      file = gseaFile, sheetName = onto,
      row.names = FALSE)
  } else {
    xlsx::write.xlsx(
      x = dfOut[dfOut$pvalue < 0.05, ],
...
[Download Excel file of complete GSEA results](`r gseaFile`)
