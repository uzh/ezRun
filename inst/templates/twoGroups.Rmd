...
### MetaCore 
```{r metacore annotate, echo=FALSE, message=FALSE, error=FALSE}
...
```{r metacore prepare, echo=FALSE, message=FALSE, error=FALSE}
resultMetaCore <- resultLoaded %>%
  filter(isPresent, pValue <= param$pValThreshGO) %>%
  dplyr::left_join(geneAnno) %>%
  dplyr::select(gene_id, entrez_id, "log2 Ratio", pValue)
```

```{r metacore export, echo=FALSE, message=FALSE, error=FALSE}
