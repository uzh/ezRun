...
```{r count, echo=FALSE}
dat.file=paste0(params$sample, "/bowtie2.log" )
...
```{r s1, echo=FALSE}
list.file=paste0(params$sample, "/summary_table.tsv")
...
if (nrow(sub)!=0){
	...
	symbols(agg$genomeCov_pect, agg$aveDepth, circles=sqrt(agg$mappedBases/pi), inches=0.35, fg=factor(agg$Family), bg="white", xlab="% genome coverage", ylab="Mean depth", xlim=c(min(agg$genomeCov_pect)-max(sqrt(agg$mappedBases/pi))*0.35, max(agg$genomeCov_pect)+max(sqrt(agg$mappedBases/pi))*0.35), ylim=c(min(agg$aveDepth)-max(sqrt(agg$mappedBases/pi))*0.35, max(agg$aveDepth)+max(sqrt(agg$mappedBases/pi))*0.35))
text(agg$benomeCov_pect, agg$aveDepth,agg$Family, cex=0.7, col = 1:length(agg$Family))
}else{
  txt<-paste("No virus detected with more than", params$minReadCount, "reads.")
  txt
}
...
```{r s3, echo=FALSE}
if (nrow(sub)!=0){
  mylist<-split(sub, factor(subFamily))
...
if (nrow(sub)!=0){
  for(i in 1:nrow(sub)) {
	chr=sub$ID
	common_name=sub$CommonName
	csv.file<-paste0(params$sample, "/", chr, ".csv")
