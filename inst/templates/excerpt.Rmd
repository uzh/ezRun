...
```{r prepare data output, include=FALSE}
## start of debug
# require(ezRun)
data.dir = grep(pattern='*CORE_RESULTS*',list.dirs('/export/local/scratch/test_exceRpt/testData_human/', full.names = TRUE, recursive = FALSE),value = TRUE)
output.dir = paste(data.dir,'processed_output',sep='/')
## end of debug

data.dir = param$OUTPUT_DIR
output.dir = paste(data.dir,'processed_output',sep='/')

plots = processSamplesInDir(data.dir,output.dir, getPlotsObjects=TRUE)
```

## exceRpt_Result [reference](https://www.sciencedirect.com/science/article/pii/S2405471219300742) {.tabset}
...
```{r read-length vs counts, echo=FALSE}
## raw
...
```{r fractions of aligned reads, echo=FALSE}
