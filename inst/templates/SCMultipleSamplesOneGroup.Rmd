...
plotReducedDim(sce, dimred =  reduction_name, colour_by = "Plate", pt.size = 0.8)
plotReducedDim(sce, dimred =  reduction_name, label = TRUE, pt.size = ) 
```
...
cellIdents_perSample = FetchData(object = scData, vars = c(active_idents, 'Plate'))
barplot = ggplot(data=cellIdents_perSample, aes(x=active_idents, fill=Plate)) + geom_bar(stat="Count")
