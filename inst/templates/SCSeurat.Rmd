...
library(scater)
library(dplyr)
library(Seurat, lib.loc="/home/gtan/R-library-Seurat3")
...
scData <- subset(scData, subset = nFeature_RNA > param$minGenesPerCell & 
                   nFeature_RNA < param$maxGenesPerCell & 
                   percent.mt < param$maxMitoPercent)
```
...
FeaturePlot(object=scData, features="percent.mt",
            reduction="umap")
DimPlot(scData, reduction="umap", group.by="CellCycle") + ggtitle("Cell cycle phase")
```
...
FeaturePlot(object=scData, features="percent.mt",
            reduction="tsne")
DimPlot(scData, reduction="tsne", group.by="CellCycle") + ggtitle("Cell cycle phase")
```
...
write_tsv(as_tibble(markers, rownames="id"), path=posMarkersFn)
```
...
for(eachCluster in unique(markers$cluster)){
  ...
  for(eachMarker in markersPerCluster){
    VlnPlot(object = scData, features=eachMarker)  + theme(legend.position = "none")
    FeaturePlot(object=scData, features=eachMarker, reduction="umap")
    FeaturePlot(object=scData, features=eachMarker, reduction="tsne")
  }
  cat("\n")
}
```
  ...
  for(i in 1:length(param$knownMarkers)){
    ...
    if(length(check_markers) > 0){
      for(j in 1:length(check_markers)){
        VlnPlot(object = scData, features=check_markers[j])  + 
          theme(legend.position = "none")
        FeaturePlot(object=scData, features=check_markers[j], reduction="umap")
        FeaturePlot(object=scData, features=check_markers[j], reduction="tsne")
      }
    }
    cat("\n")
  }
...
```
  ...
  * Final Seurat object: [`r basename(output$getColumn("Live Report")`](`r basename(output$getColumn("Live Report")`)
