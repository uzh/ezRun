...
```{r plot SingleR, echo=FALSE, fig.width=14, eval=doSingleR}
...
all2allDEFn <- "all2allDE"
dir.create(all2allDEFn)
if(param$all2allMarkers){
  ...
  for(comparison in names(all2allMarkers)){
    if(nrow(all2allMarkers[[comparison]]) > 0){
      write_tsv(as_tibble(all2allMarkers[[comparison]], rownames="Gene"),
                path=file.path(all2allDEFn, paste0(comparison, ".tsv")))
    }else{
      write_tsv(as_tibble(all2allMarkers[[comparison]]),
                path=file.path(all2allDEFn, paste0(comparison, ".tsv")))
    }
  }
  lapply(list.files(all2allDEFn, "Cluster", full.names = TRUE), R.utils::gzip,
         compression=9)
}
```
...
```
  ...
  * Differential genes of pairwise comparisons of all clusters (when enabled): [`r all2allDEFn`](`r all2allDEFn`)
