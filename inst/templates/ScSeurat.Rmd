...
VlnPlot(scData.unfiltered, features=c("nCount_RNA", "nFeature_RNA", "percent_mito", "percent_ribo"), group.by="Batch", ncol = 4)
```
...
num.qc.ribo <- sum(scData.unfiltered$qc.ribo)
...
```{r gene filtering}
cat("genes removed:", nrow(scData.unfiltered)-nrow(scData))
...
VlnPlot(scData, features=c("nCount_RNA", "nFeature_RNA", "percent_mito", "percent_ribo"), group.by="Batch", ncol = 4)
```

```{r QC plot mito after filtering, fig.align='center'}
FeatureScatter(scData, feature1="nCount_RNA", feature2="percent_mito", group.by="discard")
```
...
cellIdents_perSample <- data.frame(ident = Idents(scData), orig.ident=scData$orig.ident)
