...
scData <- ezLoadRobj("scData", nthreads = param$cores)
output <- ezLoadRobj("output")
scData$qc.doublet <- scData$doubletClass %in% "doublet"
allCellsMeta <- ezLoadRobj("allCellsMeta")
species <- getSpecies(param$refBuild)

if (ezRobjExists("aziResults")) {
  aziResults <- ezLoadRobj("aziResults")
  scData <- AddMetaData(scData, aziResults)
} else {
  aziResults <- NULL
}

if (ezRobjExists("cellxgeneResults")) {
  cellxgeneResults <- ezLoadRobj("cellxgeneResults")
  scData <- AddMetaData(scData, cellxgeneResults)
} else {
  cellxgeneResults <- NULL
}

if (ezRobjExists("singler.results")) {
  singler.results <- ezLoadRobj("singler.results")
} else {
  singler.results <- NULL
}

if (ezRobjExists("cells.AUC")) {
  cells.AUC <- ezLoadRobj("cells.AUC")
} else {
  cells.AUC <- NULL
}

if (ezRobjExists("enrichRout")) {
  enrichRout <- ezLoadRobj("enrichRout")
...
if (ezRobjExists("cellsPerGeneFraction")) {
  cellsPerGeneFraction <- ezLoadRobj("cellsPerGeneFraction")
} else {
  cellsPerGeneFraction <- NULL
}

# Load scType results (only tissue_type string, annotation is in scData metadata)
if (ezRobjExists("sctype_results")) {
  sctype_results <- ezLoadRobj("sctype_results")
  # Handle old format where scData was saved inside results
  if ("scData" %in% names(sctype_results)) sctype_results$scData <- NULL
  sctype_available <- "sctype_classification" %in% colnames(scData@meta.data)
} else {
  sctype_results <- NULL
  sctype_available <- FALSE
}

# Check Azimuth Pan-Human availability from metadata
azimuth_available <- "final_level_labels" %in% colnames(scData@meta.data)

# Check CyteTypeR availability from metadata
cytetype_available <- "CyteTypeR_annotation" %in% colnames(scData@meta.data)
if (cytetype_available && file.exists("cytetype_results.rds")) {
  cytetype_results <- readRDS("cytetype_results.rds")
} else {
  cytetype_results <- NULL
  cytetype_available <- FALSE
}

# Check mLLMCelltype availability from metadata
mllm_available <- "mLLMCelltype" %in% colnames(scData@meta.data)
if (mllm_available && file.exists("mllmcelltype_results.rds")) {
  mllm_results <- readRDS("mllmcelltype_results.rds")
} else {
  mllm_results <- NULL
...
```{r hemoglobin plots, fig.width=8, fig.height=4}
...
if (any(!is.na(xxAll$doubletScore))) {
  plot8 <- plotColData(
    xxAll,
    x="Sample",
    y="doubletScore",
...
qcStats[ "Library Size", ] <- c(
  c(sum(allCellsMeta$qc.lib, na.rm = TRUE), range(allCellsMeta$nCount_RNA[allCellsMeta$useCell])) %>% as.integer(),
  round(sum(allCellsMeta$qc.lib, na.rm = TRUE)/nrow(allCellsMeta), 2)
)
qcStats[ "Expressed genes", ] <- c(
  c(sum(allCellsMeta$qc.nexprs, na.rm = TRUE), range(allCellsMeta$nFeature_RNA[allCellsMeta$useCell])) %>% as.integer(),
  round(sum(allCellsMeta$qc.nexprs, na.rm = TRUE)/nrow(allCellsMeta), 2)
)
qcStats[ "Mitochondrial Percent", ] <- c(
  c(sum(allCellsMeta$qc.mito, na.rm = TRUE), range(allCellsMeta$percent_mito[allCellsMeta$useCell])) %>% as.integer(),
  round(sum(allCellsMeta$qc.mito, na.rm = TRUE)/nrow(allCellsMeta), 2)
)
qcStats[ "Ribosomal protein Percent", ] <- c(
  c(sum(allCellsMeta$qc.riboprot, na.rm = TRUE), range(allCellsMeta$percent_riboprot[allCellsMeta$useCell])) %>% as.integer(),
  round(sum(allCellsMeta$qc.riboprot, na.rm = TRUE)/nrow(allCellsMeta), 2)
)
qcStats[ "Doublets", ] <- c(
  sum(allCellsMeta$qc.doublet, na.rm = TRUE), 
  NA, 
  NA, 
  round(sum(allCellsMeta$qc.doublet, na.rm = TRUE) / nrow(allCellsMeta), 2)
)
qcStats[ "Empty Droplets", ] <- c(
  sum(allCellsMeta$qc.empty, na.rm = TRUE), 
  NA, 
  NA, 
  round(sum(allCellsMeta$qc.empty, na.rm = TRUE) / nrow(allCellsMeta), 2)
)
...
## Spell out what each criterion measures AND the cut that was actually applied. Neither was
## visible before: "Library Size" does not say it means total UMIs, and the Min/Max Pass Value
## columns show the range of the KEPT cells, not the threshold - so a reader could not tell from
## this table whether a filter was strict or loose. An unset threshold is not "no filter", it
## selects the automatic per-sample MAD cut, which is worth naming here too.
qcCut <- function(value, cmp, unit = "") {
  if (!ezIsSpecified(value)) return("automatic threshold")
  sprintf("removed if %s %s%s", cmp, format(value), unit)
}
qcLabels <- c(
  "Library Size"              = sprintf("Library Size (total UMIs, %s)",
                                        qcCut(param$nUMI, "<")),
  "Expressed genes"           = sprintf("Expressed genes (genes detected, %s)",
                                        qcCut(param$ngenes, "<")),
  "Mitochondrial Percent"     = sprintf("Mitochondrial Percent (%s)",
                                        qcCut(param$perc_mito, ">", "%")),
  "Ribosomal protein Percent" = sprintf("Ribosomal protein Percent (%s)",
                                        qcCut(param$perc_riboprot, ">", "%")),
  "Doublets"                  = "Doublets (scDblFinder)",
  "Empty Droplets"            = "Empty Droplets (emptyDrops)"
)
hit <- qc_dt$Metric %in% names(qcLabels)
qc_dt$Metric[hit] <- qcLabels[qc_dt$Metric[hit]]

## The per-criterion rows OVERLAP: one cell can fail several criteria at once, and is counted
## in each of their rows but only once in "Total removed". So the rows deliberately sum to more
## than the total, which reads as an arithmetic error unless the table says so. State it, with
## the numbers computed from this sample rather than described in the abstract.
qcFlagCols <- intersect(c("qc.lib", "qc.nexprs", "qc.mito", "qc.riboprot",
                          "qc.doublet", "qc.empty"), colnames(allCellsMeta))
qcCaption <- NULL
if (length(qcFlagCols) > 0) {
  qcFlagMat  <- vapply(qcFlagCols, function(f) !is.na(allCellsMeta[[f]]) &
                         allCellsMeta[[f]], logical(nrow(allCellsMeta)))
  nPerCell   <- rowSums(qcFlagMat)
  nMulti     <- sum(nPerCell > 1)
  nRowSum    <- sum(qcFlagMat)
  nRemoved   <- sum(!allCellsMeta$useCell, na.rm = TRUE)
  qcText <- if (nMulti > 0) {
    ## Criteria overlapped here, so the rows genuinely do not sum to the total. Say by how much.
    sprintf(paste("Each row counts cells failing that criterion. A cell can fail several, so the",
                  "rows sum to %s while %s distinct cells were removed: %s cell%s failed more",
                  "than one criterion."),
            format(nRowSum, big.mark = ","), format(nRemoved, big.mark = ","),
            format(nMulti, big.mark = ","), ifelse(nMulti == 1, "", "s"))
  } else {
    ## No cell failed more than one criterion, so the rows DO sum to the total. Saying
    ## "criteria overlap" here would be wrong and would make a correct table look suspect.
    sprintf(paste("Each row counts cells failing that criterion. No cell failed more than one,",
                  "so the rows sum exactly to the %s cells removed."),
            format(nRemoved, big.mark = ","))
  }
  qcCaption <- htmltools::tags$caption(
    style = "caption-side: bottom; text-align: left; font-size: 90%; color: #6c757d;",
    paste(qcText, "A dash means the criterion was not applied.")
  )
}
...
```{r clustering plot1, fig.width=9}
...
new_names[is_pct_col] <- sub("_fraction$", " (%)", 
                             new_names[is_pct_col])
...
if (any(!is.na(scData$doubletScore))) {
  print(VlnPlot(scData, features ="doubletScore") + ggtitle("DoubletScore vs ident") + NoLegend())
  print(FeaturePlot(scData, features="doubletScore", label = TRUE))
}
```
...
if (length(genesToPlot) > 0){
  if (length(genesToPlot) < 20){
    ...
    ezDoHeatmap(scData, features=genesToPlot)
...
## DoHeatmap crashes the whole report when `features` is empty: Seurat only
## errors on *some* features missing (`any(!features %in% possible)` is FALSE
## for character(0)), so a 0-feature matrix reaches SingleRasterMap -> Melt(),
## which collapses to 1 column, and `colnames<-` then fails with
##   'names' attribute [3] must be the same length as the vector [1]
## genesToPlot is empty whenever no markers were found (e.g. a low-cell sample
## that yields a single cluster). Skip the heatmap, keep the rest of the report.
tryCatch(
  print(ezDoHeatmap(scData, features = unique(genesToPlot))),
  error = function(e) {
    message("Skipping marker heatmap (DoHeatmap failed): ", conditionMessage(e))
  }
)
DotPlot(scData, features=genesToPlot) + coord_flip() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
if (length(genesToPlot) > 0){
  tryCatch(
    print(ezDoHeatmap(scData, features = unique(genesToPlot))),
    error = function(e) {
      message("Skipping controlSeqs heatmap (DoHeatmap failed): ", conditionMessage(e))
    }
  )
  DotPlot(scData, features=genesToPlot) + coord_flip() +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
```{r AUC skipped, eval= is.null(cells.AUC), results='asis'}
## Without this, an unrun AUCell section leaves only the heading above and the
## reader cannot tell whether it was skipped or found nothing. Say which, and why.
if (!species %in% c("Human", "Mouse")) {
  cat(sprintf(paste0("**Not run.** CellMarker2 covers only human and mouse, and this dataset's ",
                     "reference (`%s`) resolves to neither. No cell type annotation was ",
                     "attempted from marker gene sets.\n\n"), param$refBuild))
} else if (!ezIsSpecified(param$tissue)) {
  cat(paste0("**Not run.** No tissue was selected, so no CellMarker2 gene sets were loaded. ",
             "Re-run the app with one or more tissues selected in the `tissue` parameter to ",
             "get this section. Selecting every tissue is not recommended - it scores ~930 ",
             "overlapping gene sets at once and measurably degrades the labels.\n\n"))
} else {
  cat(sprintf(paste0("**Not run.** No CellMarker2 gene sets with at least 3 genes were found ",
                     "for %s in the selected tissue(s): %s.\n\n"),
              species, paste(param$tissue, collapse = ", ")))
}
```
...
if(azimuth_available){
  ...
  # 5. FeaturePlot of confidence scores for final level celltype assignment
  ...
  # 6. Histogram of confidence scores for final level celltype assignment
  if("final_level_confidence" %in% colnames(scData_az@meta.data)){
  ...
  # 7. Violin plot of confidence scores for final level celltype assignment per cell type
  if("final_level_confidence" %in% colnames(scData_az@meta.data) &&
...
# Create shortened cell type labels for plotting (originals are kept)
scData$cellxgene_label_short <- scData$predicted.id.cellxgene.authorlabel %>%
  ...
  {ifelse(
      nchar(.) > 30,
      paste0(substr(., 1, 8), "...", substr(., nchar(.) - 14, nchar(.))), .)
  }
...
# Legend is too large, so we plot it separately and patch it together
...
## mapping quality by Vlnplots (use short labels for x-axis readability)
...
```{r cellxgene_3, eval=!is.null(cellxgeneResults), fig.width=12, fig.height=10}
tryCatch({
...
`r if(mllm_available) sprintf("mLLMCelltype annotates each cluster from its top marker genes using a large language model. The model (%s) runs on an FGCZ-internal server, so no expression data leaves FGCZ. Only the top marker gene names per cluster are sent, and the labels are LLM suggestions that should be confirmed against the marker genes below.", mllm_results$model)`

```{r mllmcelltype results, eval=mllm_available, results='asis', fig.width=14, fig.height=10}
cat("**Tissue context given to the model:** ", mllm_results$tissue_name, "\n\n")

if (isTRUE(mllm_results$tissue_is_auto)) {
  cat("<div style=\"border-left:4px solid #c0392b; background:#fdecea;",
      "padding:10px 14px; margin:10px 0;\">\n",
      "<b>Treat these labels with caution: no tissue context was supplied.</b>",
      "`mLLMCelltype.tissue` was left at <code>auto</code>, so the model was",
      "only told <b>&quot;", mllm_results$tissue_name, "&quot;</b>.\n\n",
      "This is the single biggest factor in label quality. The model sees only",
      "the top marker genes, so without knowing what was sequenced it will",
      "assign whichever lineage those genes suggest in a general context. On a",
      "FACS-sorted B-cell dataset that produced confident monocyte, Treg, ILC2",
      "and stromal calls for clusters that were in fact all B cells (4 of 14",
      "correct); re-running the identical markers with",
      "<code>mLLMCelltype.tissue = \"sorted B cells\"</code> gave 13 of 14.\n\n",
      "Set <code>mLLMCelltype.tissue</code> to the tissue or sorted population",
      "and re-run before using these labels.\n</div>\n\n", sep = " ")
}

print(DimPlot(scData, reduction = "umap", label = TRUE, repel = TRUE,
              group.by = "mLLMCelltype", order = TRUE,
              cols = shared_colors, raster = FALSE) +
        ggtitle(paste("mLLMCelltype -", mllm_results$model)) +
        theme(legend.position = "right", aspect.ratio = 1))
cat("\n\n")

mllm_table <- unique(data.frame(
  Cluster = as.character(scData$seurat_clusters),
  mLLMCelltype = scData$mLLMCelltype,
  stringsAsFactors = FALSE
))
## the marker genes the model cited for each call, so the label can be checked
if (!is.null(mllm_results$clusterSupport)) {
  mllm_table$SupportingMarkers <- unname(
    mllm_results$clusterSupport[mllm_table$Cluster])
}
mllm_table <- mllm_table[order(suppressWarnings(as.numeric(mllm_table$Cluster)),
                               mllm_table$Cluster), ]
cat("#### Cluster labels proposed by mLLMCelltype\n\n")
cat("`SupportingMarkers` are the genes the model itself cited. Check each label",
    "against them: these are suggestions from marker names alone, with no",
    "reference dataset, and the model will name a cell type even for a",
    "low-quality cluster.\n\n")
ezInteractiveTableRmd(mllm_table, rowNames = FALSE)
```

### CyteTypeR

`r if(cytetype_available) "CyteTypeR provides AI-powered cell type annotation using large language models via the Nygen Analytics API."`

```{r cytetype results, eval=cytetype_available, results='asis', fig.width=14, fig.height=10}
if (cytetype_available && !is.null(cytetype_results)) {

  # Show cloud report link if available
  cytetype_report_url <- NULL

  # Try @misc$cytetype_jobDetails (fields: job_id, report_url, api_url, group_key, cluster_labels)
  if (!is.null(scData@misc$cytetype_jobDetails)) {
    jd <- scData@misc$cytetype_jobDetails
    if (!is.null(jd$report_url)) {
      cytetype_report_url <- jd$report_url
    } else if (!is.null(jd$job_id)) {
      cytetype_report_url <- paste0("https://prod.cytetype.nygen.io/report/", jd$job_id)
    } else if (!is.null(jd$jobId)) {
      cytetype_report_url <- paste0("https://prod.cytetype.nygen.io/report/", jd$jobId)
    }
  }

  # Fallback: check for job_details JSON files created by CyteTypeR
  if (is.null(cytetype_report_url)) {
    json_files <- Sys.glob("job_details_*.json")
    if (length(json_files) > 0) {
      job_json <- jsonlite::fromJSON(json_files[1])
      job_id_val <- if (!is.null(job_json$job_id)) job_json$job_id else job_json$jobId
      if (!is.null(job_id_val)) {
        cytetype_report_url <- paste0("https://prod.cytetype.nygen.io/report/", job_id_val)
      }
    }
  }
  ...
  if (!is.null(cytetype_report_url)) {
    cat(sprintf(
      "**[CyteType Interactive Report](%s)** -- detailed annotation justifications, markers, and quality metrics\n\n",
      cytetype_report_url
    ))
  }

  # 1. CyteTypeR cell type annotation UMAP
  if ("CyteTypeR_annotation" %in% colnames(scData@meta.data)) {
    cat("#### CyteTypeR Cell Type Annotation\n\n")
    print(DimPlot(scData, reduction = "umap", label = TRUE, repel = TRUE,
                  group.by = "CyteTypeR_annotation", order = TRUE,
                  cols = shared_colors, raster = FALSE) +
            ggtitle("CyteTypeR Cell Type Annotation") +
            theme(legend.position = "right", aspect.ratio = 1))
    cat("\n\n")
  }

  # 2. Granular annotation UMAP (NoLegend for granular — many categories, labels sufficient)
  if ("CyteTypeR_granular" %in% colnames(scData@meta.data)) {
    cat("#### CyteTypeR Granular Annotation\n\n")
    print(DimPlot(scData, reduction = "umap", label = TRUE, repel = TRUE,
                  group.by = "CyteTypeR_granular", order = TRUE,
                  cols = shared_colors, raster = FALSE) +
            ggtitle("CyteTypeR Granular Cell Type Annotation") +
            NoLegend() +
            theme(aspect.ratio = 1))
    cat("\n\n")
  }

  # 3. Annotation details table
  cat("#### Annotation Details\n\n")
  display_cols <- intersect(
    c("clusterId", "annotation", "granularAnnotation", "ontologyTerm",
      "supportingMarkers", "conflictingMarkers"),
    colnames(cytetype_results)
  )
  knitr::kable(cytetype_results[, display_cols], row.names = FALSE) |> cat(sep = "\n")
  cat("\n\n")

  # 4. Cell type distribution
  cat("#### Cell Type Distribution\n\n")
  ct_counts <- as.data.frame(table(scData$CyteTypeR_annotation))
  colnames(ct_counts) <- c("CellType", "Count")
  ct_counts <- ct_counts[order(-ct_counts$Count), ]
  ct_counts$Percentage <- sprintf("%.1f%%", 100 * ct_counts$Count / sum(ct_counts$Count))

  p_bar <- ggplot(ct_counts, aes(x = reorder(CellType, -Count), y = Count, fill = CellType)) +
...
# ezFrame(Value=param[c("npcs","pcGenes","resolution", "SCT.regress.CellCycle", "DE.method", "cellsFraction", "nUMIs", "nmad", "normalize")] |> 
#           unlist()) |> rownames_to_column("Parameter") |>
  # ezKable()
