...
```{r azimuth, eval=!is.null(aziResults), fig.width=15, fig.height=8}
...
```{r azimuth panhuman results, eval=azimuth_available, results='asis', fig.width=12, fig.height=8}
if(azimuth_available && !is.null(azimuth_results)){
  cat("**Reference:** Pan-Human Azimuth neural network model\n\n")
  cat("**Cells annotated:** ", azimuth_results$n_cells_annotated, "\n\n")
  cat("**Confidence threshold:** ", azimuth_results$confidence_threshold, "\n\n")
  ...
  if("final_level_softmax_prob" %in% colnames(scData_az@meta.data)){
    ...
    p <- ggplot(scData_az@meta.data, aes(x = final_level_softmax_prob)) +
      ...
      geom_vline(xintercept = azimuth_results$confidence_threshold, 
                 linetype = "dashed", color = "red", size = 1) +
      theme_minimal() +
      labs(title = "Azimuth Pan-Human: Softmax Probabilities",
           x = "Softmax Probability", 
           y = "Number of Cells",
           subtitle = paste("Confidence threshold:", azimuth_results$confidence_threshold))
    print(p)
    cat("\n\n")
  }
}
...
```{r sctype results, eval=sctype_available, results='asis', fig.width=12, fig.height=8}
if(sctype_available && !is.null(sctype_results)){
  cat("**Detected tissue:** ", sctype_results$tissue_type, "\n\n")
  cat("**Database path:** ", sctype_results$database_path, "\n\n")
  cat("**Confidence threshold:** ", sctype_results$confidence_threshold, "\n\n")
  ...
  # 2. scType confidence scores by cell
  if("sctype_scores" %in% colnames(scData_sc@meta.data)){
    cat("#### scType Confidence Scores by Cell\n\n")
    print(FeaturePlot(scData_sc, features = "sctype_scores", reduction = "umap") + 
          ggtitle("scType Confidence Scores") +
          scale_colour_viridis_c())
    cat("\n\n")
  }
  
  # 3. Confidence scores by cluster (bar plot)
  if(!is.null(sctype_results$sctype_scores)){
    cat("#### scType Confidence Scores by Cluster\n\n")
    library(ggplot2)
    score_data <- sctype_results$sctype_scores
    score_data$cluster <- factor(score_data$cluster)
    
    p <- ggplot(score_data, aes(x = cluster, y = scores, fill = type)) +
      geom_bar(stat = "identity") +
      geom_hline(yintercept = score_data$ncells * sctype_results$confidence_threshold, 
                 linetype = "dashed", color = "red", size = 1) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1),
            legend.position = "bottom") +
      labs(title = "scType Confidence Scores by Cluster",
           x = "Cluster", y = "scType Score", fill = "Cell Type",
           subtitle = paste("Red line shows confidence threshold"))
    print(p)
    cat("\n\n")
  }
  
  # 4. Summary table of scType results
  if(!is.null(sctype_results$sctype_scores)){
    cat("#### scType Results Summary\n\n")
    DT::datatable(sctype_results$sctype_scores, 
                  caption = "scType Cell Type Predictions by Cluster",
