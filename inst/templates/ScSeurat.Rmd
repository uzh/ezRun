...
do_ViolinPlot(scData, features ="DecontX_contFrac") + ggtitle("DecontX ambient fraction vs ident") + NoLegend()
...
do_DotPlot(scData, features=genesToPlot) + coord_flip()
```
...
```{r PathwayActivity, eval= !is.null(pathwayActivity),fig.height=12, fig.width=10}
# General heatmap.
...
p1 <- SCpubr::do_DimPlot(scData, label = T)

for (idx in 1:length(out$feature_plots)){
  p2 <- out$feature_plots[[idx]]
  p3 <- out$geyser_plots[[idx]]
  print((p1 | p2) / (p3))
}
```
...
```{r tfActivity, eval= !is.null(TFActivity),fig.height=12, fig.width=10}
# General heatmap.
