...
DimPlot(scData, label=TRUE, group.by = "Azimuth.celltype.l1", repel = TRUE, reduction = "umap") + NoLegend()
if('Azimuth.celltype.l2' %in% colnames(scData@meta.data))
  DimPlot(scData, label=TRUE, group.by = "Azimuth.celltype.l2", repel = TRUE, reduction = "umap")  + NoLegend()
if('Azimuth.celltype.l3' %in% colnames(scData@meta.data))
DimPlot(scData, label=TRUE, group.by = "Azimuth.celltype.l3", repel = TRUE, reduction = "umap")  + NoLegend()
if('Azimuth.celltype.l4' %in% colnames(scData@meta.data))
DimPlot(scData, label=TRUE, group.by = "Azimuth.celltype.l4", repel = TRUE, reduction = "umap")  + NoLegend()
```

### Cell x gene
```{r cellxgene, eval=!is.null(cellxgeneResults), fig.width=10, fig.height=10}
...
print(param$cellxgene_URL)
## get the DOI
d = datasets()
c = collections()
dataset_version_id_URL <- gsub("\\.rds$", "", basename(param$cellxgene_URL))
...
DimPlot(scData, label = TRUE, repel = TRUE, group.by = 'predicted.id.cellxgene.authorlabel', cols = colz) + 
        ggtitle(paste(sample_name, "- CellxGene - Predicted ID"))

## plots of mapping score
FeaturePlot(scData,features = "prediction.score.max", cols = c("lightgrey", "dodgerblue"), pt.size = 1, max.cutoff = 'q97', order = T )+ggtitle(paste(sample_name, "- mapping score"))

## mapping quality by Vlnplots
VlnPlot(scData, features = 'prediction.score.max', group.by = 'predicted.id.cellxgene.authorlabel') +
...
[single cell explorer](`r {paste0("https://fgcz-shiny.uzh.ch/exploreSC", "/?data=",output$getColumn("SC Cluster Report"), "/scData.rds")}`){target="_blank"}
