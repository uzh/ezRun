...
if (!all(is.na(scData$doubletScore))) {
  ...
  cat("Doublet score not available (doublet detection was skipped for this sample).\n")
}
```
...
`r if(cytetype_available) "AI-powered cell type annotation using large language models, applied per cluster from the cluster marker genes."`
...
if (cytetype_available && !is.null(cytetype_results)) {
  ...
  # Backend note: local (internal LLM) vs Nygen cloud
  ct_backend <- if (!is.null(scData@misc$cytetype_jobDetails$backend)) {
    scData@misc$cytetype_jobDetails$backend
  } else {
    "nygen"
  }
  if (ct_backend == "qwen_local") {
    cat(sprintf(
      paste0("Annotations were generated **on FGCZ infrastructure** using the ",
             "internal LLM `%s` (no data left FGCZ). Calls are advisory -- ",
             "verify against canonical markers before relying on them.\n\n"),
      scData@misc$cytetype_jobDetails$model
    ))
  }

  # Show cloud report link if available (Nygen backend only)
