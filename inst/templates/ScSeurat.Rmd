...
clusterSet <- posMarkers$cluster %>% unique() %>% str_sort(., numeric=TRUE)
clusterSetInteger <- clusterSet %>% as.integer() %>% sort()
if ((length(clusterSet) == length(clusterSetInteger)) &&
    any(is.na(clusterSetInteger))){
...
```{r azimuth, eval=!is.null(scData@meta.data$Azimuth.celltype.l2), fig.width=10, fig.height=6}
