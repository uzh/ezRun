...
```{r clustering plot1, fig.height=6, fig.width=12}
...
(p1 + p2) +
  plot_layout(nrow = 1, guides = if (show_leg) "keep" else "none")
  # plot_annotation(
  #   theme = theme(
...
if ("seurat_clusters" %in% names(df)) {
  names(df)[names(df) == "seurat_clusters"] <- "Cluster"
}

is_pct_col <- grepl("_fraction$", colnames(df))
colnames(df) <- colnames(df) |> str_replace_all("_fraction$", " (%)")
is_count_col <- !is_pct_col & colnames(df) != "seurat_clusters"
...
numeric_columns <- apply(as.data.frame(posMarkers), 2, typeof)

ezInteractiveTableRmd2 = function(values, 
  ...
  interactiveTable <- datatable(values, 
                                ...
                                caption=title, colnames=colNames, rownames=rowNames,
...
ezInteractiveTableRmd2(data.frame(posMarkers), rowNames=FALSE, title="foo") |>
  DT::formatSignif(c("pct.1",
                     "pct.2",
                     "avg_log2FC",
                     "p_val_adj",
                     "diff_pct"),
