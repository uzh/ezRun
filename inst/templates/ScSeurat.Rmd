...
qcStats <- ezFrame("Number of Cells"=numeric(0), 
                      "Min Pass Value"=numeric(0), "Max Pass Value"=numeric(0))
qcStats[ "Library Size", ] <- c(sum(allCellsMeta$qc.lib), 
                               range(allCellsMeta$nCount_RNA[allCellsMeta$useCell]))
qcStats[ "Expressed genes", ]<- c(sum(allCellsMeta$qc.nexprs), 
                               range(allCellsMeta$nFeature_RNA[allCellsMeta$useCell]))
qcStats[ "Mitochondrial genes", ]<- c(sum(allCellsMeta$qc.mito), 
                               range(allCellsMeta$percent_mito[allCellsMeta$useCell]))
qcStats[ "Ribosomal protein genes", ]<- c(sum(allCellsMeta$qc.riboprot), 
                               range(allCellsMeta$percent_riboprot[allCellsMeta$useCell]))
