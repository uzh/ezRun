...
if (!is.null(xxAll$percent_ribosomal)){
  plotColData(xxAll, x="Sample", y="percent_ribosomal", colour_by="discard") + ggtitle("Ribosomal percent")
  plotColData(xxAll, x="nCount_RNA", y="percent_ribosomal", colour_by="discard")+ scale_x_log10()
}
...
VlnPlot(scData, features ="percent_ribosomal") + ggtitle("Ribosomal RNA percentage vs ident") + NoLegend()
FeaturePlot(scData, features="percent_ribosomal", label = TRUE)
...
The two heatmaps below display the scores for all individual cells (left) and each original cluster (right) across all reference labels, which allows users to inspect the confidence of the predicted labels across the dataset. The bar Labels on the top shows the actual assigned label.

The single cell (top) and cluster (bottom) annotations are also shown on the UMAPs. Place the mouse over the cells to get information such as their UMAP coordinates, original cluster, the cells name and the label assigned by SingleR. You can also zoom in specific areas of the UMAP by dragging and drop with the mouse.

```{r fig.height=12, fig.width=12, results = 'asis', eval= !is.null(singler.results)}
...
for (r in names(singler.results)) {
  ...
  cat("\n\n")
  clusterInfos <- ezFrame(Sample=as.character(unique(scData$Sample)), Cluster=levels(Idents(scData)))
  ...
  print(ezInteractiveTableRmd(clusterInfos, title = r))
  cat("\n\n")
}
```
...
The iSEE shiny app can be accessed through this link [iSEE explorer](`r {paste0("https://fgcz-shiny.uzh.ch/fgcz_iSEE", sample(0:9, size=1), "/?data=",output$getColumn("SC Cluster Report"))}`){target="_blank"}
...
[clusterInfos.xlsx](clusterInfos.xlsx)
...
param[c("npcs","pcGenes","resolution", "SCT.regress", "DE.method", "cellsFraction", "nUMIs", "nmad", "normalize")]
```
