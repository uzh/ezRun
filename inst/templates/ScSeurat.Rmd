...
if(sctype_available && !is.null(sctype_results)){
  ...
  # 2. Summary table showing cell type counts
  if("sctype_classification" %in% colnames(scData_sc@meta.data)){
    cat("#### scType Results Summary\n\n")
    
    # Create summary of cell type classifications
    sctype_summary <- scData_sc@meta.data %>%
      count(sctype_classification) %>%
      arrange(desc(n)) %>%
      rename("Cell Type" = sctype_classification, "Count" = n) %>%
      mutate(Percentage = round(Count / sum(Count) * 100, 1))
    
    DT::datatable(sctype_summary, 
                  caption = "scType Cell Type Classification Summary",
