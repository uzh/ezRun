...
# invisible placeholder tile
p_invisible <- ggplot() + theme_void() +
  theme(
...
# plotColData(xxAll, x="Sample", 
#             y="nCount_RNA", colour_by="discard") + scale_y_log10() + ggtitle("Number of UMIs")
```
...
qc_dt$Group <- ""
qc_dt$Group[1:5] <- "Cells Removed by QC Thresholds"


qc_dt <- qc_dt[, c("Group", "Metric", setdiff(names(qc_dt), c("Group", "Metric")))]
...
dt <- datatable(
  ...
  caption = tags$caption(
    ...
    HTML(sprintf("<span style='font-weight: normal; font-size: 14px;'>Bin size: %s</span>", param$binSize))
  ),
  escape   = FALSE,
  rownames = FALSE,
  extensions = "RowGroup",
  options  = list(
    ...
    paging = FALSE,
    ordering = FALSE,
    rowGroup = list(dataSrc = 0),  # group by the first column ("Group")
    columnDefs = list(
      list(visible = FALSE, targets = 0)  # hide the Group column itself
    )
  )
)


num_cols <- which(vapply(qc_dt, is.numeric, logical(1)))

if (length(num_cols)) {
  if (length(num_cols) >= 2) {
    dt <- DT::formatRound(dt, columns = num_cols[-length(num_cols)], digits = 0)
    dt <- DT::formatRound(dt, columns = num_cols[length(num_cols)],  digits = 2)
  } else {
