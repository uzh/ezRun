...
for(cond in colnames(metadata(rawData)$design)){
  metadata(rawData)$sampleColors[[cond]] <- setNames(metadata(rawData)$condColors[[cond]][metadata(rawData)$design[[cond]]],
                                                     rownames(metadata(rawData)$design))
}
# metadata(rawData)$condleColors <- lapply(metadata(rawData)$sampleColors, function(x, y){setNames(x,y)},
#                                         rownames(metadata(rawData)$design))
```
...
for(each in c(countFile, signalFile, rpkmFile, tpmFile)){
  cat("\n")
