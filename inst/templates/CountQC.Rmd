...
interactiveTable = head(combinedTopGenes[, useInInteractiveTable, drop=FALSE], 
                        param$maxTableRows) ## restrict number of table rows if necessary
...
#### Sample correlation
```{r gene wise sample correlation data, include=FALSE}
## All genes
...
```{r gene wise sample correlation, echo=FALSE, fig.height=figure.height, fig.width=figure.height*1.25}
## All genes
ezCorrelationPlot(zValues, cond=conds, condLabels=conds,
...
```{r gene wise sample clustering, echo=FALSE, fig.height=7, fig.width=min(7 + (nSamples-10)*0.3, 30), results='asis'}
if (nSamples > 3){
  cat("#### Sample clustering", "\n")
  ## All genes
...
```{r Clustering of High Variance Features, echo=FALSE, message=FALSE, fig.width=max(8,4+0.1*ncol(xNormed)), fig.height=10, results='asis'}
...
```{r MDS plot, echo=FALSE, results='asis', message=FALSE, fig.width=7, fig.height=7}
if(nSamples > 3){
  cat("### MDS Plot", "\n")
  ## Present genes
  ...
  ezMdsPlot(signal=x[topGenes,], sampleColors=sampleColors, 
            main=sub('.png','',pngName))
}
```

```{r scatter plot, echo=FALSE, results='asis'}
if(nSamples > 3){
...
```{r density plot, echo=FALSE, results='asis'}
if(nSamples > 3){
