...
```{r scatter plot all pairs, echo=FALSE, message=FALSE, results='asis', fig.width=min(max(ncol(signalCond) * 2, 4), 20), fig.height=min(max(ncol(signalCond) * 2, 4), 20), eval=ncol(rawData) > 3 & param$writeScatterPlots}
...
for(gridOfPlots in qcScatterFiles){
  ...
  plotGrob <- plot_grid(
    ...
    ncol=nItems,
    byrow=FALSE,
    rel_heights=c(rep(1, nItems - 1), 1.1),
    rel_widths=c(1.1, rep(1, nItems - 1))
  )
  ...
  for(i in 1:length(qcScatterFiles)){ #
    ...
    for(rowOfPlots in rowsOfPlots){
      ...
      grid.arrange(grobs=scatterPlots, 
                   nrow=rowOfPlots$nrow, 
                   widths=c(rep(4, rowOfPlots$ncol), 2), 
                   ncol=rowOfPlots$ncol + 1)
      cat("\n")
    }
    cat("\n")
  }
