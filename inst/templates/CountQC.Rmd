...
if(!exists("rawData")){
  ...
  # rawData <- readRDS("/srv/gstore//projects/p3613/CountQC_49303_2020-09-03--20-54-11/Count_QC/rawData.rds")
...
```{r scatter plot setup, echo=FALSE, message=FALSE}
envir = list(nPlots = 1) ## make sure envir is defined even if there are only two samples
```

```{r scatter plot setup, echo=FALSE, message=FALSE, results='asis', eval=ncol(rawData) > 3 & param$writeScatterPlots}
...
envir <- scatterPlotData$envir
```

```{r scatter plot wide, echo=FALSE, message=FALSE, results='asis', fig.width=min(max(ncol(signalCond) * 2, 4), 20), fig.height=min(max(ncol(signalCond) * 2, 4), 20), eval=ncol(rawData) > 3 & param$writeScatterPlots}

qcScatterFiles <- scatterPlotData$widePlots

for(nm in names(qcScatterFiles)){
  cat("\n")
  cat("####", nm, "\n")
  for(nm2 in names(qcScatterFiles[[nm]])){
    eval(qcScatterFiles[[nm]][nm2], envir=envir)
    cat("\n")
  }
  cat("\n")
}
...
```{r scatter plot narrow, echo=FALSE, message=FALSE, results='asis', eval=ncol(rawData) > 3 & param$writeScatterPlots, fig.width=min(envir$nPlots, 6) * 3 * 1.65, fig.height=max(ceiling(envir$nPlots/6), 1) * 3.3 * 1.65, out.width="1500px"}
...
if(length(qcScatterFiles) > 0){
  for(i in 1:length(qcScatterFiles)){
    ...
    for(j in 1:length(qcScatterFiles[[i]])){
      eval(qcScatterFiles[[i]][j], envir=envir)
      cat("\n")
    }
    cat("\n")
  }
