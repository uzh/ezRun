...
myClusterStats <- data.frame(nClusters = c(2:10), dunn = NA, ch = NA, db = NA, silhouette = NA, entropy = NA, gap = gap_stat$Tab[2:10,3])
for (i in 2:10){
    clusters <- cutree(hc, k = i)
    cluster_scores <- cluster.stats(dist_matrix, clusters)
    myClusterStats$dunn[i-1] <- cluster_scores$dunn
    myClusterStats$ch[i-1] <- cluster_scores$ch
    myClusterStats$db[i-1] <- index.DB(assays(rawData)$log2signalCentered[varFeatures, ], clusters)$DB
    myClusterStats$silhouette[i-1] <- cluster_scores$avg.silwidth
    myClusterStats$entropy[i-1] <- cluster_scores$entropy
}
