...
```{r scatter plot narrow, echo=FALSE, message=FALSE, results='asis', eval=ncol(rawData) > 3 & param$writeScatterPlots, fig.width=min(nPlots, 6) * 3 * 1.65, fig.height=max(ceiling(nPlots/6), 1) * 3.3 * 2.00, out.width="1000px"}


qcScatterFiles <- scatterPlotData$narrowPlots

if(length(qcScatterFiles) > 0){
  for(i in 1:length(qcScatterFiles)){ #length(qcScatterFiles)
    cat("\n")
    cat("####", names(qcScatterFiles)[i], "\n")
    ps <- qcScatterFiles[[i]][[1]]
    for(ps in qcScatterFiles[[i]]){
      scatterPlots <- ps$scatters
      ...
      scatterPlots[[length(scatterPlots) + 1]] <- legend_temp
      grid.arrange(grobs=scatterPlots, 
                   nrow=ps$nrow, 
                   widths=c(rep(4, ps$ncol), 2), 
                   ncol=ps$ncol + 1)
      cat("\n")
    }
    cat("\n")
