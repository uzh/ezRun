...
## start of debug
# require(ezRun)
 rawData <- readRDS("/scratch/test_count_QC/p2223/Count_QC/rawData.rds")
debug <- FALSE
## end of debug
...
if(nSamples > 3){
  ...
  if (sum(use, na.rm=TRUE) > param$minGenesForClustering){
    ...
    if(param$nSampleClusters > 6){
        param$nSampleClusters = 6
    }
    clusterColors = hcl.colors(param$nSampleClusters, palette = "BluGrn")
    clusterResult <-
      clusterPheatmap(xNormed[use, ], design, param, clusterColors,
                      lim=c(-param$logColorRange, param$logColorRange),
                      doClusterColumns=TRUE, sampleColors=sampleColors)
...
if(nSamples > 3){
  ...
  ezMdsPlotly(signal=x, design=design, ndim=3,
              main="mdsPlot_PresentGenes 3D",sampleColors=sampleColors)
}
if(nSamples > 3){
  ezMdsPlotly(signal=x[topGenes,], design=design, ndim=3,
              main="mdsPlot_TopGenes 3D",sampleColors=sampleColors)
}
if(nSamples > 3){
  ezMdsPlotly(signal=x, design=design, ndim=2,
              main="mdsPlot_PresentGenes 2D",sampleColors=sampleColors)
}
if(nSamples > 3){
  ezMdsPlotly(signal=x[topGenes,], design=design, ndim=2,
              main="mdsPlot_TopGenes 2D",sampleColors=sampleColors)
}
```
