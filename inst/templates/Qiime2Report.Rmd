...
#Reports
demux <-output$getColumn("Demux Report")
denoise <-output$getColumn("Denoising stats")
asv_table <-output$getColumn("Feature Table")
rep_seqs <-output$getColumn("Rep Seqs Report")
taxbarplot <-output$getColumn("Taxonomy Barplot")
shannon_report <-output$getColumn("Shannon Diversity")
jaccard_report <-output$getColumn("Jaccard Diversity")
bc_report <-output$getColumn("Bray Curtis Diversity")
jaccard_pca <-output$getColumn("Jaccard Emperor Plot")
bc_pca <-output$getColumn("Bray Curtis Emperor Plot")
alpha_raref <-output$getColumn("Alpha Rarefaction")
difabundance <-output$getColumn("Differential Abundance")
```
...
#### ASVs Frequency Histogram

Here is a histogram reflecting the distribution of the final number of ASVs per sample.

```{r, fig.width = 5, fig.height = 5, echo=FALSE}
ps0_tax_plot <- ggplot(dada2$data, aes(non.chimeric)) +
  geom_histogram() + theme_bw() + ylab("Number of samples") + xlab("Abundance of ASVs")
...
#### Read filtering.  
And here you can find the number of reads filtered at each stage and the final number of reads that remained after denoising (number of ASVs).
...
The counts are then transformed into relative abundance. Lets have a look whether the transformation changed the abundance patters for all the genra.

```{r, fig.height= 10, fig.width = 17, echo=FALSE}
p1 <- plot_bar(physeq1, "Group") + facet_wrap(~Genus, ncol=3) + ggtitle("Before transformation")
p2 <- plot_bar(physeq2, "Group") + facet_wrap(~Genus, ncol=3) + ggtitle("After transformation")
...
If we did want to rarefy, lets look at what we could expect from the data: 
...
### Plots with marked significance

```{r, fig.width = 9, fig.height = 5, echo=FALSE}
metadata_melt <- reshape2::melt(metadata)
lev <- levels(metadata_melt$Group) # get the variables
...
xy <- t(combn(colnames(as.matrix(jaccardmatrix$data)),2))
jaccard_distance <- data.frame(xy, dist=as.matrix(jaccardmatrix$data)[xy])
...
```{r, echo=FALSE}
permanova <- adonis2(jaccardmatrix$data ~ metadata$Group, permutations = 999, method = "jaccard")
knitr::knit_print(paste0("Global PERMANOVA p-value = ", permanova$`Pr(>F)`[1]))
kable(pairwise.adonis(jaccardmatrix$data, metadata$Group, perm = 999, sim.method = "bray")[,c("pairs", "p.value", "p.adjusted", "sig")] %>% mutate(significance = sig) %>% select(c("pairs", "p.value", "p.adjusted", "significance"))) %>% kable_styling()
```
...
```{r, fig.height = 5, fid.width = 8, echo=FALSE}
metadata$SampleID <- as.character(metadata$SampleID)
bc <- t(combn(colnames(as.matrix(braymatrix$data)),2))
bray_distance <- data.frame(bc, dist=as.matrix(braymatrix$data)[bc])
...
```{r, echo=FALSE}
permanova <- adonis2(braymatrix$data ~ metadata$Group, permutations = 999, method = "bray")
knitr::knit_print(paste("Global PERMANOVA p-value = ", permanova$`Pr(>F)`[1]))
kable(pairwise.adonis(braymatrix$data, metadata$Group, perm = 999, sim.method = "bray")[,c("pairs", "p.value", "p.adjusted", "sig")] %>% mutate(significance = sig) %>% select(c("pairs", "p.value", "p.adjusted", "significance"))) %>% kable_styling()
```
...
```{r, fig.height = 5, fid.width = 8, echo=FALSE}
metadata$SampleID <- as.character(metadata$SampleID)
bc <- t(combn(colnames(as.matrix(unifracmatrix$data)),2))
unifrac_distance <- data.frame(bc, dist=as.matrix(unifracmatrix$data)[bc])
...
```{r, echo=FALSE}
permanova <- adonis2(unifracmatrix$data ~ metadata$Group, permutations = 999, method = "uni")
knitr::knit_print(paste("Global PERMANOVA p-value = ", permanova$`Pr(>F)`[1]))
kable(pairwise.adonis(braymatrix$data, metadata$Group, perm = 999, sim.method = "bray")[,c("pairs", "p.value", "p.adjusted", "sig")] %>% mutate(significance = sig) %>% select(c("pairs", "p.value", "p.adjusted", "significance"))) %>% kable_styling()
```
...
```{r, echo=FALSE}
dispersion <- betadisper(unifracmatrix$data, metadata$Group)
...
for (i in colnames(tab_lfc[,grep(grouping,colnames(tab_lfc))])) {
  
  tab_for_plot = tab_lfc %>% 
  dplyr::left_join(tab_se, by = "taxon_id") %>%
  dplyr::transmute(tab_lfc$taxon_id, tab_lfc[,i], tab_se[,paste0(i,"SE")]) %>% 
  ...
  print(ggplot(data = tab_for_plot, 
    ...
    labs(x = NULL, y = "Log fold change", 
       title = paste("Log fold changes as one unit increase of", "Group", strsplit(i,"Group")[[1]][[3]])) + 
...
Result from the ANCOM-BC global test to determine taxa that are differentially abundant between at least two groups across three or more different groups. The result contains: 1) test statistics; 2) p-values; 3) adjusted p-values; 4) indicators whether the taxon is differentially abundant (TRUE) or not (FALSE).
...
ds2 = DESeq(ds2, test="Wald", fitType="mean")
...
- Volcano plot showing under- (negative log fold change) and over- (positive log fold change) represented taxa between reference group and sample group
</br>
...
#plot
g1 = ggplot(res_deseq_edger_sig, aes(x=Genus, y=log2FoldChange, color=Phylum)) + geom_point(size=3) + facet_wrap(~ res_deseq_edger_sig$contrast, ncol = 3) +
...
g2 <- ggplot(res_deseq_edger, aes(x=log2FoldChange, y=log10p, color=Significant, label=TaxonToPrint)) +
  ...
  scale_color_manual(values=c("black","red")) + ggtitle("Vulcano plots per group")

plotGrob <- plot_grid(g1, g2, nrow=2, ncol=1, byrow=FALSE)
print(plot_grid(plotGrob, nrow=1, ncol=1, rel_heights=c(1, 25)))