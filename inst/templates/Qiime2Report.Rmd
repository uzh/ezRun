...
dada2 <- ezLoadRobj("dada2")
metadata <- ezLoadRobj("metadata")
colnames(metadata) <- c("SampleID", "Group")
shannon <- ezLoadRobj("shannon")
evenness <- ezLoadRobj("evenness")
richness <- ezLoadRobj("richness")
shannon <- shannon$data %>% rownames_to_column("SampleID")
richness <- richness$data %>% rownames_to_column("SampleID")
evenness <- evenness$data %>% rownames_to_column("SampleID")
jaccard <- ezLoadRobj("jaccard")
jaccardmatrix <- ezLoadRobj("jaccardmatrix")
jaccardmatrix <- as.matrix(jaccardmatrix$data)
bray <- ezLoadRobj("bray")
braymatrix <- ezLoadRobj("braymatrix")
braymatrix <- as.matrix(braymatrix$data)
unifrac <- ezLoadRobj("unifrac")
unifracmatrix <- ezLoadRobj("unifracmatrix")
unifracmatrix <- as.matrix(unifracmatrix$data)

#Define grouping parameter
params <- ezLoadRobj("param")
grouping <- "Group"
...
```{r, fig.width = 15, fig.height=11, echo=FALSE}
...
x1 <- list()
for (group in levels(as.factor(metadata[,grouping, drop = TRUE]))) {
...
if (length(levels(as.factor(metadata[,grouping, drop = TRUE]))) > 5) {
  venn1 <- UpSet(
    m = m1,
...
df <- reshape2::melt(otu_table(physeq1))
...
Advanced methods of differential abundance testing based on linear regression are applied because sequencing data sets deviate from symmetric, continuous, Gaussian assumptions in many ways. Types of tests that could be applied:
...
#### ANCOM-BC Primary Test Results Genus
...
```{r, echo=FALSE}
res<- ANCOMBC::ancombc2(data = physeq1,
                 fix_formula = grouping, 
                 tax_level = "Genus",
                 p_adj_method = "BH", 
                 lib_cut = 0, 
                 prv_cut = 0.1,
...
```{r, echo=FALSE}
res$res %>%
  select(taxon, starts_with("diff_") & !starts_with("diff_robust_")) %>%
  rename_with(~ gsub("diff_Group|diff_", "", .x), starts_with("diff_")) %>%
  datatable()
```
</br> 
...
```{r, echo=FALSE}
tab_lfc <- res$res %>% select(taxon, starts_with("lfc_"))
tab_lfc %>% 
  datatable(caption = "Log Fold Changes from the Primary Result") %>% 
  formatRound(colnames(tab_lfc)[-1], digits = 2)
```
</br> 
...
```{r, echo=FALSE}
tab_q <- res$res %>% select(taxon, starts_with("q_"))
tab_q %>% 
...
# Build lfc and se tables masked by diff_ significance
diff_cols <- res$res %>% select(starts_with("diff_") & !starts_with("diff_robust_"))
lfc_cols  <- res$res %>% select(starts_with("lfc_"))
se_cols   <- res$res %>% select(starts_with("se_"))

# Align column names
colnames(diff_cols) <- gsub("diff_", "", colnames(diff_cols))
colnames(lfc_cols)  <- gsub("lfc_",  "", colnames(lfc_cols))
colnames(se_cols)   <- gsub("se_",   "", colnames(se_cols))

tab_lfc <- data.frame(lfc_cols * diff_cols, check.names = FALSE) %>%
  mutate(taxon_id = res$res$taxon) %>%
  select(taxon_id, everything())

tab_se <- data.frame(se_cols * diff_cols, check.names = FALSE) %>%
  mutate(taxon_id = res$res$taxon) %>%
  select(taxon_id, everything())
colnames(tab_se)[-1] <- paste0(colnames(tab_se)[-1], "_SE")

group_cols <- colnames(tab_lfc)[grepl("Group", colnames(tab_lfc))]

for (i in group_cols) {
  se_col <- paste0(i, "_SE")
  
  tab_for_plot <- tab_lfc %>%
    left_join(tab_se, by = "taxon_id") %>%
    transmute(taxon_id, fold_change = .data[[i]], SE = .data[[se_col]]) %>%
    filter(fold_change != 0) %>%
    arrange(desc(fold_change)) %>%
    mutate(direct = ifelse(fold_change > 0, "Positive LFC", "Negative LFC"))
  
  tab_for_plot$taxon_id <- factor(tab_for_plot$taxon_id, levels = tab_for_plot$taxon_id)
  tab_for_plot$direct   <- factor(tab_for_plot$direct, levels = c("Positive LFC", "Negative LFC"))
...
```{r, echo=FALSE}
# 1. Table of structural zero taxa
res$zero_ind %>%
  filter(rowSums(.[,-1]) > 0) %>%
  datatable(caption = "Taxa excluded from model due to structural zeros (absent in at least one group)")

# 2. Agglomerate to genus and fix names
physeq_genus <- tax_glom(physeq1, taxrank = "Genus", NArm = FALSE)
tax_df <- as.data.frame(tax_table(physeq_genus))
genus_names <- ifelse(is.na(tax_df$Genus), paste0("Unknown_", seq_len(nrow(tax_df))), tax_df$Genus)
genus_names <- make.unique(genus_names)
taxa_names(physeq_genus) <- genus_names

# 3. Compute observed lfc from physeq_genus (ALL taxa)
log_obs_abn <- as(otu_table(physeq_genus), "matrix")

# Robustly ensure Taxa are rows and Samples are columns
if (!taxa_are_rows(physeq_genus)) {
  log_obs_abn <- t(log_obs_abn)
}

# Apply log transformation
log_obs_abn <- log(log_obs_abn + 1)

# Bias correction using samp_frac from the model (only for the 3 modelled taxa)
# For all taxa we just use raw log abundances as observed lfc
group_info   <- factor(sample_data(physeq_genus)[[grouping]])
ref_group    <- levels(group_info)[1]
other_groups <- levels(group_info)[-1]

group_means <- sapply(levels(group_info), function(g) {
  rowMeans(log_obs_abn[, group_info == g, drop = FALSE])
})

obs_lfc <- as.data.frame(group_means[, other_groups, drop = FALSE] - group_means[, ref_group])
obs_lfc$taxon <- rownames(obs_lfc)

# 4. Significance from model (Keeping your original logic)
sig_df <- res$res %>%
  select(taxon, starts_with("diff_") & !starts_with("diff_robust_")) %>%
  rename_with(~ gsub("diff_Group", "", .x), starts_with("diff_"))

# 5. Structural zero taxa
struc_zero_taxa <- res$zero_ind$taxon

# 6. Plot per group
for (grp in other_groups) {
  tab_for_plot <- obs_lfc %>%
    select(taxon, fold_change = all_of(grp)) %>%
    left_join(sig_df %>% select(taxon, significant = all_of(grp)), by = "taxon") %>%
    mutate(
      significant = ifelse(is.na(significant), FALSE, significant),
      status = case_when(
        significant                ~ "Significant (modelled)",
        taxon %in% struc_zero_taxa ~ "Structural zero",
  ...
  print(
    ggplot(tab_for_plot, aes(x = taxon, y = fold_change, fill = status, color = status)) +
      geom_bar(stat = "identity", width = 0.7) +
      scale_fill_manual(values = c(
        "Significant (modelled)" = "#E41A1C",
        "Not significant"        = "#999999",
        "Structural zero"        = "#377EB8"
      )) +
      ...
      labs(x = NULL, y = "Log fold change (observed)",
           title = paste("Log fold changes vs", ref_group, ":", grp),
           fill = NULL, color = NULL,
           caption = "Red = significant in ANCOMBC2 model | Blue = structural zeros | Grey = not significant") +
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5),
            panel.grid.minor.y = element_blank(),
            axis.text.x = element_text(size = 6, angle = 60, hjust = 1))
...
#### ANCOM-BC Global Test Results Genus
...
#### ANCOM-BC Primary Test Results Family
...
# 2. Agglomerate to family and fix names
physeq_family <- tax_glom(physeq1, taxrank = "Family", NArm = FALSE)
tax_df <- as.data.frame(tax_table(physeq_family))
family_names <- ifelse(is.na(tax_df$Family), paste0("Unknown_", seq_len(nrow(tax_df))), tax_df$Family)
family_names <- make.unique(family_names)
taxa_names(physeq_family) <- family_names

# 3. Compute observed lfc from physeq_family (ALL taxa)
log_obs_abn <- as(otu_table(physeq_family), "matrix")
...
# Bias correction using samp_frac from the model (only for the 3 modelled taxa)
# For all taxa we just use raw log abundances as observed lfc
group_info   <- factor(sample_data(physeq_family)[[grouping]])
ref_group    <- levels(group_info)[1]
other_groups <- levels(group_info)[-1]
...
#### ANCOM-BC Global Test Results Family
