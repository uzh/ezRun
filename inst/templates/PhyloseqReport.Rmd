...
for (rankNames  in taxRanks){
  tempAbundPlotFilt <- abundPlot(rankNames,physeqFullObject,xAesLogic="S",numTopRanks)
...
for (rankNames  in taxRanks){
tempAbundPlotUnfilt <-  abundPlot(rankNames,physeqObjectNoTreeUnfilt,xAesLogic="S",numTopRanks)
...
for (rankNames  in taxRanks){
  ...
  tempAbundPlotFilt <- abundPlot(rankNames,physeqFullObject,xAesLogic="G",numTopRanks)
...
abundPlotGroupList[[rankNames]] <- tempAbundPlotFilt$abPlot +labs(title=plotTitle) + 
  ...
  for (rankNames  in taxRanks){
      tempAbundPlotUnfilt <-  abundPlot(rankNames,physeqObjectNoTreeUnfilt,xAesLogic="G",numTopRanks)
    if (tempAbundPlotUnfilt$stop==TRUE) {
      isAbundPlotGroupUnfilt[[rankNames]] <- FALSE 
    next
  } else {
    isAbundPlotGroupUnfilt[[rankNames]] <- TRUE
plotTitle <- paste("Community composition in each group at the rank", rank, "\nfor the unfiltered dataset.")
...
ordPlotTaxa <- ordPlot(rank,physeqFullObject,"taxa",areThereMultVar,numTopRanks)+
...
ordPlotSample<- ordPlot(rank,physeqFullObject,"samples",areThereMultVar,numTopRanks)+
  labs(title=plotTitle) + 
  theme(plot.title=element_text(size=11,hjust=0.5))
```
...
plotTitle <- paste("Rarefaction plots based on the\nselection of the most abundant", numTopRanks, "ranks")
rarefPlot <- rarefactionPlot(physeqFullObject) +labs(title=plotTitle) + 
    theme(plot.title=element_text(size=11,hjust=0.5))
```
...
### Rarefaction plots
```{r raref, echo=FALSE,out.width="66%"}
plot(rarefPlot)
```
...
```{r deSeqTopTableShow, echo=FALSE,eval=isGroupThere}
