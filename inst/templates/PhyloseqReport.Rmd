...
if (isGroupThere) {
  abundPlotGroup <- plot_bar(physeqFullObject, "Genus", fill="Phylum", facet_grid=~Group) +
...
plotOrdTaxa = plot_ordination(physeqFullObject, ordinateDS, type="biplot") + facet_grid(rows=vars(Genus), cols=vars(Group)) + ggtitle("Taxa ordination") + theme(legend.position = "none") 
```
...
### Heatmaps
```{r heat, echo=FALSE,eval=isGroupThere,out.width="60%"}
show_pHeatmap()
```
