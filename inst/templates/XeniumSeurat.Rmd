...
ggplot(scData@meta.data, aes_string(x = nCountCol, y = nFeatureCol)) +
...
if (isTRUE(has_cooc)) {
 cat(sprintf(paste0(
   ...
   "(attraction), blue = less (avoidance). `*` marks pairs with permutation p < 0.05. ",
...
`r if (!has_split) "SPLIT spatial purification was not run for this sample (enable the doSPLIT parameter). SPLIT removes transcript spill-over from neighbouring cells using the RCTD fit; classification metadata and a purified object (scData.purified.qs2) are produced when enabled."`
...
n_eval <- sum(!is.na(scData$split.celltype_after))
n_changed <- sum(!is.na(scData$split.celltype_after) &
                 before_lab != as.character(scData$split.celltype_after))
cat("\n\n**", n_changed, "/", n_eval, " purified cells (",
    round(100 * n_changed / max(n_eval, 1), 1),
...
pal <- setNames(unname(pals::polychrome())[seq_along(all_types)], all_types)
...
u1 <- DimPlot(scData, reduction = "umap", group.by = "RCTD_Main",
  ...
  coord_fixed() + ggtitle("Before (raw counts)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"), legend.position = "none")
u2 <- DimPlot(scData_purified, reduction = "umap", group.by = "RCTD_Main",
              cols = palu, raster = FALSE, order = TRUE) +
  coord_fixed() + ggtitle("After (purified counts)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
print(u1 + u2)
```
...
prop_df <- dplyr::bind_rows(
  data.frame(stage = "Before", cell_type = as.character(scData$RCTD_Main)),
  data.frame(stage = "After", cell_type = as.character(scData$split.celltype_after))
) |>
