...
**either** (Jaccard co-detection). High values suggest transcript spill-over /
segmentation contamination. No separate single-cell reference is used: markers
are picked from this sample's expression within the RCTD cell types.
...
if ("RCTD_Main" %in% colnames(scData@meta.data) && !all(is.na(scData$RCTD_Main))) {
 ...
 if (length(types) >= 2) {
   ...
   top_per_type <- vapply(types, function(tt) {
     ...
     m <- Matrix::rowMeans(data_mat[, which(ct == tt), drop = FALSE])
     names(which.max(m / (overall + 1e-9)))  # most enriched gene in this type
   }, character(1))
   mecr_markers <- unique(unname(top_per_type))
 }
}
