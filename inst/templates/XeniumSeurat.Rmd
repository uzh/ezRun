...
```{r css_overrides, echo=FALSE, results='asis'}
# fgcz.css sets a global `h1 { font-size: 8px }` (to hide the markdown tabset
# title). ezInteractiveTableRmd renders its `title` as an <h1> inside the table
# caption, so it inherits 8px. Override caption h1 only (keeps the tabset h1 hidden).
cat('<style>caption h1 { font-size: 14px !important; font-weight: 600; color: #2c3e50; margin: 2px 0 6px 0; }</style>\n')
```

```{r load_data}
# Load pre-computed data
scData <- ezLoadRobj("scData")
param <- ezLoadRobj("param")

# Load unfiltered data if available
if (ezRobjExists("scData.unfiltered")) {
  scData_unfiltered <- ezLoadRobj("scData.unfiltered")
  has_unfiltered <- TRUE
} else {
...
## rename by ORIGINAL column name BEFORE dropping empty flags, so the name vector always matches
## the frame. (Renaming after the drop assigned 2 names to a 1-col frame -> render halted whenever
## a flag had zero TRUEs, e.g. minCounts=0 or minFeatures=0.)
names(qcFrame) <- c(qc.lib = "Transcript_Counts", qc.nexprs = "nGenes")[names(qcFrame)]
## select only columns that have at least one flag
qcFrame <- qcFrame[ , apply(qcFrame, 2, any), drop=FALSE]
...
qcMetrics <- tibble(
  ...
  add_row(
    metric = "Library size – total sum of counts across all relevant features for each cell",
...
```{r spatial_qc_blank, fig.width=14, fig.height=6, results='asis'}
...
```{r spatial_qc_control_codeword, fig.width=14, fig.height=6, results='asis'}
...
```{r spatial_qc_control_probe, fig.width=14, fig.height=6, results='asis'}
...
ggplot(scData@meta.data, aes(x = .data[[nCountCol]])) +
...
ggplot(scData@meta.data, aes(x = .data[[nFeatureCol]])) +
 geom_histogram(bins = 50, fill = "steelblue", color = "white", alpha = 0.8) +
 scale_x_log10(labels = label_number()) +
 theme_minimal(base_size = 12) +
...
#### Transcript vs Gene scatter

```{r scatter_transcripts_genes, fig.width=7, fig.height=6}
ggplot(scData@meta.data, aes(x = .data[[nCountCol]], y = .data[[nFeatureCol]])) +
  geom_point(size = 0.3, alpha = 0.3, color = "steelblue") +
  scale_x_log10(labels = label_number()) +
  scale_y_log10(labels = label_number()) +
  theme_minimal(base_size = 12) +
  labs(title = "Transcripts vs genes detected per cell",
       x = "Transcript counts (log10 scale)",
       y = "Genes detected (log10 scale)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
```
...
```{r cell_area, fig.width=14, fig.height=5, results='asis'}
...
if (has_valid_cell_area) {
 ...
 p1 <- ggplot(scData@meta.data, aes(x = .data[[cell_area_col]])) +
...
if (has_valid_nucleus_area) {
 ...
 p1 <- ggplot(scData@meta.data, aes(x = .data[[nucleus_area_col]])) +
...
```{r segmentation_method, fig.width=10, fig.height=8, results='asis'}
...
```{r nucleus_count_barplot, fig.width=8, fig.height=5, results='asis'}
if ("nucleus_count" %in% colnames(scData@meta.data) &&
    !all(is.na(scData$nucleus_count))) {
 nc <- scData$nucleus_count
 nc[is.na(nc)] <- 0
 nc_cat <- ifelse(nc >= 3, "3+", as.character(nc))
 nc_cat <- factor(nc_cat, levels = c("0", "1", "2", "3+"))
 df_nc <- as.data.frame(table(nuclei = nc_cat))
 df_nc$pct <- round(100 * df_nc$Freq / sum(df_nc$Freq), 1)

 print(
   ggplot(df_nc, aes(x = nuclei, y = Freq)) +
     geom_col(fill = "#1f77b4", alpha = 0.85) +
     geom_text(aes(label = paste0(pct, "%")), vjust = -0.3, size = 4) +
     theme_minimal(base_size = 13) +
     labs(title = "Number of nuclei per cell",
          subtitle = "0 = anucleate; >1 = potential segmentation merge",
          x = "Nuclei per cell", y = "Number of cells") +
     theme(plot.title = element_text(hjust = 0.5, face = "bold"))
 )
} else {
 cat("Nucleus count information not available (older Xenium output).")
}
```

#### Signal density

```{r signal_density, fig.width=14, fig.height=5, results='asis'}
if ("signal_density" %in% colnames(scData@meta.data) &&
    any(is.finite(scData$signal_density))) {
 p1 <- ggplot(scData@meta.data, aes(x = signal_density)) +
   geom_histogram(bins = 50, fill = "#16a085", color = "white", alpha = 0.85) +
   scale_x_log10() +
   theme_minimal(base_size = 12) +
   labs(title = "Signal density",
        subtitle = "Transcripts per cell area",
        x = expression(paste("Transcripts / ", mu, m^2, " (log10)")),
        y = "Number of cells") +
   theme(plot.title = element_text(hjust = 0.5, face = "bold"))
 p2 <- ImageFeaturePlot(scData, features = "signal_density",
                        max.cutoff = "q95", size = 0.5) +
   ggtitle("Signal density - Spatial") +
   theme(plot.title = element_text(hjust = 0.5, face = "bold"))
 print(p1 + p2)
} else {
 cat("Signal density not available (requires cell_area).")
}
```

#### Nucleus:cell ratio

```{r nucleus_cell_ratio, fig.width=14, fig.height=5, results='asis'}
if ("nucleus_cell_ratio" %in% colnames(scData@meta.data)) {
 p1 <- ggplot(scData@meta.data, aes(x = nucleus_cell_ratio)) +
   geom_histogram(bins = 50, fill = "#8e44ad", color = "white", alpha = 0.85) +
   theme_minimal(base_size = 12) +
   labs(title = "Nucleus : cell area ratio",
        subtitle = "0 = no detected nucleus; high = nucleus fills cell",
        x = "Nucleus area / cell area", y = "Number of cells") +
   theme(plot.title = element_text(hjust = 0.5, face = "bold"))
 p2 <- ImageFeaturePlot(scData, features = "nucleus_cell_ratio",
                        max.cutoff = "q95", size = 0.5) +
   ggtitle("Nucleus:cell ratio - Spatial") +
   theme(plot.title = element_text(hjust = 0.5, face = "bold"))
 print(p1 + p2)
} else {
 cat("Nucleus:cell ratio not available (requires nucleus_area + cell_area).")
}
```

#### Negative-control fraction

```{r neg_control_frac, fig.width=14, fig.height=5, results='asis'}
ctrl_cols <- intersect(
  c("nCount_ControlProbe", "nCount_ControlCodeword", "nCount_BlankCodeword"),
  colnames(scData@meta.data)
)
if ("neg_control_frac" %in% colnames(scData@meta.data) && length(ctrl_cols) > 0) {
 # Sample-level negative-control rate is the meaningful QC number; the per-cell
 # fraction is ~0 for almost all cells in a healthy run.
 total_ctrl <- sum(as.matrix(scData@meta.data[, ctrl_cols, drop = FALSE]), na.rm = TRUE)
 total_all <- sum(scData$nCount_Xenium, na.rm = TRUE) + total_ctrl
 sample_frac <- round(100 * total_ctrl / total_all, 4)
 pct_any <- round(100 * mean(scData$neg_control_frac > 0, na.rm = TRUE), 1)
 cat("\n\n**Sample-level negative-control rate:** ", sample_frac,
     "% of all transcripts (", format(total_ctrl, big.mark = ","), " control / ",
     format(total_all, big.mark = ","), " total). **", pct_any,
     "%** of cells carry any negative-control transcript",
     " (the rest are exactly 0 - expected for a clean run).\n\n", sep = "")

 nz <- which(scData$neg_control_frac > 0)
 if (length(nz) > 0) {
   p1 <- ggplot(scData@meta.data[nz, ], aes(x = neg_control_frac)) +
     geom_histogram(bins = 50, fill = "#c0392b", color = "white", alpha = 0.85) +
     theme_minimal(base_size = 12) +
     labs(title = "Negative-control fraction (cells with >0 only)",
          subtitle = "Per-cell control / total counts",
          x = "Negative-control fraction", y = "Number of cells") +
     theme(plot.title = element_text(hjust = 0.5, face = "bold"))
   p2 <- ImageFeaturePlot(scData, features = "neg_control_frac",
                          max.cutoff = "q95", size = 0.5) +
     ggtitle("Neg-control fraction - Spatial") +
     theme(plot.title = element_text(hjust = 0.5, face = "bold"))
   print(p1 + p2)
 }
} else {
 cat("Negative-control fraction not available (no control assays detected).")
}
```

### Outlier flags

Cells flagged as morphological / signal outliers (`r paste0(ifelse(is.null(param$qcNmads), 3, param$qcNmads))` MADs).
**Flagged cells are reported for inspection but are NOT removed** - only the
transcript-count and gene-count thresholds remove cells (see Cell filtering).
The table gives the exact criterion and threshold per flag; the spatial map below
colours each flagged cell by its category (multi-flagged cells coloured by priority:
cell area > signal density > negative control > nuclei per cell).

```{r outlier_flags, fig.width=12, fig.height=8, results='asis'}
obj_for_flags <- if (has_unfiltered) scData_unfiltered else scData
nmads <- as.numeric(ifelse(is.null(param$qcNmads), 3, param$qcNmads))
flag_labels <- c(
  outlier.cell_area = "Cell area",
  outlier.signal_density = "Signal density",
  outlier.neg_control = "Negative control",
  outlier.nucleus_count = "Nuclei per cell"
)
flag_cols <- intersect(names(flag_labels), colnames(obj_for_flags@meta.data))

if (length(flag_cols) > 0) {
 md <- obj_for_flags@meta.data
 # --- per-flag criterion + actual threshold + count ---
 crit_for <- function(fc) {
   n <- sum(md[[fc]], na.rm = TRUE)
   crit <- switch(fc,
     outlier.cell_area = {
       thr <- attr(scater::isOutlier(md$cell_area, nmads = nmads, type = "both", log = TRUE), "thresholds")
       sprintf("%g-MAD outlier (both tails, log): cell area < %.0f or > %.0f um2", nmads, thr[1], thr[2])
     },
     outlier.signal_density = {
       sd <- md$signal_density; sd[!is.finite(sd) | sd <= 0] <- NA
       thr <- attr(scater::isOutlier(sd, nmads = nmads, type = "lower", log = TRUE), "thresholds")
       sprintf("%g-MAD outlier (lower tail, log): transcripts/um2 < %.3f", nmads, thr[1])
     },
     outlier.neg_control = {
       thr <- attr(scater::isOutlier(md$neg_control_frac, nmads = nmads, type = "higher"), "thresholds")
       sprintf("%g-MAD outlier (upper tail): neg-control fraction > %.4f", nmads, thr[2])
     },
     outlier.nucleus_count = "fixed rule: nucleus_count == 0 (anucleate) OR > 1 (multinucleate / merged segmentation)",
     fc)
   data.frame(Flag = flag_labels[[fc]], Criterion = crit,
              `Cells flagged` = n, `%` = round(100 * n / nrow(md), 2),
              check.names = FALSE)
 }
 crit_tbl <- do.call(rbind, lapply(flag_cols, crit_for))
 knitr::kable(crit_tbl, format = "markdown", row.names = FALSE) |> cat(sep = "\n")

 any_flag <- Reduce(`|`, lapply(flag_cols, function(cc) { v <- md[[cc]]; v[is.na(v)] <- FALSE; v }))
 cat("\n\n**", sum(any_flag), "** cells (", round(100 * sum(any_flag) / nrow(md), 2),
     "%) flagged by >=1 metric (flagged, NOT removed).\n\n", sep = "")

 # --- spatial map: one colour per flag category (multi-flagged -> by priority,
 #     order: cell area > signal density > neg control > nuclei per cell) ---
 fm <- md[, flag_cols, drop = FALSE]; fm[is.na(fm)] <- FALSE
 cat_vec <- rep("none", nrow(fm))
 for (fc in rev(flag_cols)) cat_vec[fm[[fc]]] <- flag_labels[[fc]]
 lvl <- c(unname(flag_labels[flag_cols]), "none")
 obj_for_flags$outlier_cat <- droplevels(factor(cat_vec, levels = lvl))
 pal_out <- c("Cell area" = "#e74c3c", "Signal density" = "#3498db",
              "Negative control" = "#f39c12", "Nuclei per cell" = "#9b59b6",
              "none" = "grey88")
 print(
   ImageDimPlot(obj_for_flags, group.by = "outlier_cat", size = 0.5,
                cols = pal_out[levels(obj_for_flags$outlier_cat)]) +
     ggtitle("Outlier flags - spatial (one colour per flag category)") +
     theme(plot.title = element_text(hjust = 0.5, face = "bold"))
 )
} else {
 cat("No outlier flags available in metadata.")
}
```

### Segmentation health

Two absolute, sample-level readouts of how well the cell boundaries were drawn.
Deliberately not another MAD flag: a MAD-based score flags a roughly fixed slice
of any dataset, good or bad, so it can never tell you a sample is clean.

```{r segmentation_health, results='asis'}
sh <- xeniumSegmentationHealth(scData@meta.data)
if (!is.na(sh$median_nucleus_cell_ratio)) {
 cat("**Median nucleus : cell area ratio:** ",
     round(sh$median_nucleus_cell_ratio, 3), ". ",
     "A nucleus-seeded boundary with nothing to stop it inflates into empty ",
     "tissue, leaving the nucleus an implausibly small share of the cell. ",
     "Across 7 reference datasets this runs ~0.41-0.50 for stain-segmented runs ",
     "but ~0.32 for DAPI-nucleus + 15 um expansion.\n\n", sep = "")
 cat("**Candidate ballooned cells** (ratio < 0.1): ",
     round(100 * sh$frac_ratio_below_0.1, 2), "%\n\n", sep = "")
} else {
 cat("Nucleus:cell ratio not available (needs nucleus_area + cell_area).\n\n")
}
if (!is.null(sh$segmentation_mix)) {
 mix <- sh$segmentation_mix
 mix$pct <- round(mix$pct, 2)
 cat("Segmentation method mix (cells that fell back to nucleus expansion because ",
     "no stain boundary was found are the ones most likely to be ballooned):\n\n", sep = "")
 print(knitr::kable(mix, format = "markdown", row.names = FALSE))
 cat("\n\n")
} else {
 cat("`segmentation_method` not present (run predates Xenium Onboard Analysis 3.0).\n\n")
}
```

```{r free_unfiltered, include=FALSE}
# Last use of the pre-filter object. It and the SPE copy are each roughly the
# size of scData, and makeRmdReport renders in the SAME R process that still
# holds the app's own copies - so holding all three to the end of the render
# was the main driver of peak memory.
suppressWarnings(rm(list = intersect(c("scData_unfiltered", "spe", "obj_for_flags", "md", "fm"),
                                     ls())))
...
Mutually Exclusive Co-expression Rate: for each pair of cell-type-defining marker
genes (one most-specific gene per RCTD cell type, so each labels a distinct cell
type), the fraction of cells expressing **both** divided by cells expressing
**either** (Jaccard co-detection). Higher values indicate more transcript
spill-over / segmentation contamination.

**Read this as a relative, not an absolute, measure.** The markers are picked
from *this* sample by searching for the most type-specific gene available, on the
same data the rate is then measured on. That search is biased toward exclusive
genes, so the absolute value is optimistic and a low MECR is not by itself proof
of a clean run. It is meaningful for comparing sections processed the same way,
or against a fixed external marker panel. Genes must be detected in at least 10%
of their cell type's cells, and types need at least 50 cells, to be eligible.

```{r mecr, fig.width=8, fig.height=7, results='asis'}
# MECR markers: one most-specific gene per CELL TYPE. Cell types come from the
# RCTD annotation (defined by the single-cell reference), so markers of distinct
# types are expected to be mutually exclusive - matching SpatialQM's
# marker->cell-type design. Falls back to per-cluster markers without annotation.
counts_mat <- GetAssayData(scData, assay = "Xenium", layer = "counts")
mecr_markers <- character(0)
mecr_source <- "cell-type markers (RCTD)"

if ("RCTD_Main" %in% colnames(scData@meta.data) && !all(is.na(scData$RCTD_Main))) {
 ct <- as.character(scData$RCTD_Main)
 data_mat <- GetAssayData(scData, assay = "Xenium", layer = "data")
 # >= 50 cells, not 20. The score below is an enrichment RATIO, and for a gene
 # seen only in type t at cell fraction f the ratio tends to 1/f regardless of
 # expression level - so a tiny type lets a one-cell noise gene win, and a gene
 # detected in ~no cells co-detects with nothing, dragging MECR toward a
 # spuriously clean 0.
 types <- names(which(table(ct) >= 50))
 if (length(types) >= 2) {
   overall <- Matrix::rowMeans(data_mat)
   detected <- data_mat > 0
   top_per_type <- vapply(types, function(tt) {
     # which() drops NA (unannotated cells); ct == tt alone yields NA subscripts
     idx <- which(ct == tt)
     m <- Matrix::rowMeans(data_mat[, idx, drop = FALSE])
     # Detection floor: the gene must actually be expressed in >=10% of this
     # type's cells before it can be called that type's marker.
     detRate <- Matrix::rowMeans(detected[, idx, drop = FALSE])
     score <- m / (overall + 1e-9)
     score[detRate < 0.1] <- -Inf
     if (all(!is.finite(score))) NA_character_ else names(which.max(score))
   }, character(1))
   mecr_markers <- unique(stats::na.omit(unname(top_per_type)))
 }
}
if (length(mecr_markers) < 2 && exists("posMarkers") && nrow(posMarkers) > 0 &&
    "gene" %in% colnames(posMarkers)) {
 pm <- posMarkers
 if ("avg_log2FC" %in% colnames(pm)) pm <- pm[order(pm$cluster, -pm$avg_log2FC), ]
 mecr_markers <- pm |> dplyr::group_by(cluster) |> dplyr::slice_head(n = 1) |>
   dplyr::pull(gene) |> as.character() |> unique() |> head(20)
 mecr_source <- "cluster markers (no annotation)"
}
mecr_markers <- intersect(mecr_markers, rownames(counts_mat))

if (length(mecr_markers) >= 2 && exists("computeXeniumMECR")) {
 mecr_res <- computeXeniumMECR(counts_mat, mecr_markers)
 cat("\n\n**Mean MECR:** ", round(mecr_res$mecr, 3), " over ", mecr_res$n_pairs,
     " pairs of ", length(mecr_res$markers_used), " ", mecr_source, ".\n\n", sep = "")
 pw <- mecr_res$pairwise
 pw_df <- expand.grid(g1 = rownames(pw), g2 = colnames(pw))
 pw_df$rate <- as.vector(pw)
 print(
   ggplot(pw_df, aes(x = g1, y = g2, fill = rate)) +
     geom_tile(color = "white") +
     scale_fill_viridis_c(option = "magma", direction = -1, na.value = "grey90",
                          name = "Co-expr.\nrate") +
     theme(aspect.ratio = 1) +
     theme_minimal(base_size = 11) +
     labs(title = "Marker-pair co-expression (MECR)", x = NULL, y = NULL) +
     theme(axis.text.x = element_text(angle = 45, hjust = 1),
           plot.title = element_text(hjust = 0.5, face = "bold"))
 )
} else {
 cat("Not enough markers available to compute MECR.")
}
```
...
# UMAP second, square panel (house style: theme(aspect.ratio = 1))
...
```{r dotplot_setup}
# Define the eval guard UNCONDITIONALLY (an eval= flag assigned only inside an if-block would be
# undefined when that block is skipped and abort the whole render).
genesToPlot <- character(0)
has_genes_to_plot_clusters <- FALSE
if (nrow(posMarkers) > 0) {
 ...
 has_genes_to_plot_clusters <- length(genesToPlot) > 0
}
```
...
```{r dotplot, fig.width=15, fig.height=20, eval=has_genes_to_plot_clusters}
...
```{r dotplot_none, results='asis', echo=FALSE, eval=nrow(posMarkers) > 0 && !has_genes_to_plot_clusters}
cat("_No matching genes found for the cluster-marker dotplot (top markers absent from the assay)._\n")
```
...
```{r no_banksy, eval=!has_banksy, results='asis'}
if (!is.null(param$banksyError)) {
 cat("<div style='padding:10px;border-left:4px solid #c0392b;background:#fdf1f0'>\n",
     "**BANKSY niche analysis FAILED for this sample.** Reason: `",
     param$banksyError, "`\n\n</div>\n\n", sep = "")
} else {
 cat("BANKSY analysis was not performed.\n\n")
}
```
...
```{r no_niche_markers, eval=!has_banksy || nrow(posMarkersBanksy) == 0, results='asis'}
...
```{r no_niche_genes, eval=has_banksy && nrow(posMarkersBanksy) > 0 && !has_niche_genes, results='asis'}
...
```{r no_enrichr_niches, eval=!has_banksy || nrow(posMarkersBanksy) == 0, results='asis'}
...
```{r no_niche_cluster_heatmap, eval=!has_banksy, results='asis'}
...
# Define these UNCONDITIONALLY. Both are otherwise assigned only inside chunks
# that are themselves gated on has_annotation, and knitr evaluates every `eval=`
# expression eagerly -- a missing symbol aborts the WHOLE render, producing no
# HTML at all. They currently survive only because every consumer happens to
# write the guard left-to-right and R short-circuits `&&`; reorder one guard and
# the default run (rctdReference = 'None') breaks. Same class of bug as the
# has_cooc one fixed below.
has_celltype_markers <- FALSE
has_genes_to_plot <- FALSE

# Report the reference RCTD ACTUALLY used, with the app's precedence (rctdFile overrides the
# dropdown). Prefer param$rctdRefUsed (recorded by the app); fall back to rctdFile / dropdown for
# objects analysed before that field existed. This stops the tab from (a) naming the dropdown atlas
# when a custom file replaced it, and (b) claiming "not performed" when only rctdFile was set.
rctd_ref_name <- if (!is.null(param$rctdRefUsed) && param$rctdRefUsed != "") {
 param$rctdRefUsed
} else if (!is.null(param$rctdFile) && param$rctdFile != "") {
 param$rctdFile
...
```{r no_rctd_ref, eval=!has_annotation || is.null(rctd_ref_name), results='asis'}
...
```{r no_annotation, eval=!has_annotation, results='asis'}
...
```{r niche_celltype_heatmap plot, fig.width=12, fig.height=9, eval=has_banksy && has_annotation_niche}
...
# Draw with explicit padding (extra bottom room for rotated cell-type labels)
draw(ht, padding = unit(c(2, 0.5, 0.5, 0.5), "cm"))  # bottom, left, top, right
```


```{r no_niche_celltype_heatmap, eval=!has_banksy || !has_annotation_niche, results='asis'}
cat("Niche x Cell type heatmap not available (no BANKSY results or no cell type annotation).")
```

### Cell-type co-occurrence

```{r cooccurrence_check}
has_cooc_coords <- has_annotation && length(scData@images) > 0
# has_cooc is really assigned in cooccurrence_compute below, but that chunk is itself gated on
# has_cooc_coords. Without a default here, running with no cell-type annotation (the SUSHI default
# rctdReference='None') leaves has_cooc undefined, and knitr aborts the WHOLE render when it
# evaluates eval=has_cooc on the heatmap chunk -> no HTML is produced at all.
has_cooc <- FALSE
cooc_radius <- if (!is.null(param$coocRadius)) as.numeric(param$coocRadius) else 30
cooc_nperm <- if (!is.null(param$coocNperm)) as.numeric(param$coocNperm) else 1000
cooc_fdr <- if (is.null(param$coocFdr)) TRUE else isTRUE(param$coocFdr)
```

```{r cooccurrence_compute, eval=has_cooc_coords}
# Neighbourhood-enrichment co-occurrence (cached: re-render skips the permutation).
cooc_cache <- "cooccurrence_results.qs2"
cooc <- NULL
if (file.exists(cooc_cache)) {
 cached <- tryCatch(qs2::qs_read(cooc_cache), error = function(e) NULL)
 if (!is.null(cached) && isTRUE(cached$radius == cooc_radius) &&
     isTRUE(cached$n_perm == cooc_nperm) &&
     identical(cached$celltype_col, celltype_col)) {
   cooc <- cached
 }
}
if (is.null(cooc)) {
 coords_df <- Seurat::GetTissueCoordinates(scData)
 if (!all(c("x", "y") %in% colnames(coords_df))) {
   colnames(coords_df)[1:2] <- c("x", "y")
 }
 cell_ids <- if ("cell" %in% colnames(coords_df)) as.character(coords_df$cell) else rownames(coords_df)
 ct_aligned <- scData@meta.data[cell_ids, celltype_col]
 cooc <- computeCelltypeCooccurrence(
   coords_df[, c("x", "y")], ct_aligned,
   radius = cooc_radius, n_perm = cooc_nperm
 )
 cooc$celltype_col <- celltype_col
 tryCatch(qs2::qs_save(cooc, cooc_cache), error = function(e) NULL)
}
has_cooc <- !is.null(cooc$log2enrich) && length(cooc$levels) >= 2
```

```{r cooccurrence_intro, eval=has_cooc_coords, results='asis'}
if (isTRUE(has_cooc)) {
 cat(sprintf(paste0(
   "Neighbourhood enrichment between cell types on a **%g um** radius graph ",
   "(%s cells, mean %.1f neighbours/cell), against **%d** label permutations. ",
   "The permutation keeps the graph fixed, so it controls for cell-type abundance ",
   "and overall density. The colour is the **log2 enrichment** of proximity ",
   "(log2[(observed+1)/(expected+1)], Giotto-style): red = co-occur more than chance ",
   "(attraction), blue = less (avoidance). `*` marks pairs with %s. ",
   "log2 enrichment is a bounded effect size, comparable across cell-type pairs and samples; ",
   "the permutation z-score (computed but not shown) inflates with cell count at Xenium scale, ",
   "so it is not used as the displayed statistic. Colour is capped at the 98th percentile of ",
   "off-diagonal magnitude (the always-large diagonal self co-occurrence saturates).\n\n"),
   cooc$radius, format(cooc$n_cells, big.mark = ","), cooc$mean_degree, cooc$n_perm,
   if (cooc_fdr) "BH-adjusted permutation q < 0.05" else "permutation p < 0.05"))
 if (length(cooc$dropped_types) > 0) {
   cat(sprintf("Cell types with too few cells were dropped: %s.\n\n",
               paste(cooc$dropped_types, collapse = ", ")))
 }
 cat(paste0(
   "**How to read the significance marks.** The null shuffles cell-type labels ",
   "over the whole section, so it is violated by any tissue architecture at all. ",
   "At Xenium cell counts almost every pair reaches the smallest attainable ",
   "p-value (1/(n_perm+1)), including pairs whose only 'signal' is a sub-1% ",
   "depletion that follows geometrically from a third type being clustered. ",
   "Read the **colour** (effect size) first and treat the marks as a weak filter, ",
   "not as evidence of a biological interaction.\n\n"))
} else if (isTRUE(has_cooc_coords)) {
 cat("Fewer than two cell types with enough cells - co-occurrence not computed.\n\n")
}
```

```{r cooccurrence_heatmap, fig.width=8.5, fig.height=7.5, eval=has_cooc}
trunc_ct <- function(x, n = 30) ifelse(nchar(x) > n, paste0(substr(x, 1, n - 3), "..."), x)
lv <- trunc_ct(cooc$levels)
L <- cooc$log2enrich
P <- cooc$pval
# Multiple testing: K(K+1)/2 pairs are tested (210 at 20 cell types) and the
# raw p-values were starred uncorrected. Correct on the unique off-diagonal
# pairs only -- the diagonal is self co-occurrence, not a hypothesis.
if (cooc_fdr) {
 ut <- upper.tri(P)
 q <- P
 q[ut] <- stats::p.adjust(P[ut], method = "BH")
 q[lower.tri(q)] <- t(q)[lower.tri(q)]
 diag(q) <- NA
 P <- q
}
dimnames(L) <- dimnames(P) <- list(lv, lv)

# Cluster once and fix the order for rows and columns (symmetric matrix).
ord <- tryCatch(hclust(dist(L))$order, error = function(e) seq_along(lv))
L <- L[ord, ord]; P <- P[ord, ord]

# Colour scale from OFF-DIAGONAL pairs only: the diagonal (self co-occurrence) is
# always large and would otherwise blow out the scale and hide cross-type structure.
rlim <- function(m, q = 0.98) {
 v <- abs(m[upper.tri(m) | lower.tri(m)])
 l <- stats::quantile(v, q, na.rm = TRUE)
 if (is.finite(l) && l > 0) as.numeric(l) else 1
}
mx <- rlim(L)
col_l <- circlize::colorRamp2(c(-mx, 0, mx), c("#2166AC", "white", "#B2182B"))
star <- function(j, i, x, y, w, h, fill) {
 if (!is.na(P[i, j]) && P[i, j] < 0.05) grid.text("*", x, y, gp = gpar(fontsize = 9))
}
ht <- ComplexHeatmap::Heatmap(
 L, name = "log2\nenrich", col = col_l,
 column_title = "Cell-type co-occurrence (log2 enrichment of proximity)",
 column_title_gp = gpar(fontsize = 12, fontface = "bold"),
 cluster_rows = FALSE, cluster_columns = FALSE,
 row_names_gp = gpar(fontsize = 8), column_names_gp = gpar(fontsize = 8),
 column_names_rot = 45, rect_gp = gpar(col = "grey85", lwd = 0.5), cell_fun = star
)
ComplexHeatmap::draw(ht, padding = unit(c(2, 0.5, 0.5, 0.5), "cm"))
```

```{r cooccurrence_na, eval=!has_cooc_coords, results='asis'}
cat("Cell-type co-occurrence not available (needs cell-type annotation and spatial coordinates).\n\n")
```

### Cell type markers

```{r celltype_markers_compute, eval=has_annotation}
# Read the markers the app pre-computed. They used to be recomputed here with a
# full FindAllMarkers on every render (minutes on a 500k-cell object) and were
# never written out, so the user could not download them - unlike the cluster
# and niche marker tables.
celltype_markers <- data.frame()
if (file.exists("posMarkersCellType.xlsx")) {
 celltype_markers <- as.data.frame(readxl::read_xlsx("posMarkersCellType.xlsx"))
}
if (nrow(celltype_markers) > 0 && !"diff_pct" %in% colnames(celltype_markers)) {
 celltype_markers$diff_pct <- abs(celltype_markers$pct.1 - celltype_markers$pct.2)
}

has_celltype_markers <- nrow(celltype_markers) > 0
```
...
```{r no_celltype_markers, eval=has_annotation && !has_celltype_markers, results='asis'}
...
n_na <- sum(is.na(original_types))
if (n_na > 0) {
 cat("\n\n*Note:* ", n_na, " cells with unassigned cell types are excluded from the dotplot.\n\n", sep = "")
}

# No subset() here. Seurat v5's subset() copies every layer of every assay
# (including the dense scale.data), which on a 500k-cell object was a second
# near-full copy of the object just to drop NA idents -- and DotPlot already
# drops NA groups. Sanitise the labels on a factor instead.
cleaned_types <- gsub("[/\\\\]", "_", original_types)
cleaned_types <- gsub("\\s+", "_", cleaned_types)
cleaned_types <- make.names(cleaned_types)
cleaned_types[is.na(original_types)] <- NA
scData$celltype_clean <- factor(cleaned_types)

# No tryCatch fallback here: a swallowed error printed one skimmable line and
# shipped a report with this whole section silently missing. If the dotplot
# cannot be drawn, the render must fail so it gets fixed.
print(
 DotPlot(scData, features = rev(genesToPlot_celltype), group.by = "celltype_clean") +
   coord_flip() +
   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
...
```{r no_celltype_dotplot, eval=has_annotation && has_celltype_markers && !has_genes_to_plot, results='asis'}
...
```{r no_rctd_class, eval=!has_spot_class, results='asis'}
cat("RCTD classification (spot_class) not available. Run RCTD annotation to generate.")
```

### Prediction scores

```{r prediction_scores_violin, fig.width=14, fig.height=10, eval=has_annotation, results='asis'}
if (!exists("celltype_colors")) {
 celltypes_unique <- sort(unique(na.omit(scData@meta.data[[celltype_col]])))
 n_celltypes <- length(celltypes_unique)
 extended_colors <- c(
...
if (length(weight_cols) > 0) {
 weight_matrix <- as.matrix(scData@meta.data[, weight_cols, drop = FALSE])
 # rowMaxs, not apply(): apply() over 500k rows is minutes of wall time.
 max_weights <- matrixStats::rowMaxs(weight_matrix, na.rm = TRUE)
 max_weights[is.infinite(max_weights)] <- NA
 names(max_weights) <- rownames(weight_matrix)
 # RCTD writes weights for every SCORED cell, including spot_class == "reject"
 # cells it explicitly declined to classify. Showing those on a "prediction
 # confidence" scale reads as high confidence for a non-call, so blank them:
 # RCTD_Main is already NA for exactly those cells.
 if ("spot_class" %in% colnames(scData@meta.data)) {
   max_weights[which(as.character(scData$spot_class) == "reject")] <- NA
 }
...
if (length(weight_cols_sp) > 0 && "rctd_max_weight" %in% colnames(scData@meta.data)) {
 ...
 if (valid_weights_sp > 0) {
   # Cells with no weight (RCTD rejected them, or they fell below UMI_min and
   # were never scored) are drawn grey via na.value, NOT recoded to 0. Mapping
   # them to 0 put three distinct states -- confident, rejected, never-scored --
   # on one 0-1 "confidence" scale, with the caveat printed after the figure.
   na_count <- sum(is.na(scData$rctd_max_weight))
   cat("\n\n", format(valid_weights_sp, big.mark = ","),
       " cells have an RCTD weight; **", format(na_count, big.mark = ","),
       "** (rejected by RCTD, or below UMI_min and never scored) are drawn in grey.\n\n",
       sep = "")

   p1 <- ImageFeaturePlot(scData, features = "rctd_max_weight",
                          max.cutoff = "q95", size = 0.5) +
   ...
   p2 <- FeaturePlot(scData, features = "rctd_max_weight",
                     reduction = "umap", raster = FALSE, order = TRUE) +
...
```{r prediction_scores_summary, eval=has_annotation, results='asis'}
...
## SPLIT purification {.tabset}

```{r split_check}
has_split <- "split.purification_status" %in% colnames(scData@meta.data)
# Distinguish "not requested" from "requested but failed". Previously any
# failure inside the app's SPLIT tryCatch left has_split FALSE, and the report
# told the user to enable a parameter they had already enabled - while the
# Parameters table on the same page said SPLIT = yes.
split_requested <- isTRUE(param$doSPLIT)
split_failed <- split_requested && !has_split
```

```{r split_status_msg, eval=!has_split, results='asis'}
if (split_failed) {
 cat("<div style='padding:10px;border-left:4px solid #c0392b;background:#fdf1f0'>\n",
     "**SPLIT purification was requested (doSPLIT = yes) but FAILED for this sample.** ",
     "No purified object was produced and the results below are unavailable.\n\n",
     if (!is.null(param$splitError))
       paste0("Reason: `", param$splitError, "`\n\n") else
       "See log.txt in the result folder for the reason.\n\n",
     "</div>\n\n", sep = "")
} else {
 cat("SPLIT spatial purification was not run for this sample (enable the doSPLIT ",
     "parameter). SPLIT removes transcript spill-over from neighbouring cells using ",
     "the RCTD fit; classification metadata and a purified object ",
     "(scData.purified.qs2) are produced when enabled.\n\n", sep = "")
}
```

### Classification

SPLIT decides, per cell, whether to replace its counts with the purified version.
The default **neighbourhood-aware** mode marks a cell `purified` only when its
secondary cell type is actually present in its spatial neighbourhood (15um, k=20),
else `raw` (counts unchanged); `reject` cells are `removed`. This is conservative
by design - it avoids decomposing real-but-unreferenced populations. ("full" mode
purifies every cell with a secondary type, regardless of spatial context.)

```{r split_class, fig.width=14, fig.height=6, eval=has_split, results='asis'}
mode_used <- ifelse(is.null(param$splitMode), "neighborhood", as.character(param$splitMode))
thr_used <- ifelse(is.null(param$splitNeighborThreshold), 0.05, param$splitNeighborThreshold)
st <- as.character(scData$split.purification_status); st[is.na(st)] <- "n/a"
scData$split_status_disp <- st
sc_tab <- as.data.frame(table(status = st))
sc_tab$pct <- round(100 * sc_tab$Freq / sum(sc_tab$Freq), 1)
cat("\n\n**Mode:** ", mode_used,
    if (mode_used == "neighborhood") paste0(" (spatial threshold ", thr_used, ", 15um / k=20)") else "",
    ". **Purified (counts replaced):** ", round(100 * mean(st == "purified"), 1),
    "% | kept raw: ", round(100 * mean(st == "raw"), 1),
    "% | removed (reject): ", round(100 * mean(st == "removed"), 1), "%\n\n", sep = "")

pal_st <- c(raw = "#27ae60", purified = "#e67e22", removed = "grey55", "n/a" = "grey85")
p1 <- ggplot(sc_tab, aes(x = reorder(status, -Freq), y = Freq, fill = status)) +
 geom_col(alpha = 0.85) +
 geom_text(aes(label = paste0(pct, "%")), vjust = -0.2, size = 4) +
 scale_fill_manual(values = pal_st) +
 theme_minimal(base_size = 13) +
 labs(title = "SPLIT purification status", x = NULL, y = "Cells") +
 theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold"))

p2 <- ImageDimPlot(scData, group.by = "split_status_disp", size = 0.5,
                   cols = pal_st[sort(unique(st))]) +
 ggtitle("Purification status - Spatial") +
 theme(plot.title = element_text(hjust = 0.5, face = "bold"))
print(p1 + p2)
```
...
```{r split_by_celltype, fig.width=10, fig.height=6, eval=has_split, results='asis'}
ct_col <- if ("RCTD_Main" %in% colnames(scData@meta.data)) "RCTD_Main" else "split.first_type"
if (ct_col %in% colnames(scData@meta.data) && !all(is.na(scData@meta.data[[ct_col]]))) {
 pdf_pure <- scData@meta.data |>
   dplyr::filter(!is.na(.data[[ct_col]])) |>
   dplyr::group_by(cell_type = .data[[ct_col]]) |>
   dplyr::summarise(n_cells = dplyr::n(),
                    purified_rate = round(100 * mean(split.purification_status == "purified", na.rm = TRUE), 1),
                    .groups = "drop") |>
   dplyr::arrange(dplyr::desc(purified_rate))
 print(
   ggplot(pdf_pure, aes(x = reorder(cell_type, purified_rate), y = purified_rate)) +
     geom_col(fill = "#e67e22", alpha = 0.85) +
     geom_text(aes(label = n_cells), hjust = -0.1, size = 3) +
     coord_flip() +
     theme_minimal(base_size = 12) +
     labs(title = "Purified-cell rate by cell type",
          subtitle = "bar label = n cells; % of cells whose counts SPLIT replaced",
          x = NULL, y = "% purified") +
     theme(plot.title = element_text(hjust = 0.5, face = "bold"))
 )
} else {
 cat("Cell type information not available for purity breakdown.")
}
```

### Annotation: before vs after

Cell types re-annotated with RCTD on the **purified** counts (after) vs the
original annotation on raw counts (before). Because SPLIT removes contaminating
transcripts, mixed / contaminated cells may switch label.

```{r split_anno_check}
has_split_anno <- has_split &&
  all(c("split.celltype_after", "RCTD_Main") %in% colnames(scData@meta.data)) &&
  any(!is.na(scData$split.celltype_after))

# Load the purified Seurat object (own UMAP + re-annotation) for the UMAP panel
has_split_umap <- FALSE
scData_purified <- NULL
if (has_split_anno && ezRobjExists("scData.purified")) {
  scData_purified <- ezLoadRobj("scData.purified")
  has_split_umap <- "umap" %in% names(scData_purified@reductions) &&
    "RCTD_Main" %in% colnames(scData_purified@meta.data)
}
```

`r if (has_split && !has_split_anno) "Post-purification re-annotation is not available for this sample (RCTD re-run on purified counts was skipped or failed)."`

```{r split_anno_spatial, fig.width=15, fig.height=7, eval=has_split_anno, results='asis'}
before_lab <- as.character(scData$RCTD_Main)
after_lab <- as.character(scData$split.celltype_after)
after_lab[is.na(after_lab)] <- "removed/NA"

# The denominator must be the PURIFIED cells only. split.celltype_after is
# populated for every RETAINED cell (purified AND raw), so dividing by all of
# them deflated the headline several-fold in the default conservative mode,
# where most cells are kept raw.
after_raw <- as.character(scData$split.celltype_after)
status <- as.character(scData$split.purification_status)
scored <- !is.na(after_raw) & !is.na(before_lab)
isPur <- scored & status == "purified"
isRaw <- scored & status == "raw"

n_changed <- sum(isPur & before_lab != after_raw)
cat("\n\n**", n_changed, "/", sum(isPur), " purified cells (",
    round(100 * n_changed / max(sum(isPur), 1), 1),
    "%) changed cell-type label after purification.**\n\n", sep = "")

# Cells SPLIT left untouched still go through an independent second RCTD run, so
# their label can flip for reasons that have nothing to do with purification.
# That flip rate is the re-run-instability floor for the number above.
if (sum(isRaw) > 0) {
 n_raw_changed <- sum(isRaw & before_lab != after_raw)
 cat("Baseline: **", round(100 * n_raw_changed / sum(isRaw), 1),
     "%** of the ", format(sum(isRaw), big.mark = ","),
     " cells SPLIT left *unpurified* also changed label. Their counts are ",
     "unchanged, so that is re-run instability of RCTD itself, not purification ",
     "- read the number above against it.\n\n", sep = "")
}

all_types <- sort(unique(c(before_lab[!is.na(before_lab)],
                           after_lab[after_lab != "removed/NA"])))
# polychrome() has 36 colours; past that seq_along() silently yields NA colours.
pal_src <- unname(pals::polychrome())
if (length(all_types) > length(pal_src)) {
 pal_src <- grDevices::colorRampPalette(pal_src)(length(all_types))
}
pal <- setNames(pal_src[seq_along(all_types)], all_types)
pal_after <- c(pal, "removed/NA" = "grey85")
scData$split_after_lab <- after_lab

p1 <- ImageDimPlot(scData, group.by = "RCTD_Main", size = 0.5, cols = pal) +
  ggtitle("Before (raw counts)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"), legend.position = "none")
p2 <- ImageDimPlot(scData, group.by = "split_after_lab", size = 0.5, cols = pal_after) +
  ggtitle("After (purified counts)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
print(p1 + p2)
```

UMAP before (raw counts) vs after (re-clustered on purified counts), coloured by
RCTD cell type.

```{r split_anno_umap, fig.width=15, fig.height=7, eval=has_split_umap, results='asis'}
ut <- sort(unique(c(as.character(scData$RCTD_Main),
                    as.character(scData_purified$RCTD_Main))))
ut <- ut[!is.na(ut)]
palu <- setNames(unname(pals::polychrome())[seq_along(ut)], ut)
u1 <- DimPlot(scData, reduction = "umap", group.by = "RCTD_Main",
              cols = palu, raster = FALSE, order = TRUE) +
  theme(aspect.ratio = 1) + ggtitle("Before (raw counts)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"), legend.position = "none")
u2 <- DimPlot(scData_purified, reduction = "umap", group.by = "RCTD_Main",
              cols = palu, raster = FALSE, order = TRUE) +
  theme(aspect.ratio = 1) + ggtitle("After (purified counts)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
print(u1 + u2)
```

```{r split_anno_prop, fig.width=10, fig.height=6, eval=has_split_anno, results='asis'}
# Both stages must be computed over the SAME cells. Taking "Before" over every
# annotated cell and "After" over only the retained ones made a cell type that
# is over-represented among SPLIT's removed cells look like it shrank for
# biological reasons.
keepBoth <- !is.na(scData$RCTD_Main) & !is.na(scData$split.celltype_after)
cat("\n\nBoth bars are computed over the same ", format(sum(keepBoth), big.mark = ","),
    " cells annotated before *and* retained after purification.\n\n", sep = "")
prop_df <- dplyr::bind_rows(
  data.frame(stage = "Before",
             cell_type = as.character(scData$RCTD_Main)[keepBoth]),
  data.frame(stage = "After",
             cell_type = as.character(scData$split.celltype_after)[keepBoth])
) |>
  dplyr::filter(!is.na(cell_type)) |>
  dplyr::count(stage, cell_type) |>
  dplyr::group_by(stage) |>
  dplyr::mutate(pct = 100 * n / sum(n)) |>
  dplyr::ungroup()
prop_df$stage <- factor(prop_df$stage, levels = c("Before", "After"))
print(
  ggplot(prop_df, aes(x = reorder(cell_type, -pct), y = pct, fill = stage)) +
    geom_col(position = "dodge", alpha = 0.85) +
    scale_fill_manual(values = c(Before = "#95a5a6", After = "#2980b9")) +
    theme_minimal(base_size = 12) +
    labs(title = "Cell-type proportions: before vs after SPLIT",
         x = NULL, y = "% of cells") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          plot.title = element_text(hjust = 0.5, face = "bold"))
)
...
```{r explore_links, results='asis'}
sample_name <- unique(scData$Sample)[1]
relative_path <- file.path(param$resultDir, sample_name)
cat("Browse the annotated object interactively - UMAP, spatial view, and per-gene ",
    "expression, coloured by any of the categorical annotations ",
    "(clusters, niches, cell types, QC flags):\n\n", sep = "")
cat("[Open in exploreSC](https://fgcz-shiny.uzh.ch/app/exploreSC/?data=",
    relative_path, "/scData.qs2){target=\"_blank\"}\n\n", sep = "")
cat("<small>Both links are LDAP-protected (B-Fabric login).</small>\n\n")
```

### Data files

```{r data_file_links, results='asis'}
cat("- [posMarkers.xlsx](posMarkers.xlsx) - positive markers of each cluster\n")
cat("- [posMarkersBanksy.xlsx](posMarkersBanksy.xlsx) - positive markers of each BANKSY niche\n")
if (file.exists("posMarkersCellType.xlsx")) {
 cat("- [posMarkersCellType.xlsx](posMarkersCellType.xlsx) - positive markers of each RCTD cell type\n")
}
cat("- [scData.qs2](scData.qs2) - the final annotated Seurat object\n")
if (file.exists("scData.purified.qs2")) {
 cat("- [scData.purified.qs2](scData.purified.qs2) - SPLIT-purified counts, re-clustered and re-annotated\n")
}
if (file.exists("rctd_results.qs2")) {
 cat("- [rctd_results.qs2](rctd_results.qs2) - the post-processed RCTD object\n")
}
cat("- [log.txt](log.txt) - per-step run log (cell counts, RCTD/SPLIT/BANKSY status)\n")
```
...
param_list <- list(
 ...
 "Outlier MADs (flag only)" = ifelse(!is.null(param$qcNmads), param$qcNmads, 3),
 ...
 "SPLIT purification" = ifelse(isTRUE(param$doSPLIT), "yes", "no")
)
