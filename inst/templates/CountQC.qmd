...
settings["Normalisation method:"] <- param$normMethod
...
# plotly sizes in px; knitr's fig-height/fig-width are ignored for widgets.
# layout(height=...) alone only sizes the DRAWN plot — the htmlwidget
# CONTAINER falls back to knitr's fig.height * fig-dpi (e.g. 5in * 300dpi =
# 1500px) and leaves a tall empty band under the plot. Setting p$height on
# the widget object sizes the container to match too. Width is set
# to "100%" (a CSS string, not a pixel number) so the widget fills whatever
# column Quarto's layout-ncol:2 flex gives it (50% of the tab card), instead
# of either fighting that with a forced pixel width (overflows the card) or
# falling back to the site-wide out.width:40% default meant for standalone
# figures (leaves the column mostly empty). MDS's mds() helper below CAN
# hardcode a pixel width because it sits in a custom .fig-grid, not
# layout-ncol.
sizePlot <- function(p) {
  p$width <- "100%"; p$height <- plotHeight
  plotly::layout(p, height = plotHeight, margin = m,
                 xaxis = list(tickangle = 45)) |> plotly::config(responsive = TRUE)
}

## Total reads
plot_ly(toPlot, x = ~samples, y = ~totalCounts, type = "bar") |>
...
plot_ly(toPlot, x = ~samples, y = ~presentCounts, type = "bar",
  ...
  layout(title = "Genomic features with reads above threshold", yaxis = list(title = "Counts")) |>
  sizePlot()
```
...
# Correlation matrices are ~square (N samples x N samples); grow both dimensions
# together with sample count, tied by the same 1.25 width:height ratio the
# pre-Quarto report used, rather than scaling width and height independently.
figure.height <- min(7 + (ncol(rawData) - 10) * 0.3, 30)
figure.width  <- figure.height * 1.25
```
...
::: {.callout-note}
Hierarchical clustering (Ward D2) of samples based on all detected (left) and top 100 most abundant (right) features.
:::
...
if (ncol(rawData) > 3) {
  ...
  if (length(varFeatures) > param$minGenesForClustering) {
    ...
    hasGoTable <- !is.null(clusterResult$GO)
    if (hasGoTable) {
    ...
    # .fig-row/.fig-main/.fig-side (fgcz.scss) place the table beside the plot
    # via flexbox — NOT kableExtra's position="float_right", which used to sit
    # the table on top of/behind the plot's wrapping figure div (CSS floats
    # don't reliably clear a subsequent block-level figure), leaving every link
    # in the table unclickable. Flex items can't overlap this way, and unlike
    # the float they don't force plot-before/after-table source ordering.
    if (hasGoTable) cat("\n::::: {.fig-row}\n:::: {.fig-main}\n")
...
mdsHeight <- 520   # px per plot — tweak to taste
mdsWidth  <- 620   # px per plot — two + gap must fit the .fig-main column; wraps to one-per-row if not
