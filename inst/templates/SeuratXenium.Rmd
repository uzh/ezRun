...
p1 <- ImageFeaturePlot(scData, features = nCountCol, max.cutoff = "q95", size = 1) +
...
p1 <- ImageFeaturePlot(scData, features = nFeatureCol, max.cutoff = "q95", size = 1) +
...
if (length(cell_area_col) > 0 && cell_area_col != "") {
  ...
  p2 <- ImageFeaturePlot(scData, features = cell_area_col, max.cutoff = "q95", size = 1) +
...
if (length(nucleus_area_col) > 0 && nucleus_area_col != "") {
  ...
  p2 <- ImageFeaturePlot(scData, features = nucleus_area_col, max.cutoff = "q95", size = 1) +
...
p1 <- ImageDimPlot(scData, fov = names(scData@images)[1], group.by = "ident",
                   axes = TRUE, cols = cluster_colors, size = 1) +
...
p1 <- ImageDimPlot(scData, fov = names(scData@images)[1], group.by = "banksy_cluster",
  ...
  labs(fill = "Niches") +
  guides(fill = if (show_leg_banksy) guide_legend(title = "Niches") else "none")
...
genesToPlot_banksy <- intersect(genesToPlot_banksy, rownames(scData))
...
Idents(scData) <- "banksy_cluster"
DotPlot(scData, features = rev(genesToPlot_banksy)) +
  coord_flip() +
  scale_y_discrete(limits = as.character(sort(as.integer(levels(Idents(scData)))))) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
```
...
```{r celltype_spatial, fig.width=14, fig.height=7, eval=has_annotation}
# Create named color vector for consistent coloring across plots
celltypes <- sort(unique(na.omit(scData@meta.data[[celltype_col]])))
celltype_colors <- setNames(cluster_colors[seq_along(celltypes)], celltypes)

# Spatial plot first
p1 <- ImageDimPlot(scData, fov = names(scData@images)[1],
                   group.by = celltype_col,
                   axes = TRUE, cols = celltype_colors, size = 1) +
  ggtitle("Cell Types - Spatial") +
  labs(fill = "Cell Type")

# UMAP second with coord_fixed and bigger size
p2 <- DimPlot(scData, group.by = celltype_col, cols = celltype_colors,
              label = TRUE, repel = TRUE, pt.size = 0.8) +
  coord_fixed() +
...
# Use same named color vector as spatial/UMAP plots for consistency
...
# Create cleaned cell type names that match the DotPlot x-axis order
# This must match the transformation applied in celltype_dotplot chunk
celltype_markers_ordered <- celltype_markers |>
  dplyr::mutate(
    cluster_clean = make.names(gsub("\\s+", "_", gsub("[/\\\\]", "_", as.character(cluster))))
  )

# Get top 5 markers per cell type, ordered by diff_pct
top5_celltype <- celltype_markers_ordered |>
  dplyr::group_by(cluster) |>
  dplyr::arrange(desc(diff_pct)) |>
  dplyr::slice_head(n = 5) |>
  dplyr::ungroup()

# For genes appearing in multiple clusters, keep only the assignment with highest diff_pct
# This ensures each gene is assigned to its most representative cluster
top5_unique <- top5_celltype |>
  dplyr::group_by(gene) |>
  dplyr::slice_max(diff_pct, n = 1, with_ties = FALSE) |>
  dplyr::ungroup()

# Sort by CLEANED cluster names (alphabetically) to match DotPlot x-axis order
# Then by diff_pct within each cluster for consistent gene ordering
top5_unique <- top5_unique |>
  dplyr::arrange(cluster_clean, desc(diff_pct))

genesToPlot_celltype <- as.character(top5_unique$gene)
# Seurat converts underscores to dashes in feature names
...
```{r celltype_dotplot, fig.width=15, fig.height=12, eval=has_annotation && has_celltype_markers && has_genes_to_plot}
...
tryCatch({
  # Genes are ordered by cleaned cluster name to create diagonal pattern
  # With coord_flip: genes on y-axis (bottom-to-top), cell types on x-axis (left-to-right)
  # Reverse gene order so Astro genes appear at top, matching x-axis left-to-right order
  ...
  tryCatch({
    ...
    top_genes <- head(genesToPlot_celltype, 20)
...
ImageDimPlot(scData, fov = names(scData@images)[1], group.by = "spot_class",
             axes = TRUE, cols = rctd_class_colors, size = 1) +
...
```{r rctd_na_analysis, eval=has_spot_class}
...
```{r prediction_scores_violin, fig.width=14, fig.height=6, eval=has_annotation}
...
[Vitessce Explorer](`r {relative_path <- sub(".*/srv/gstore/projects/", "", getwd()); paste0("https://fgcz-shiny.uzh.ch/app/exploreVitessceXenium/?data=", relative_path, "/scData.qs2")}`){target="_blank"}
