...
p2 <- DimPlot(scData, group.by = celltype_col, cols = cluster_colors,
...
n_types <- nrow(celltype_counts)
colors_to_use <- cluster_colors[1:min(n_types, length(cluster_colors))]
...
top5_celltype <- celltype_markers |>
  ...
  dplyr::ungroup() |>
  # Sort by cluster (alphabetically) to match y-axis order in dotplot
  dplyr::arrange(cluster, desc(diff_pct))

genesToPlot_celltype <- unique(as.character(top5_celltype$gene))
# Seurat converts underscores to dashes in feature names
...
ImageDimPlot(scData, fov = names(scData@images)[1], group.by = "spot_class",
             axes = TRUE, cols = rctd_class_colors) +
