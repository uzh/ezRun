...
# Heatmap showing cluster composition per niche
pheatmap::pheatmap(
  prop_matrix,
  cluster_rows = FALSE,
  cluster_cols = FALSE,
  ...
  main = "Cluster Composition per Niche",
  angle_col = 45,
  fontsize_row = 10,
  fontsize_col = 10,
...
# Check for cell type annotation (needed before the annotation section)
...
colnames(prop_matrix_ct) <- truncate_label(colnames(prop_matrix_ct), max_chars = 30)

# Heatmap showing cell type composition per niche
pheatmap::pheatmap(
  prop_matrix_ct,
  cluster_rows = FALSE,
  cluster_cols = TRUE,
...
# Filter out cells with NA cell types (e.g., RCTD rejects) to avoid "NA." category
...
if (length(cells_with_celltype) < ncol(scData)) {
  ...
  cat("Note: Excluding", n_na, "cells with unassigned cell types from dotplot.\n\n")
}

# Subset to cells with valid cell types
...
# Clean up subset object to free memory
rm(scData_subset)
```
