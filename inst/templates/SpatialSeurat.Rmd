...
if (!is.null(xxAll$percent_ribo)){
  print(plotColData(xxAll, x="Sample", y="percent_ribo", colour_by="discard") + ggtitle("Ribosomal percent"))
  print(plotColData(xxAll, x="nCount_Spatial", y="percent_ribo", colour_by="discard")+ scale_x_log10())
}
```
...
plot7 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_mito",  pt.size.factor = 3) +
  theme(legend.position = "right")
plot8 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_ribo",  pt.size.factor = 3) +
  theme(legend.position = "right")
...
qcStats[ "Library Size", ] <- c(sum(allCellsMeta$qc.lib), 
                               range(allCellsMeta$nCount_RNA[allCellsMeta$useCell])) %>% as.integer()
qcStats[ "Expressed genes", ]<- c(sum(allCellsMeta$qc.nexprs), 
                               range(allCellsMeta$nFeature_RNA[allCellsMeta$useCell])) %>% as.integer()
...
qcStats[ "Doublets", ]<- c(sum(allCellsMeta$qc.doublet), NA, NA)
qcStats[ "Total removed", ]<- c(sum(!allCellsMeta$useCell), NA, NA)
qcStats[ "Cells remaining", ]<- c(sum(allCellsMeta$useCell), NA, NA)
