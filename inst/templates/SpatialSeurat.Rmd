...
Spatial QC Plots - cropped version

```{r QC plots3, fig.width=28}
plot5 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "nCount_Spatial", pt.size.factor = 3 * param$imageEnlargementFactor) + 
  theme(legend.position = "right")
plot6 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "nFeature_Spatial",  pt.size.factor = 3 * param$imageEnlargementFactor) +
  theme(legend.position = "right")
plot7 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_mito",  pt.size.factor = 3 * param$imageEnlargementFactor) +
  theme(legend.position = "right")
plot8 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_riboprot",  pt.size.factor = 3 * param$imageEnlargementFactor) +
  theme(legend.position = "right")

plot5 + plot6 + plot7 + plot8 + plot_layout(nrow = 1, ncol = 4)
```

Spatial QC Plots - uncropped version

```{r QC plots3_b, fig.width=28}
plot5 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "nCount_Spatial", pt.size.factor = 3, crop = FALSE) + 
  theme(legend.position = "right")
plot6 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "nFeature_Spatial",  pt.size.factor = 3,  crop = FALSE) +
  theme(legend.position = "right")
plot7 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_mito",  pt.size.factor = 3,  crop = FALSE) +
  theme(legend.position = "right")
plot8 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_riboprot",  pt.size.factor = 3,  crop = FALSE) +
  theme(legend.position = "right")
...
p1 <- DimPlot(scData, label = FALSE) + labs(color = "seurat_clusters")
p1 <- LabelClusters(p1, id = "ident",  fontface = "bold", color = "black", size = 5)
p2 <- SpatialDimPlot(scData, label = TRUE, label.size = 4, pt.size.factor = 3 * param$imageEnlargementFactor) +
...
```{r, fig.height=8, fig.width=20}
p2 <- SpatialDimPlot(scData, label = TRUE, label.size = 4, pt.size.factor = 3, crop = FALSE) +
  labs(fill = "seurat_clusters")
...
SpatialDimPlot(scData, cells.highlight = CellsByIdentities(scData), facet.highlight = TRUE, pt.size.factor = 3 * param$imageEnlargementFactor, ncol = 4, cols.highlight = c('red', 'white'))
```
...
for (eachCluster in levels(posMarkers$cluster)) {
  ...
  for (eachMarker in markersPerCluster) {
    sp = SpatialFeaturePlot(object = scData, features = eachMarker, alpha = c(0.1, 1), pt.size.factor = 3 * param$imageEnlargementFactor)
    print(sp)
  }
  for (eachMarker in markersPerCluster) {
    sp = SpatialFeaturePlot(object = scData, features = eachMarker, alpha = c(0.1, 1), pt.size.factor = 3, crop = FALSE)
    print(sp)
  }
  cat("\n")
}
...
for(gene in top.features) {
   sp = SpatialFeaturePlot(object = scData, features = gene, alpha = c(0.1, 1), pt.size.factor = 3 * param$imageEnlargementFactor)
   print(sp)
}
```
