...
We use a heatmap and a dotplot to visualize simultaneously the top 5 markers in each cluster. Be aware that some genes may be in the top markers for different clusters.

<br/>
```{r markers plots}
top5 <- posMarkers %>% group_by(cluster) %>% top_n(n = 5, wt = diff_pct)
SpatialFeaturePlot(object = scData, features = top5$gene, alpha = c(0.1, 1), ncol = 4)
```
...
spatial_markers$cluster = as.factor(spatial_markers$cluster)
spatial_markers$gene = as.factor(spatial_markers$gene)
ezInteractiveTableRmd(spatial_markers, digits=4)
```

### Spatially Genes plots 

```{r markers plots}
top5 <- spatialMarkers %>% group_by(cluster) %>% top_n(n = 5, wt = diff_pct)
SpatialFeaturePlot(object = scData, features = top5$gene, alpha = c(0.1, 1), ncol = 4)
```
