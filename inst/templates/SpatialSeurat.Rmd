...
### Azimuth 
```{r azimuth, eval=!is.null(aziResults), fig.width=10, fig.height=10}
DimPlot(scData, label=TRUE, group.by = "Azimuth.celltype.l1", repel = TRUE, reduction = "umap")
SpatialDimPlot(scData, label = TRUE, label.size = 4, pt.size.factor = 2 * param$imageEnlargementFactor) + labs(fill = "Azimuth.celltype.l1") + theme(aspect.ratio = myRatio)
...
if('Azimuth.celltype.l2' %in% colnames(scData@meta.data)){
    ...
    SpatialDimPlot(scData, label = TRUE, label.size = 4, pt.size.factor = 2 * param$imageEnlargementFactor) + labs(fill = "Azimuth.celltype.l2") + theme(aspect.ratio = myRatio)
}
if('Azimuth.celltype.l3' %in% colnames(scData@meta.data)){
    ...
    SpatialDimPlot(scData, label = TRUE, label.size = 4, pt.size.factor = 2 * param$imageEnlargementFactor) + labs(fill = "Azimuth.celltype.l3") + theme(aspect.ratio = myRatio)
}
if('Azimuth.celltype.l4' %in% colnames(scData@meta.data)){
    ...
    SpatialDimPlot(scData, label = TRUE, label.size = 4, pt.size.factor = 2 * param$imageEnlargementFactor) + labs(fill = "Azimuth.celltype.l4") + theme(aspect.ratio = myRatio)
}
```

### Interactive
[simple explorer](`r {paste0("https://fgcz-shiny.uzh.ch/exploreSpatial", "/?data=",output$getColumn("SC Cluster Report"), "/scData.rds")}`){target="_blank"}
