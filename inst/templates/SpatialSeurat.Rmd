...
plot5 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "nCount_Spatial", pt.size.factor = 3) + 
  theme(legend.position = "right")
plot6 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "nFeature_Spatial",  pt.size.factor = 3) +
  theme(legend.position = "right")
plot7 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_mito",  pt.size.factor = 3) +
  theme(legend.position = "right")
plot8 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_riboprot",  pt.size.factor = 3) +
  theme(legend.position = "right")
...
p2 <- SpatialDimPlot(scData, label = TRUE, label.size = 4, pt.size.factor = 3) +
...
SpatialDimPlot(scData, cells.highlight = CellsByIdentities(scData), facet.highlight = TRUE, pt.size.factor = 3, ncol = 4, cols.highlight = c('red', 'white'))
```
...
for (eachCluster in levels(posMarkers$cluster)) {
  ...
  for (eachMarker in markersPerCluster) {
    sp = SpatialFeaturePlot(object = scData, features = eachMarker, alpha = c(0.1, 1), pt.size.factor = 3)
    print(sp)
  }
  cat("\n")
}
...
for(gene in top.features) {
   sp = SpatialFeaturePlot(object = scData, features = gene, alpha = c(0.1, 1),pt.size.factor = 3)
   print(sp)
}
```
