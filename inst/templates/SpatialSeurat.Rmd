...
if(is.null(param$imageEnlargementFactor)){
    param$imageEnlargementFactor <- 1
}
coord <- GetTissueCoordinates(object = scData@images$slice1)
...
plot5 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "nCount_Spatial", pt.size.factor = 2 * param$imageEnlargementFactor) + theme(legend.position = "right") + theme(aspect.ratio = myRatio)
plot6 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "nFeature_Spatial",  pt.size.factor = 2 * param$imageEnlargementFactor) + theme(legend.position = "right") + theme(aspect.ratio = myRatio)
plot7 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_mito",  pt.size.factor = 2 * param$imageEnlargementFactor) + theme(legend.position = "right") + theme(aspect.ratio = myRatio)
plot8 <- SpatialFeaturePlot(scData.unfiltered_spatial, features = "percent_riboprot",  pt.size.factor = 2 * param$imageEnlargementFactor) + theme(legend.position = "right") + theme(aspect.ratio = myRatio)

plot5 + plot6 + plot7 + plot8 + plot_layout(nrow = 2, ncol = 2)
```
<br/>

b.) based on all spots without image based filtering by SpaceRanger

```{r QC plots4, fig.width=24, fig.height=10}
if(param$imageEnlargementFactor >= 1.5){
plot1 <- SpatialFeaturePlot(scDataRaw, features = "nCount_Spatial", pt.size.factor = 2.5, min.cutoff = 'q1', max.cutoff = 'q50') + theme(legend.position = "right") 
plot2 <- SpatialFeaturePlot(scDataRaw, features = "nFeature_Spatial", pt.size.factor = 2.5, min.cutoff = 'q1', max.cutoff = 'q50') + theme(legend.position = "right")
} else {
    plot1 <- SpatialFeaturePlot(scDataRaw, features = "nCount_Spatial", pt.size.factor = 2.5 * param$imageEnlargementFactor, min.cutoff = 'q1', max.cutoff = 'q50') + theme(legend.position = "right") 
    plot2 <- SpatialFeaturePlot(scDataRaw, features = "nFeature_Spatial", pt.size.factor = 2.5 * param$imageEnlargementFactor, min.cutoff = 'q1', max.cutoff = 'q50') + theme(legend.position = "right")  
}
plot1 + plot2 + plot_layout(nrow = 1, ncol = 2)
```
<br/>
...
qcStats <- ezFrame("Number of Spots"=integer(0), 
                      "Min Pass Value"=integer(0), "Max Pass Value"=integer(0))
qcStats[ "Library Size", ] <- c(sum(allCellsMeta$qc.lib), 
                               range(allCellsMeta$nCount_Spatial[allCellsMeta$useCell])) %>% as.integer()
qcStats[ "Expressed genes", ]<- c(sum(allCellsMeta$qc.nexprs), 
                               range(allCellsMeta$nFeature_Spatial[allCellsMeta$useCell])) %>% as.integer()
qcStats[ "Mitochondrial Percent", ]<- c(sum(allCellsMeta$qc.mito), 
                               range(allCellsMeta$percent_mito[allCellsMeta$useCell])) %>% as.integer()
qcStats[ "Ribosomal protein Percent", ]<- c(sum(allCellsMeta$qc.riboprot), 
                               range(allCellsMeta$percent_riboprot[allCellsMeta$useCell])) %>% as.integer()
qcStats[ "Total removed", ]<- c(sum(!allCellsMeta$useCell), NA, NA)
qcStats[ "Spots remaining", ]<- c(sum(allCellsMeta$useCell), NA, NA)
...
p2 <- SpatialDimPlot(scData, label = TRUE, label.size = 4, pt.size.factor = 2 * param$imageEnlargementFactor) +
...
SpatialDimPlot(scData, cells.highlight = CellsByIdentities(scData), facet.highlight = TRUE, pt.size.factor = 3 * param$imageEnlargementFactor, ncol = 4, cols.highlight = c('red', 'white')) & theme(aspect.ratio = myRatio)
```
...
barplot = ggplot(data=cellIdents_perSample, aes(x=seurat_clusters, fill=Batch)) + geom_bar(stat="Count")
...
for (eachCluster in levels(posMarkers$cluster)) {
  ...
  for (eachMarker in markersPerCluster) {
    sp = SpatialFeaturePlot(object = scData, features = eachMarker, alpha = c(0.1, 1), pt.size.factor = 3 * param$imageEnlargementFactor) + theme(aspect.ratio = myRatio)
    print(sp)
  }
  cat("\n")
}
...
top.features <- head(spatialMarkers$GeneSymbol, 20)
for(gene in top.features) {
   sp = SpatialFeaturePlot(object = scData, features = gene, alpha = c(0.1, 1), pt.size.factor = 3 * param$imageEnlargementFactor) + theme(aspect.ratio = myRatio)
   print(sp)
}
```
...
##### Mean expression of every gene across the spots in each cluster

[geneMeanPerCluster](gene_means_per_cluster.tsv)

##### Mean expression of every gene across all the spots
