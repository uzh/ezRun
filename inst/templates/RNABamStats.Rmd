...
if (length(resultList[[1]][["genebody_coverage"]]) != 0){ ## TODO this could be done better by searching alls results for a valid genebody_coverage element
    ...
    maxYlim = 0.03 ## this means we allow at most 10-fold enrichment at a percentile #max(sapply(resultList, function(item){max(item[["genebody_coverage"]])}))
    gbcTemplate = resultList[[1]][["genebody_coverage"]]
    #regions = intersect(c("less than 500nt", "1000 to 1200nt"), rownames(pngMatrix))
    for (rn in names(gbcTemplate)){
      for (cn in names(gbcTemplate[[1]])){ #"medium expressed"){ #}
