...
if (length(resultList[[1]][["genebody_coverage"]]) != 0){ ## TODO this could be done better by searching alls results for a valid genebody_coverage element
  ...
  gbcp <- ggplot(data=xx, aes(x=Percentile, y=value)) + geom_line(aes(color=Sample)) + 
...
if(!is.null(resultList[[1]][["Junction"]])){
  ...
  for (nm in names(resultList[[1]][["Junction"]][["junctionSaturation"]])){
        ...
        junctionPlots[[nm]] <- ggplot(data=junctionDF, aes(x=readFraction, y=Junctions, group = Sample)) + geom_line(aes(col=Sample)) + geom_point(aes(col=Sample), size = 1)
        junctionPlots[[nm]] <- ggplotly(junctionPlots[[nm]] + labs(x="percent of total reads", y="Number of splicing junctions (K)") + ggtitle(nm))
  }
}
```
...
if(!is.null(resultList[[1]][["dupRate"]])){
  ...
  dupPlot <- ggplotly(ggplot(data = dupResult, aes(x=Slope, y=Intercept)) + geom_point(aes(col=Sample), size = 2) + ggtitle('MultiSample Plot'))
}
```
