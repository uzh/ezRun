...
# library(Seurat)
# library(AUCell)
# sce_list <- readRDS("~/workspaceR/ezRun/web_summary.html/sce_list.rds")
# param = metadata(sce_list$sce)$param
...
## Preprocessing and downstream analysis {.tabset}
...
plotColData(sce.unfiltered, x="Batch", y="subsets_Mito_percent", colour_by="discard") + ggtitle("Ribo percent")
```
...
A standard approach is to filter cells with a low amount of reads as well as genes that are present in at least a certain amount of cells. While simple, using fixed thresholds requires knowledge of the experiment and of the experimental protocol. An alternative approach is to use adaptive, data-driven thresholds to identify outlying cells, based on the set of QC metrics just calculated. To obtain an adaptive threshold, we assume that most of the dataset consists of high-quality cells. A cell was considered an outlier for a specific QC metric if its log-transformed library size was more than `r param$nmad` MADs (median absolute deviation) below the median. An outlier for any of these metrics is considered to be of low-quality and discarded. 
A summary of the number of cells removed for each QC metric is shown below.

```{r low quality cells}
df <- data.frame(sum = colData(sce.unfiltered)$sum, detected=colData(sce.unfiltered)$detected, subsets_Mito_percent = colData(sce.unfiltered)$subsets_Mito_percent, subsets_Ribo_percent=colData(sce.unfiltered)$subsets_Ribo_percent)
reasons <- quickPerCellQC(df, percent_subsets=c("subsets_Mito_percent", "subsets_Ribo_percent"))
...
We next aimed to identify a subset of features (i.e. genes) exhibiting high variability across cells, and therefore represent heterogeneous features to prioritize for downstream analysis. We first applied a variance-stabilizing transformation to correct for the mean-variance relationship that is inherent to single-cell RNA-seq, as first outlined by Mayer, Hafemeister & Bandler et al. By default, we return 2,000 features.
Next, we apply a linear transformation (‘scaling’) that is a standard pre-processing step before dimensional reduction techniques. With this procedure, the expression of each gene is shifted, so that the mean expression across cells is 0. It also scales the expression of each gene, so that the variance across cells is 1 to avoid the results to be dominated by the signal of a few highly-expressed genes.

```{r hvg plot, fig.align='center'}
top10 <- head(VariableFeatures(scData), 10)
plot_hvg <- VariableFeaturePlot(scData)
...
DimPlot(scData, reduction = "umap", label = TRUE, pt.size = 0.7, label.size = 5, repel = TRUE) 
```
...
stopifnot(all(colnames(sce) == colnames(scData)))
## Get clustering results from the Seurat object
clust_seurat <- scData@meta.data %>% 
...
```{r doublet tsne}
FeaturePlot(scData, features = "scDblFinder.score", reduction = "tsne")
...
```{r singler, eval= (param$species != "other"), results='asis', echo=FALSE, message=FALSE}
cat("\n")
cat(paste0("SingleR can be used through this shiny app: [SingleR browser](http://fgcz-shiny.uzh.ch/fgcz_SingleRBrowser_app/?data=" , output$getColumn("Report"), "/singler.browser.rds)"))
```
...
```{r assigments heatmap, eval= (param$species != "other"), results='asis',fig.align='center', fig.width=6, fig.height=6}
cat("We can explore the cell assignment results using different plots. Below, we show a heatmap with the cell type each cell was assigned to. Gray color represents the successful assignment of the cell to the cell type and black non-assignment. Cell types with no cells assigned are not shown.")
cat("\n\n")
...
The iSEE shiny app can be accessed through this link [iSEE explorer](`r paste0("http://fgcz-shiny.uzh.ch/fgcz_iSEE/?data=",output$getColumn("Report"),"/sce.rds")`)
...
##### The final Single Cell Experiment Object is [here](sce.rds)

### Parameters
```{r report parameters, echo=FALSE}
param[c("npcs","pcGenes","resolution")]
```
