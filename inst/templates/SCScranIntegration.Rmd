...
sce <- SingleCellExperiment(assays=list(counts=do.call(cbind, lapply(rescaled, assay, "counts")),
...
assay(sce.mnn, "counts") <- do.call(cbind,
                                  lapply(rescaled,
                                         function(x){assay(x[chosen, ], "counts")}))
assay(sce.mnn, "logcounts") <- do.call(cbind,
                                  lapply(rescaled,
...
sce.mnn <- switch(metadata(sce.mnn)$param$visMethod,
                  "TSNE"=runTSNE(sce.mnn, use_dimred="PCA",
                                 perplexity=getPerplexity(ncol(sce.mnn))),
                  "UMAP"=runUMAP(sce.mnn, use_dimred="PCA"),
                  "DiffusionMap"=runDiffusionMap(sce.mnn, use_dimred="PCA")
                  )
```

#### Visualization of integration

```{r vis in low-dim, echo=FALSE}
