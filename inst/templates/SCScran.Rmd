...
```{r normalize2, echo=FALSE, cache=TRUE}
...
```{r mean-variance3, echo=FALSE, cache=TRUE}
...
```{r dim reduction1, echo=FALSE, cache=TRUE}
set.seed(1000)
...
```{r vis in low-dim, echo=FALSE, cache=TRUE}
set.seed(1000)
...
if(metadata(sce)$param$visMethod=="tSNE"){
  plotTSNE(sce, colour_by="Cluster", text_by="Cluster") + scale_fill_hue() +
    labs(fill="Cluster")
}
...
```{r markers, echo=FALSE, include=FALSE, cache=TRUE}
