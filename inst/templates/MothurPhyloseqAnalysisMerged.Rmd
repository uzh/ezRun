...
mergedAbPlotGroupClass <- plot_bar(physeqMerged, "Group", "Abundance", fill="Class") + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="stack")
...
plotTreeRadialMerged <- plot_tree(forRadialTree,  color="Group", shape="Family", ladderize="left", size = "abundance", label.tips="Genus") + coord_polar(theta="y") + ggtitle("BActeroidetes-only tree")
...
tableToPlot <- tableToPlot[tableToPlot$padj < 0.05,]
...
#### Summary table (20 most significant OTUs)
```{r table, echo=FALSE}
