...
# data10X <- Read10X(paste0(metadata(sce.singlets)$output$getFullPaths('ResultDir'), "/filtered_feature_bc_matrix"))
# data10X_GeneExp <- data10X$`Gene Expression`
# cellCycleFn <- list.files(path=paste0(metadata(sce.singlets)$output$getFullPaths('ResultDir'), "/filtered_feature_bc_matrix"),
...
#                                       colData=colData)
#   sce_list <- filterCellsAndGenes(sce, param)  #return sce objects filtered and unfiltered to show the QC
...
When the parameter values of nreads, ngenes and perc_mito are specified, fixed thresholds are used for filtering. Otherwise, filtering is performed excluding cells that are outliers by more than `param$nmad` MADs below the median for the library size and the number of genes detected. Cells with a percentage counts of mitochondrial genes above the median by `param$nmad` MADs are also excluded. 
...
kable(tibble("QC metric" =c("Library Size", "Expressed genes", "Mitochondrial genes", "Total removed", "Cells remaining"),
             "Number of cells"= c(num.qc.lib, num.qc.nexprs, num.qc.mito, removed, ncol(sce))),
      row.names=FALSE,
...
A UMAP can help us to determine whether cell cycle is a major source of variation in our dataset. 

```{r UMAP on cell cycle, fig.align='center'}
plotUMAP(sce.singlets, colour_by="CellCycle") + ggtitle("Cell cycle phase")
```
...
variable <- rowData(sce.singlets)[rowData(sce.singlets)$sct.variable == TRUE,]
top10 <- rownames(variable[order(variable$sct.residual_variance, decreasing=TRUE),][1:10,])
plot_hvg <- VariableFeaturePlot_mod(sce.singlets)
...
cells_prop = cellsProportion(sce, groupVar1 = "ident", groupVar2 = "HTO_classification")
...
```{r heatmap, fig.width=8, fig.height=length(levels(sce.singlets$seurat_clusters))*0.8}
top5 <- posMarkers %>% group_by(cluster) %>% top_n(n = 5, wt = diff_pct)

tr_cnts <- expm1(logcounts(sce.singlets))
geneMeans <- rowsum(DelayedArray::t(tr_cnts), group=sce.singlets$seurat_clusters)
geneMeans <- sweep(geneMeans, 1, STATS=table(sce.singlets$seurat_clusters)[rownames(geneMeans)], FUN="/")
...
plotDots(sce.singlets, features=top5$gene, group="seurat_clusters")
```
...
```{r set singler heatmap height}
singler_heatmap_height= 1
if(param$species != "other")
  singler_heatmap_height= length(unique(singler.single.labels))*0.7
```


```{r singler ScoreHeatmap,  eval= (param$species != "other"), fig.height=singler_heatmap_height, fig.width=8, results='asis'}
...
```{r set AUC heatmap height}
auc_heatmap_height= 1
if(param$species != "other")
  auc_heatmap_height= nrow(tab)*0.5
```

```{r assigments heatmap, eval= (param$species != "other"), results='asis',fig.align='center', fig.width=10, fig.height=auc_heatmap_height}
...
AUCell_plotTSNE(tSNE=cellsTsne, exprMat=counts(sce.singlets), cellsAUC=cells_AUC)
```
...
param[c("nmad", "cellsFraction", "npcs","pcGenes","resolution", "SCT.regress", "DE.method")]
```
