...
options(ggrepel.max.overlaps = Inf) # never drop a cluster label because the UMAP is crowded

# Absent in objects from before the knob existed, where SCTransform was the only option
normalizationMethod <- if (ezIsSpecified(param$normalizationMethod)) param$normalizationMethod else "SCTransform"

# Figure width for a categorical axis: 0.25 inch per label, so 40+ clusters do not overlap.
figWidthFor <- function(nLabels) {
  max(10, 2 + 0.25 * nLabels)
}

# Legend below the panel, as many columns as the label length allows in figWidth inches, so
# a long batch list cannot squeeze the panel. `aesthetic` is "fill" (bars) or "color" (DimPlot).
legendBelow <- function(labels, figWidth, aesthetic = "fill") {
  # 9 pt text is ~0.07 in per character; 1 in of slack for the legend title and margins
  legendCols <- max(1, floor((figWidth - 1) / (0.07 * max(nchar(as.character(labels))) + 0.6)))
  guide <- guide_legend(ncol = legendCols, override.aes = list(size = 3))
  list(theme(legend.position = "bottom", legend.text = element_text(size = 9),
             legend.title = element_text(size = 9), legend.key.size = unit(0.35, "cm"),
             legend.key.spacing.y = unit(1, "pt")),
       do.call(guides, setNames(list(guide), aesthetic)))
}

# Cells per cluster, stacked by a grouping column.
cellCountBarplot <- function(scData, ident, group) {
  cellCounts <- scData@meta.data[, c(ident, group)]
  ggplot(cellCounts, aes(x = .data[[ident]], fill = .data[[group]])) +
    geom_bar() +
    labs(x = "Cluster", y = "Number of cells", fill = group) +
    legendBelow(cellCounts[[group]], figWidthFor(length(unique(cellCounts[[ident]]))))
}
```
...
# Check if cellxgene annotation already in metadata (loaded from individual samples)
# or load from file if available
if ("predicted.id.cellxgene.authorlabel" %in% colnames(scData@meta.data)) {
  cellxgeneResults <- TRUE
...
```{r UMAP batch, fig.width=10, fig.height=7}
if(!('Batch' %in% colnames(scData@meta.data)))
  scData@meta.data[['Batch']] <- scData@meta.data[['Sample']]
DimPlot(scData, reduction =  "umap_noCorrected", group.by = "Batch") + theme(aspect.ratio = 1) + legendBelow(scData$Batch, 10, "color")
DimPlot(scData, reduction="umap_noCorrected", group.by="ident_noCorrected", label = TRUE, repel = TRUE) + theme(aspect.ratio = 1)
```

```{r barplot batch, fig.width=figWidthFor(length(unique(scData$ident_noCorrected))), fig.height=7}
cellCountBarplot(scData, "ident_noCorrected", "Batch")
...
```{r UMAP Condition, eval=length(unique(scData$Batch)) != length(unique(scData$Condition)), fig.width=8, fig.height=6}
#if there are only two samples these plots are the same as the previous ones
DimPlot(scData, reduction =  "umap_noCorrected", group.by = "Condition") + theme(aspect.ratio = 1)
```

```{r barplot Condition, eval=length(unique(scData$Batch)) != length(unique(scData$Condition)), fig.width=figWidthFor(length(unique(scData$ident_noCorrected))), fig.height=5}
cellCountBarplot(scData, "ident_noCorrected", "Condition")
...
if (normalizationMethod == "LogNormalize") {
  cat("We started by merging all the samples in one dataset and then log-normalized the raw filtered counts: each cell's counts are divided by that cell's total, scaled to 10,000, and log1p-transformed. The most variable genes were identified with the variance-stabilizing (vst) method, and the data were then centred and scaled.")
} else {
...
cat("After inspecting the datasets and observing that cells clustered by sample we decided to integrate samples using shared highly variable genes. Oftentimes, when clustering cells from multiple conditions there are condition-specific clusters and integration can help ensure the same cell types cluster together.\n We started by normalizing, estimating the variance of the raw filtered data, and identifying the most variable genes in each sample separately.")
if (normalizationMethod == "LogNormalize") {
  cat("For this we used log-normalization: each cell's counts divided by that cell's total, scaled to 10,000 and log1p-transformed, followed by variance-stabilizing (vst) feature selection. Because log-normalization works one cell at a time, doing it per sample is identical to doing it on the merged object.")
} else {
  cat("For this, we used the SCtransform method from the Seurat package which accounts for cellular sequencing depth, or nUMIs by default.")
}
cat("\n")
if(ezIsSpecified(param$SCT.regress.CellCycle) && 
   param$SCT.regress.CellCycle) {
...
if (param$integrationMethod == "Harmony") {
  harmonyGroupBy <- if (ezIsSpecified(param$harmonyGroupBy)) param$harmonyGroupBy else "Condition"
  cat(paste0(" Harmony corrected for the metadata column(s): ", paste(harmonyGroupBy, collapse = ", "), "."))
}
...
```{r UMAP per Batch after correction, fig.width=10, fig.height=7}
DimPlot(scData, reduction="umap", group.by="Batch") + theme(aspect.ratio = 1) + legendBelow(scData$Batch, 10, "color")
p2 <- DimPlot(scData, reduction="umap", label = FALSE, pt.size = 1.5, cols = shared_colors) + labs(color = "ident") + theme(aspect.ratio = 1)
LabelClusters(p2, id = "ident", repel = TRUE, fontface = "bold", color = "black", size = 6)
```

```{r barplot per Batch after correction, fig.width=figWidthFor(nlevels(Idents(scData))), fig.height=7}
cellCountBarplot(scData, "ident", "Batch")
...
```{r UMAP per Condition after correction, eval=length(unique(scData$Batch)) != length(unique(scData$Condition)), fig.width=8, fig.height=6}
DimPlot(scData, reduction="umap", group.by="Condition") + theme(aspect.ratio = 1)
```

```{r barplot per Condition after correction, eval=length(unique(scData$Batch)) != length(unique(scData$Condition)), fig.width=figWidthFor(nlevels(Idents(scData))), fig.height=5}
cellCountBarplot(scData, "ident", "Condition")
...
```{r PCA elbow plot, fig.align='center', fig.width=14}
  k <- param$npcs

  top_n <- max(80, k)

  stdev <- scData[["pca"]]@stdev %>% head(top_n)
...
```{r cell-cycle-umap, eval =!is.null(scData@meta.data$CellCycle), fig.width=10, fig.height=6}
if (any(!is.na(scData$CellCycle))){
  DimPlot(scData, label=TRUE, repel = TRUE, split.by = "CellCycle", reduction="umap")
}
```

```{r qc violins, fig.width=figWidthFor(nlevels(Idents(scData))), fig.height=5}
VlnPlot(scData, features ="nCount_RNA", pt.size = 0) + ggtitle("Number of UMIs vs cluster") + NoLegend()
VlnPlot(scData, features ="nFeature_RNA", pt.size = 0) + ggtitle("Number of genes vs cluster") + NoLegend()
VlnPlot(scData, features ="percent_mito", pt.size = 0) + ggtitle("Mitochondrial percentage vs cluster") + NoLegend()
VlnPlot(scData, features ="percent_riboprot", pt.size = 0) + ggtitle("Ribosomal percentage vs cluster") + NoLegend()
```

```{r uninteresting variation, fig.width=6, fig.height=6}
FeaturePlot(scData, features="nCount_RNA", reduction="umap", label = TRUE, repel = TRUE)
FeaturePlot(scData, features="nFeature_RNA", reduction="umap", label = TRUE, repel = TRUE)
FeaturePlot(scData, features="percent_mito", reduction="umap", label = TRUE, repel = TRUE)
FeaturePlot(scData, features="percent_riboprot", reduction="umap", label = TRUE, repel = TRUE)
...
```{r clustree, fig.align='center', fig.width=6, fig.height=8}
# Read the prefix off the metadata instead of assuming the assay: FindClusters names
# these <assay>_snn_res.<r>, so it is SCT_, RNA_ or integrated_ depending on the run.
resCols <- grep("_snn_res\\.", colnames(scData@meta.data), value = TRUE)
if (length(resCols) == 0) {
  stop("no <assay>_snn_res.<resolution> columns in the object; clustree cannot run")
}
clustree::clustree(scData, prefix = paste0(sub("_snn_res\\..*$", "", resCols[1]), "_snn_res."))
```
...
```{r marker plot setup, results='asis'}
...
if (ncol(scData) > 3e4) {
  ...
  cat("\n\nThe heatmap and dotplot below are drawn on a random subset of at most 30'000 of the",
      format(ncol(scData), big.mark = "'"), "cells.\n\n")
  scDataToPlot <- scData[, sample(colnames(scData), size=3e4, replace=F)]
} else {
  scDataToPlot <- scData
}

# Reorder Idents numerically if all cluster levels are numeric
cluster_levels <- levels(Idents(scDataToPlot))
if (all(varhandle::check.numeric(cluster_levels))) {
  new_levels <- cluster_levels[order(as.numeric(cluster_levels))]
  Idents(scDataToPlot) <- factor(Idents(scDataToPlot), levels = new_levels)
}


```

```{r marker heatmap, fig.width=figWidthFor(nlevels(Idents(scData))), fig.height=2 + 0.2 * length(genesToPlot)}
ezDoHeatmap(scDataToPlot, features=unique(genesToPlot)) + guides(color = "none")
```

```{r marker dotplot, fig.width=figWidthFor(nlevels(Idents(scData))), fig.height=2 + 0.2 * length(genesToPlot)}
# coord_flip() puts the first feature at the bottom; reversed, cluster 0's markers read from the top like the heatmap
DotPlot(scDataToPlot, features=rev(genesToPlot)) + coord_flip()
```
...
# Enrichr rejects over-long gene lists: keep the top 500 markers per cluster by fold change (same cap as ScSeurat)
topMarkers <- posMarkers %>% group_by(cluster) %>% slice_max(avg_log2FC, n = 500, with_ties = FALSE) %>% ungroup()
genesPerCluster <- split(as.character(topMarkers$gene), topMarkers$cluster)
...
```{r singler heading, eval=!is.null(singler.results), results='asis'}
...
```{r cellxgene, eval=!is.null(cellxgeneResults), fig.width=9, fig.height=9}
...
p1 <- DimPlot(scData, label = TRUE, repel = TRUE, group.by = 'cellxgene_label_short', cols = shared_colors) +
  ggtitle("Combined Samples - CellxGene - Predicted ID") +
...
p2 <- FeaturePlot(scData, features = "prediction.score.max", cols = c("lightgrey", "dodgerblue"), pt.size = 1, max.cutoff = 'q97', order = TRUE) +
  ggtitle("Combined Samples - Mapping Score")
...
if (ezIsSpecified(param$cellxgeneUrl)) {
...
<a class="btn btn-primary btn-lg" style="font-size:1.5em; margin:0.5em 0 1em 0;" target="_blank" href="`r paste0("https://fgcz-shiny.uzh.ch/exploreSC", "/?data=", output$getColumn("Report"), "/scData.qs2")`">Open the Single Cell Explorer</a>

This is the main interactive deliverable of the report: a Shiny app to browse every cluster, annotation and gene of the final object, and to make production-ready plots and feature plots of your own markers.
...
param[c("npcs","pcGenes","resolution", "normalizationMethod", "SCT.regress.CellCycle", "DE.method", "integrationMethod", "harmonyGroupBy")]
```
