...
```{r mRNAMultiSample, echo=FALSE, results='hide', warning=FALSE, message=FALSE, eval=!debug}
###TODO: multisample plot for top 3 Hits (multi+unique hits)
```
...
```{r KrakenTopHits, echo=FALSE, results='hide', warning=FALSE, message=FALSE, eval=!debug}
...
#uniqueSpecies <- unique(as.vector(speciesPerSample))
uniqueSpecies <- unique(unlist(speciesPerSample))
...
for (i in 1:min(3, nrow(krakenResult_DF))){
  par(mar=c(15.1, 10.1, 4.1, 2.1))
    x <- unlist(krakenResult_DF[i, samples])
    bplot <- barplot(height = x, names.arg = rep('',length(samples)), col = 'royalblue3', 
                     main = paste0('TopHit', i,':', krakenResult_DF$Species[i]), ylab = 'Mapped Reads in %', las = 2)
    text(x = bplot, y = par("usr")[3] - (max(x) * 0.04), srt = 45, adj = 1, cex = 0.7,
         labels = samples, xpd = TRUE)
  }
}
```
