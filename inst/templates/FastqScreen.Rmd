...
if(length(refseqSamples) > 1){
  ...
  if(is.vector(speciesPerSample)){
      speciesPerSample <- t(data.frame(speciesPerSample))
  }
  readFractionPerSample_uniq <- sapply(refseqResult, function(listElem) return(listElem$UniqueSpeciesPercent))
  if(is.vector(readFractionPerSample_uniq)){
     readFractionPerSample_uniq <- data.frame(t(readFractionPerSample_uniq))
     rownames(readFractionPerSample_uniq) <- unique(sapply(refseqResult, rownames))
     colnames(readFractionPerSample_uniq) <- refseqSamples
  }
  readFractionPerSample_multi <- sapply(refseqResult, function(listElem) return(listElem$MultipleSpeciesPercent))
  if(is.vector(readFractionPerSample_multi)){
     readFractionPerSample_multi <- data.frame(t(readFractionPerSample_multi))
     rownames(readFractionPerSample_multi) <- unique(sapply(refseqResult, rownames))
     colnames(readFractionPerSample_multi) <- refseqSamples
  }
  ...
  refSeqResult_DF_uniq <- refSeqResult_DF_uniq[rowSums(refSeqResult_DF_uniq[,refseqSamples]) > 0,]
  refSeqResult_DF_uniq <- refSeqResult_DF_uniq[order(apply(refSeqResult_DF_uniq[,refseqSamples], 1, mean), decreasing = TRUE),]
...
if(length(refseqSamples) > 1){
  ...
  if (nrow(refSeqResult_DF_uniq) > 0){
    i = 1
    data <- unlist(refSeqResult_DF_uniq[i, refseqSamples]) + unlist(refSeqResult_DF_multi[i, refseqSamples])
    p <- plot_ly(x=refseqSamples,
                 y=data,
                 type="bar") %>%
...
if(length(krakenSamples) > 1){
  ...
  if(is.matrix(speciesPerSample) & is.matrix(readFractionPerSample)){  
    if(length(uniqueSpecies) > 0){
      for (nm in samples){
      ...
      krakenResult_DF <- krakenResult_DF[rowSums(krakenResult_DF[,krakenSamples]) > 0,]
      krakenResult_DF <- krakenResult_DF[order(apply(krakenResult_DF[,krakenSamples], 1, mean), decreasing = TRUE),]
    }
  } else {
...
if(!skipMultiSampleKraken){
  ...
  if (nrow(krakenResult_DF) > 1){
    i <- 2
    data <- unlist(krakenResult_DF[i, krakenSamples])
    p <- plot_ly(x=samples,
                 y=data,
                 type="bar") %>%
      layout(title = paste0('TopHit', i,':', krakenResult_DF$Species[i]),
  ...
  for (nm in krakenSamples){
      ...
      if (length(x[x > 0]) > 0){
          ...
          text(x = bplot, y = par("usr")[3] - 0.3, srt = 45, adj = 1, 
               labels = krakenResult_DF$Species[x > 0], xpd = TRUE)
      }
  }
}
```
