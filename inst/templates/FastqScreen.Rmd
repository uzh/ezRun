...
```{r KrakenPerSample, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
for (i in c(1:length(krakenResult))){
      par(mar=c(15.1, 4.1, 4.1, 2.1))
        bplot = barplot(krakenResult[[i]]$readFraction, names.arg = krakenResult[[i]]$name, col = 'royalblue3', 
                main = names(krakenResult)[i], ylab = 'Mapped Reads in %', las = 2)
}
```
