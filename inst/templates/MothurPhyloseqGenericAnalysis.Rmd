...
illAbPlot <- plot_bar(physeqFullObject, "Group", "Abundance", "Phylum") + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="stack")
```

```{r prepOrd, include=FALSE}
### create plots: 2. ordination
illOrd <- ordinate(physeqFullObject, "NMDS", "bray")
plotOrdTaxaIll = plot_ordination(physeqFullObject, illOrd, type="taxa", color="Phylum") + facet_wrap(~Phylum, 3) + ggtitle("Taxa ordination")
plotOrdSamplesIll = plot_ordination(physeqFullObject, illOrd, type="samples", color="Group") 
...
plotRichIll <- plot_richness(physeqFullObject, x="Group", measures=c("Simpson", "Shannon"), color="Group")
```

```{r prepTree, include=FALSE}
### create plots: 4. tree
plotTreeIll <- plot_tree(physeqFullObject , ladderize="left", color="Group")
```

```{r prepHeat, include=FALSE}
### create plots:5. heatmap
plotHeatmapIll <- plot_heatmap(physeqFullObject, taxa.label="Phylum")
```
