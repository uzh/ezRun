...
```{r singler, eval= !is.null(organism), results='hide', message=FALSE}
...
singler = CreateSinglerObject(as.matrix(GetAssayData(scData_v3, slot = "counts")), annot = NULL, project.name = scData_v3@project.name,
                              min.genes = 0, technology = "10X", species = organism,
                              citation = "", normalize.gene.length = F,
                              variable.genes = "de", fine.tune = T,
                              do.signatures = F, clusters=scData_v3@active.ident,
                              do.main.types = T, reduce.file.size=T,
                              temp.dir = NULL, numCores = param$cores)
...
```{r singlerbrowser, eval= !is.null(organism), results='asis'}
