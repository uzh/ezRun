...
# sce <- readRDS("/srv/gstore/projects/p2284/SCReport_28929_2018-10-18--10-06-03/AVM_17_08_2018_SCReport/sce.rds")
...
## Significant markers
cm <- markers[#which(markers$p_val_adj<0.05),
...
We eliminated the genes that are expressed in < 5 cells. We eliminated the cells that have < `r param$minGenesPerCell` genes detected, or > `r param$maxGenesPerCell` genes.
...
if(doEnrichr(param)){
  ...
  kable(enrichrTable, format="html", escape=FALSE,
        caption="Geneset enrichment") %>%
    kable_styling("striped", full_width = F, position = "left")
}
```

```{r markers table, echo=FALSE}
caption ="Expression differences of cluster marker genes (p_val_adj < 0.05)"
...
```{r heatmap, fig.width=10, fig.height=20, echo=FALSE, eval=!debug}
...
### Pseudotime
```{r pseudo time plot, echo=FALSE, fig.width=8, fig.height=5, eval=param$runPseudoTime, eval=!debug}
