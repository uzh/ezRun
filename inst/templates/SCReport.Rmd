...
geneMeans <- rowsum(as.matrix(t(tr_cnts)), group=scData@ident)
...
```{r pseudo time, echo=FALSE, message=FALSE}
getYellowBlueScale <- function(n=256, whiteLevel=0.9){

  # from blue to red going by white
  cs <- character(n)
  n1 <- ceiling(n/2)
  cs[1:n1] <- rgb(seq(from=1, to=whiteLevel, length.out=n1),
                  seq(from=1, to=whiteLevel, length.out=n1),
                  seq(from=0, to=whiteLevel, length.out=n1)
  )
  n2 <- n - n1
  cs[(n1+1):n] <- rev(rgb(seq(from=0, to=whiteLevel, length.out=n2),
                          seq(from=0, to=whiteLevel*(n2-1)/(n1-1), length.out=n2),
                          seq(from=1, to=whiteLevel*(n2-1)/(n1-1), length.out=n2)
  ))
  cs
}
...
dt <- data.frame(pseudotime=a$A[1,])
rownames(dt) <- colnames(cd)

rbPal <- colorRampPalette(getYellowBlueScale())
col.cols1 = rbPal(10)[as.numeric(cut(dt$pseudotime,breaks = 10))]
names(col.cols1)=colnames(cd)
tSNE_data = data.frame(X=scData@dr$tsne@cell.embeddings[colnames(cd),1],
                       Y=scData@dr$tsne@cell.embeddings[colnames(cd),2])
plot(tSNE_data,col="black", bg=col.cols1, cex=2,
     pch=21,xlab="",ylab="", cex.axis=0.8)
```
