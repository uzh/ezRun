...
# sce <- readRDS("/srv/gstore/projects/p2284/SCReport_30969_2018-11-07--15-44-11/Metastasis_17102018_SCReport/sce.rds")
...
We eliminated the genes that are expressed in < `r param$minCellsPerGene` cells. We eliminated the cells that have < `r param$minGenesPerCell` genes detected, or > `r param$maxGenesPerCell` genes.
...
if(doEnrichr(param)){
  ...
  kable(enrichrTable, format="html", escape=FALSE,
        caption=paste0("GeneSet enrichment: gens with pvalue <= ", pvalue_allMarkers)) %>%
...
if(doEnrichr(param) && param$all2allMarkers){
  ...
  genesPerCluster <- lapply(all2allMarkers, rownames)
  jsCall = paste0('enrich({list: "', sapply(genesPerCluster, paste, collapse="\\n"), '", popup: true});')
  enrichrCalls <- paste0("<a href='javascript:void(0)' onClick='", jsCall, 
                         "'>Analyse at Enrichr website</a>")
  enrichrTable <- tibble(Comparison=names(genesPerCluster),
                         "# of markers"=lengths(genesPerCluster),
                         "Enrichr link"=enrichrCalls,
                         "List of markers"=text_spec(paste0(names(all2allMarkers), ".tsv"),
                                                     link=paste0(names(all2allMarkers), ".tsv")))
  kable(enrichrTable, format="html", escape=FALSE,
        caption=paste0("GeneSet enrichment: gens with pvalue <= ", pvalue_all2allMarkers)) %>%
    kable_styling("striped", full_width = F, position = "left")
}
```
...
### Pseudotime
```{r pseudo time plot, echo=FALSE, fig.width=8, fig.height=5, eval=param$runPseudoTime && !debug}
