...
fa = system.file("extdata/genome.fa", package="ezRun", mustWork = TRUE)
...
# Conventions for feature annotation files

Feature annotation files must be in gtf file format. If the input file is in gff format, it can be converted to gtf with the `gffread` utility from the cufflinks suite
```
