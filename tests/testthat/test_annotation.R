context(
  "Test annotation and gtf: annotation.r; gff.r; go-analysis.R; ngsReferenceFiles.r"
)
...
test_that("Tests basic functions in gff.r", {
  ...
  newAttribute <- ezBuildAttributeField(
    mcols(gtfGR)[, c("gene_id", "gene_name", "transcript_id")],
    "gtf"
  )
...
test_that("Tests annotation functions related to GO (from go-analysis.R)", {
  ...
  goList <- goStringsToList(c(
    "GO:0034762; GO:0034763",
    "GO:0034764; GO:0034765"
  ))
...
test_that("Tests cleanGenomeFiles() from ngsReferenceFiles.r", {
  cleanedGenome <- cleanGenomeFiles(
