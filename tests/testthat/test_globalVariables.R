...
globalVarsFile = system.file("extdata/EZ_GLOBAL_VARIABLES.txt", package="ezRun", mustWork=TRUE)
...
test_that("Test file paths", {
  globalPaths = globalVars[-which(containsWhiteSpace | isInternetAddress | !hasSlash),]
  names(globalPaths) = varNames[!(containsWhiteSpace | isInternetAddress | !hasSlash)]
